...
$cora_equipment = ( in_array( $sub_page, array( 'dashboard', 'equipment', 'leads', 'bookings' ) ) ) ? get_option( 'cora_equipment_inventory', array() ) : array();
...
$cora_galleries = get_option( 'cora_studio_galleries', array() );
$cora_leads = get_option( 'cora_studio_leads', array() );
$cora_clients = get_option( 'cora_studio_clients', array() );

// Pre-process equipment assignments dynamically from Leads and Clients databases
if ( is_array( $cora_equipment ) ) {
    foreach ( $cora_equipment as $key => $item ) {
        $assigned_shoot_name = '';
        $assigned_crew_name = '';
        $assigned_note = '';
        $is_assigned = false;

        // Check active clients (Shoot Bookings) first
        if ( is_array( $cora_clients ) ) {
            foreach ( $cora_clients as $client ) {
                $client_gear = isset( $client['equipment_ids'] ) ? (array) $client['equipment_ids'] : array();
                if ( in_array( $item['id'], $client_gear ) || in_array( (string)$item['id'], $client_gear ) ) {
                    $assigned_shoot_name = $client['names'];
                    $assigned_crew_name = !empty($client['crew']) ? $client['crew'] : '';
                    $assigned_note = 'Assigned in Shoot Bookings';
                    $is_assigned = true;
                    break;
                }
            }
        ...
        if ( ! $is_assigned && is_array( $cora_leads ) ) {
            foreach ( $cora_leads as $lead ) {
                $lead_gear = isset( $lead['equipment_ids'] ) ? (array) $lead['equipment_ids'] : array();
                if ( in_array( $item['id'], $lead_gear ) || in_array( (string)$item['id'], $lead_gear ) ) {
                    $assigned_shoot_name = $lead['names'];
        ...
        if ( $is_assigned ) {
            $cora_equipment[$key]['status'] = 'In Use';
            $cora_equipment[$key]['shoot'] = $assigned_shoot_name;
            $cora_equipment[$key]['crew'] = $assigned_crew_name;
            $cora_equipment[$key]['assignment_note'] = $assigned_note;
        }
    }
}
...
$dynamic_bookings_count = count( $cora_clients );
...
$cora_financials = get_option( 'cora_studio_financials', array() );
...
$cora_update_plugins_transient = get_site_transient( 'update_plugins' );
$cora_update_available = false;
$cora_new_version = '';
$cora_plugin_base = 'cora-studio-ai-locked/cora-studio-ai.php';
if ( isset( $cora_update_plugins_transient->response[ $cora_plugin_base ] ) ) {
    $cora_update_available = true;
    $cora_new_version = $cora_update_plugins_transient->response[ $cora_plugin_base ]->new_version;
}
...
<head>
    ...
    <title>Cora for Studio - Workspace</title>
    ...
    <script>
        window.coraClients = <?php echo json_encode( $cora_clients ); ?>;
        window.coraDocuments = <?php echo json_encode( $cora_documents ); ?>;
        window.coraGalleries = <?php echo json_encode( $cora_galleries ); ?>;
    </script>
    
    <!-- Load QuillJS Rich Text Editor -->
    ...
    <style id="cora-workspace-custom-styles">
        ...
        *:not(.cora-serif-editor):not(.cora-serif-editor *) {
        ...
        /* Beehiiv Style Editor Overrides */
        .ql-toolbar.ql-snow {
            border: none !important;
            border-bottom: 1px solid #f4f4f5 !important;
        ...
        .cora-dark-theme #cora-add-shoot-drawer,
        .cora-dark-theme #cora-ai-sidebar,
        .cora-dark-theme #cora-gallery-drawer {
            background-color: #121214 !important;
            border-left-color: rgba(255, 255, 255, 0.08) !important;
            color: #f4f4f5 !important;
        }
        ...
        /* Premium Dashboard UI styling (Bento Grid) */
        #cora-page-dashboard {
    ...
    <script>
        var coraData = {
            ...
            galleries: <?php echo json_encode( $cora_galleries ); ?>,
            leads: <?php echo json_encode( $cora_leads ); ?>,
            clients: <?php echo json_encode( $cora_clients ); ?>,
            financials: <?php echo json_encode( $cora_financials ); ?>,
            equipment: <?php echo json_encode( $cora_equipment ); ?>,
...
<div class="fixed inset-0 bg-white z-[99999] flex flex-col items-center justify-center p-6 font-sans">
    <div class="max-w-md w-full border border-zinc-200 rounded-2xl shadow-xl p-8 bg-white text-center flex flex-col items-center gap-6">
        ...
        <div>
            ...
            <p class="text-xs text-zinc-505 mt-2 leading-relaxed">We sent a verification link to your email. Please click the link to activate your Cora for Studio workspace.</p>
        </div>
...
<div id="cora-workspace" class="flex flex-col min-h-screen bg-[#f7f7f5] text-zinc-900">
    ...
    <header id="cora-global-topbar" class="cora-topbar sticky top-0 z-50 h-[52px] bg-zinc-950 dark:bg-black text-white flex items-center justify-between px-4 md:px-6 border-b border-zinc-800/80 shrink-0 select-none" style="background-color: #09090b !important;">
        ...
        <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="flex items-center gap-2 cursor-pointer shrink-0" onclick="coraNavigateTo('dashboard');">
        ...
        <!-- Right Section: Actions & User Pill -->
            ...
            <div class="relative shrink-0">
                ...
                <div id="cora-header-profile-popover" class="hidden absolute right-0 mt-2 w-64 bg-white dark:bg-zinc-955 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl p-4 z-[70] flex flex-col gap-2.5 animate-in fade-in slide-in-from-top-2 duration-150 select-none">
                    ...
                    <div class="flex flex-col gap-0.5">
                        <button class="w-full text-left px-2.5 py-2 text-xs text-zinc-700 dark:text-zinc-300 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-900 hover:text-zinc-955 font-medium flex items-center gap-3 cursor-pointer transition-colors" onclick="coraNavigateTo('my-profile'); $('#cora-header-profile-popover').addClass('hidden');">
                        ...
                        <button class="w-full text-left px-2.5 py-2 text-xs text-zinc-700 dark:text-zinc-300 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-900 hover:text-zinc-955 font-medium flex items-center justify-between cursor-pointer transition-colors" onclick="coraNavigateTo('settings'); $('#cora-header-profile-popover').addClass('hidden');">
                            <div class="flex items-center gap-3">
                                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-400 shrink-0"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0 .33-1.82l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06-.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>
                                Settings
                            </div>
    ...
    <aside class="cora-sidebar w-64 bg-[#f9fafb] dark:bg-[#0c0c0e] border-r border-zinc-200/80 dark:border-zinc-800/40 flex flex-col shrink-0 h-[calc(100vh-52px)] fixed lg:sticky top-[52px] left-0 z-50 lg:z-30 transition-all duration-200">
        ...
        <div class="flex-1 flex flex-col min-h-0 overflow-y-auto">
            ...
            <div class="flex items-center justify-between gap-2 px-3 pt-4 pb-2 shrink-0 select-none">
                ...
                <div class="relative flex-1 min-w-0">
                    ...
                    <div class="cora-workspace-card flex items-center justify-between gap-2 px-2.5 py-1.5 bg-zinc-50 hover:bg-zinc-100 dark:bg-zinc-900/50 dark:hover:bg-zinc-800 border border-zinc-200 dark:border-zinc-850 rounded-lg cursor-pointer transition-all select-none" onclick="event.stopPropagation(); $('#cora-workspace-popover').toggleClass('hidden'); $('#cora-profile-popover').addClass('hidden');">
                        <div class="flex items-center gap-2 min-w-0">
                            ...
                            <span class="cora-studio-info text-zinc-900 dark:text-zinc-100 font-bold text-xs truncate">Cora Studio</span>
                        </div>
                    ...
                    <div id="cora-workspace-popover" class="hidden absolute top-full mt-2 left-0 w-[275px] bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl p-3.5 z-50 flex flex-col select-none" style="animation: popoverSlideDown 0.12s ease-out;">
                        ...
                        <div class="flex items-center gap-3 pb-3 border-b border-zinc-100 dark:border-zinc-800">
                            ...
                            <div class="flex flex-col min-w-0 flex-1 leading-snug">
                                <span class="text-sm font-bold text-zinc-900 dark:text-zinc-100 truncate">Cora Studio</span>
                                <span class="text-xs font-medium text-zinc-400 dark:text-zinc-500 truncate">Delhi Studio v2</span>
                            </div>
                        </div>

                        <!-- Settings & Team Buttons -->
                        <div class="grid grid-cols-2 gap-2 my-3">
                            <button class="flex items-center justify-center gap-2 px-3 py-2 bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-850 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-semibold text-zinc-700 dark:text-zinc-200 cursor-pointer transition-colors shadow-2xs" onclick="coraNavigateTo('settings'); $('#cora-workspace-popover').addClass('hidden');">
                            ...
                            <button class="flex items-center justify-center gap-2 px-3 py-2 bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-850 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-semibold text-zinc-700 dark:text-zinc-200 cursor-pointer transition-colors shadow-2xs" onclick="coraNavigateTo('team'); $('#cora-workspace-popover').addClass('hidden');">
                        ...
                        <div class="flex flex-col gap-1.5 pt-1 border-t border-zinc-100 dark:border-zinc-800">
                            ...
                            <div class="flex items-center justify-between px-3 py-2 bg-zinc-50 dark:bg-zinc-900/80 border border-zinc-200/80 dark:border-zinc-800/80 rounded-xl hover:bg-zinc-100/80 dark:hover:bg-zinc-850 cursor-pointer transition-colors">
                                <div class="flex items-center gap-2.5 min-w-0">
                                    ...
                                    <span class="text-xs font-semibold text-zinc-900 dark:text-zinc-100 truncate">Cora Studio</span>
                                </div>
                ...
                <button id="cora-sidebar-toggle" class="text-zinc-500 hover:text-black dark:text-zinc-455 dark:hover:text-white bg-white dark:bg-zinc-955 border border-zinc-200 dark:border-zinc-800 p-2 rounded-lg transition-colors cursor-pointer select-none shadow-2xs" title="Collapse Sidebar">
            ...
            <nav class="cora-sidebar-nav px-0 pt-1.5 pb-4 space-y-4">
                ...
                <div>
                    ...
                    <ul class="cora-nav-list space-y-0.5 mt-1">
                        ...
                        <li class="cora-nav-item <?php echo $sub_page === 'settings' ? 'cora-active' : ''; ?> flex items-center justify-between px-3 py-2 text-sm rounded-lg cursor-pointer" data-target="settings">
                            <div class="flex items-center gap-3">
                                <span class="cora-nav-icon">
                                    <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round">
                ...
                <!-- Group 2: CRM & Booking -->
                <div>
                    <div class="cora-nav-group-label px-3 text-[11px] font-bold text-zinc-500 uppercase">CRM & Booking</div>
                    <ul class="cora-nav-list space-y-0.5 mt-1">
                ...
                <!-- Group 3: Operations -->
                <div>
                    <div class="cora-nav-group-label px-3 text-[11px] font-bold text-zinc-500 uppercase">Operations</div>
                    <ul class="cora-nav-list space-y-0.5 mt-1">
                ...
                <!-- Group 4: AI & Content -->
                <div>
                    <div class="cora-nav-group-label px-3 text-[11px] font-bold text-zinc-500 uppercase">AI & Content</div>
                    <ul class="cora-nav-list space-y-0.5 mt-1">
                        ...
                        <li class="cora-nav-item flex items-center justify-between px-3 py-2 text-sm rounded-lg cursor-pointer" data-target="ai-assistants">
                            <div class="flex items-center gap-3">
                                <span class="cora-nav-icon">
                                    <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path>
                            ...
                            <span class="cora-badge cora-badge-soon cora-badge-sidebar px-1.5 py-0.5 text-[9px] font-bold rounded-md select-none bg-zinc-200 text-zinc-800">Soon</span>
                        </li>
                        <li class="cora-nav-item <?php echo $sub_page === 'plugins' ? 'cora-active' : ''; ?> flex items-center justify-between px-3 py-2 text-sm rounded-lg cursor-pointer" data-target="plugins">
                            <div class="flex items-center gap-3">
                                <span class="cora-nav-icon">
                                    <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round">
                ...
                <!-- Group 5: Quick Links -->
                <div>
                    <div class="cora-nav-group-label px-3 text-[11px] font-bold text-zinc-500 uppercase">Quick Links</div>
                    <ul class="cora-nav-list space-y-0.5 mt-1">
                        <li>
                            <a href="<?php echo esc_url( home_url( '/' ) ); ?>" target="_blank" class="cora-nav-item-link flex items-center gap-3 px-3 py-2 text-sm rounded-lg cursor-pointer">
                                <span class="cora-nav-icon">
                                    <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round">
        ...
        <div class="cora-sidebar-bottom-block shrink-0 border-t border-zinc-200/50 dark:border-zinc-800/40 z-20 sticky bottom-0 flex flex-col bg-[#f9fafb] dark:bg-[#0c0c0e]">
            ...
            <div id="cora-profile-popover" class="hidden absolute bottom-20 left-4 right-4 bg-white dark:bg-zinc-955 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-xl p-4 z-[70] flex flex-col gap-2.5 animate-in fade-in slide-in-from-bottom-2 duration-150 select-none">
                ...
                <!-- Menu Items List -->
                <div class="flex flex-col gap-0.5">
                    <button class="w-full text-left px-2.5 py-2 text-xs text-zinc-700 dark:text-zinc-300 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-900 hover:text-zinc-950 dark:hover:text-white font-medium flex items-center gap-3 cursor-pointer transition-colors" onclick="coraNavigateTo('my-profile'); $('#cora-profile-popover').addClass('hidden');">
                        <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-400 shrink-0"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"></path><circle cx="12" cy="7" r="4"></circle></svg>
                        Profile
                    </button>

                    <button class="w-full text-left px-2.5 py-2 text-xs text-zinc-700 dark:text-zinc-300 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-900 hover:text-zinc-955 font-medium flex items-center justify-between cursor-pointer transition-colors" onclick="coraNavigateTo('settings'); $('#cora-profile-popover').addClass('hidden');">
                        <div class="flex items-center gap-3">
                            <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-400 shrink-0"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0 .33-1.82l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06-.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>
                            Settings
                        </div>
    ...
    <main class="cora-main flex-1 bg-white flex flex-col min-h-screen relative pb-16 lg:pb-0 min-w-0 w-full overflow-x-clip">
        ...
        <div class="cora-content-wrapper p-3.5 sm:p-6 md:p-8 max-w-full w-full flex-1 space-y-6 min-w-0 overflow-x-hidden">
            ...
            <section id="cora-page-dashboard" class="cora-page-section cora-active space-y-6">
                ...
                <div class="text-center my-14 px-4 space-y-3 relative">
                    <div class="inline-flex items-center justify-center gap-3">
                        <!-- Slate Charcoal Star Sparkle -->
                        <span class="text-zinc-455 dark:text-zinc-600 shrink-0">
                ...
                <div class="w-full max-w-2xl mx-4 md:mx-auto mb-16 relative z-[999]" id="cora-search-container">
                    <div class="relative flex items-center bg-white/85 dark:bg-zinc-900/70 backdrop-blur-md border border-zinc-200/60 dark:border-zinc-800/50 hover:border-zinc-350 dark:hover:border-zinc-700 focus-within:border-zinc-900 dark:focus-within:border-zinc-100 focus-within:ring-2 focus-within:ring-zinc-100/30 dark:focus-within:ring-zinc-800/30 rounded-full shadow-2xs transition-all duration-200 p-2 pl-4 pr-3">
                        <span class="text-purple-600 dark:text-purple-400 mr-2.5 flex shrink-0">
                            ...
                            <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                                ...
                                <path d="M9 14.5 Q12 16.5 15 14.5" stroke="#fff" stroke-width="1.2" fill="none" stroke-linecap="round"></path>
                            </svg>
                        </span>
                        <input type="text" id="cora-command-input" placeholder="Ask anything..." class="w-full bg-transparent border-0 outline-none text-sm text-zinc-800 dark:text-zinc-150 placeholder-zinc-400 dark:placeholder-zinc-550 focus:ring-0 p-0" autocomplete="off" />
                        <div class="flex items-center gap-1.5 ml-2 shrink-0">
                            <span class="text-[10px] font-bold text-zinc-400 dark:text-zinc-555 bg-zinc-100 dark:bg-zinc-850 px-1.5 py-0.5 rounded border border-zinc-200/40 dark:border-zinc-800 select-none">⌘K</span>
                            <button id="cora-command-search-btn" class="h-7 w-7 rounded-full bg-zinc-900 dark:bg-zinc-100 hover:bg-zinc-800 dark:hover:bg-zinc-200 text-white dark:text-zinc-900 flex items-center justify-center transition-all shadow-3xs cursor-pointer">
                                <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.5" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                    <line x1="22" y1="22" x2="15" y2="15"></line>
                    ...
                    <div id="cora-command-results" class="absolute top-full left-0 right-0 mt-2 bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md border border-zinc-200/80 dark:border-zinc-800/80 rounded-2xl shadow-xl z-[9999] overflow-hidden max-h-[300px] overflow-y-auto hidden divide-y divide-zinc-100 dark:divide-zinc-800/50">
                        <!-- Filled dynamically -->
                    </div>
                </div>

                <!-- Lovable-Style Quick Action Shortcuts -->
                <div class="flex flex-col items-center gap-3.5 mb-16 select-none">
                    <div class="flex flex-wrap justify-center gap-2.5 max-w-xl px-4">
                        <span class="text-xs font-semibold text-zinc-400 dark:text-zinc-555 self-center mr-1">Quick actions:</span>
                        
                        <button onclick="coraNavigateTo('bookings')" class="inline-flex items-center gap-1.5 px-4 py-2 border border-zinc-200/80 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/50 hover:bg-zinc-50 dark:hover:bg-zinc-850 rounded-full text-xs font-semibold text-zinc-655 hover:text-zinc-900 dark:text-zinc-350 dark:hover:text-white transition-all shadow-3xs cursor-pointer">
                        ...
                        <button onclick="coraNavigateTo('gallery')" class="inline-flex items-center gap-1.5 px-4 py-2 border border-zinc-200/80 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/50 hover:bg-zinc-50 dark:hover:bg-zinc-850 rounded-full text-xs font-semibold text-zinc-655 hover:text-zinc-900 dark:text-zinc-350 dark:hover:text-white transition-all shadow-3xs cursor-pointer">
                            <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="1.8" fill="none" class="text-zinc-400">
                ...
                <div class="cora-sketch-grid pt-2 grid grid-cols-1 md:grid-cols-2 gap-6">
                    
                    <!-- BENTO CARD 1: Shoot Bookings (Spans 2 Columns) -->
                    <div class="border border-zinc-200/50 dark:border-zinc-800/55 rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-2xs md:col-span-2 min-h-[268px]">
                        <div class="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-zinc-800/40 mb-4">
                            <div class="flex flex-col gap-0.5">
                                <h3 class="text-xs font-bold text-zinc-800 dark:text-zinc-150 uppercase tracking-wider">Shoot Bookings</h3>
                                <span class="text-[9px] text-zinc-400 dark:text-zinc-500 font-medium">Your nearest photography bookings and client schedules.</span>
                            </div>
                            <button onclick="coraNavigateTo('bookings')" class="text-xs font-semibold text-zinc-500 hover:text-zinc-900 dark:text-zinc-455 dark:hover:text-white transition-colors flex items-center gap-1 cursor-pointer">
                        ...
                        <div>
                            <?php
                            $recent_active_shoots = array();
                            $reversed_clients = array_reverse( $cora_clients );
                            foreach ( $reversed_clients as $client ) {
                                if ( isset( $client['status'] ) && $client['status'] !== 'completed' ) {
                                    $recent_active_shoots[] = $client;
                                    if ( count( $recent_active_shoots ) >= 3 ) break;
                                }
                            }
                            
                            if ( empty( $recent_active_shoots ) ) :
                            ?>
                                <div class="flex flex-col items-center justify-center py-10 text-center">
                                    <div class="h-14 w-14 rounded-full bg-zinc-50 dark:bg-zinc-855 flex items-center justify-center mb-4 border border-zinc-100 dark:border-zinc-800">
                                    ...
                                    <h4 class="text-xs font-semibold text-zinc-700 dark:text-zinc-300">No shoots scheduled</h4>
                                    <p class="text-[10px] text-zinc-450 dark:text-zinc-555 mt-1 max-w-[220px]">Upcoming bookings will automatically appear in this view.</p>
                                </div>
                            <?php else : ?>
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                                    <?php foreach ( $recent_active_shoots as $shoot ) : 
                                        $badge_class = 'cora-badge-blue';
                                        $status_label = 'Confirmed';
                                        if ( isset($shoot['status']) ) {
                                        ...
                                        $short_date = explode( ',', $shoot['shoot_date'] ?? '' )[0];
                                    ?>
                                    <div class="flex flex-col justify-between p-3.5 bg-white/60 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/40 rounded-xl hover:border-zinc-300 dark:hover:border-zinc-700 transition-all cursor-pointer shadow-3xs min-h-[120px]" onclick="coraOpenClientLifecycle('<?php echo esc_attr($shoot['id']); ?>')">
                                        <div class="flex flex-col gap-1 min-w-0">
                                            ...
                                            <strong class="font-bold text-zinc-800 dark:text-zinc-150 text-xs block truncate mt-1.5"><?php echo esc_html($shoot['names']); ?></strong>
                                        </div>
                                        <div class="text-[9px] text-zinc-400 dark:text-zinc-500 border-t border-zinc-100 dark:border-zinc-800/40 pt-2 mt-2 truncate">
                                            <?php echo esc_html($shoot['shoot_type'] ?? 'Portrait Series'); ?> &bull; <?php echo esc_html($shoot['city'] ?? 'Delhi'); ?>
                                        </div>
                                    </div>
                                    <?php endforeach; ?>
                                </div>
                    ...
                    <div class="border border-zinc-200/50 dark:border-zinc-800/55 rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-2xs md:row-span-2 flex flex-col justify-between min-h-[564px]">
                        <div class="flex flex-col flex-1">
                            <div class="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-zinc-800/40 mb-4">
                                <h3 class="text-xs font-bold text-zinc-455 dark:text-zinc-555 uppercase tracking-widest">Active Assistants</h3>
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between p-3 bg-white/60 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/40 rounded-xl hover:border-zinc-300 dark:hover:border-zinc-700 cursor-pointer transition-all group shadow-3xs" onclick="coraNavigateTo('ai-assistants')">
                                    <div class="flex items-center gap-2.5 min-w-0">
                                        <div class="h-8 w-8 rounded-full bg-pink-50 dark:bg-pink-955/20 border border-pink-100 dark:border-pink-900/40 flex items-center justify-center shrink-0">
                                        ...
                                        <div class="flex flex-col min-w-0">
                                            ...
                                            <span class="text-[8px] text-zinc-400 dark:text-zinc-500 mt-1 block truncate">Draft aesthetic captions.</span>
                                        </div>
                                    </div>
                                ...
                                <div class="flex items-center justify-between p-3 bg-white/60 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/40 rounded-xl hover:border-zinc-300 dark:hover:border-zinc-700 cursor-pointer transition-all group shadow-3xs" onclick="coraNavigateTo('gallery')">
                                    <div class="flex items-center gap-2.5 min-w-0">
                    ...
                    <div class="border border-zinc-200/50 dark:border-zinc-800/55 rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-2xs min-h-[258px]">
                        ...
                        <div class="space-y-2">
                            <?php
                            $recent_leads = is_array($cora_leads) ? array_slice(array_reverse($cora_leads), 0, 3) : array();
                                ...
                                <?php foreach ( $recent_leads as $lead ) : ?>
                                    <div class="flex items-center justify-between p-2.5 bg-white/60 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/40 rounded-xl hover:border-zinc-300 dark:hover:border-zinc-700 transition-all cursor-pointer shadow-3xs gap-3" onclick="coraNavigateTo('leads')">
                                        <div class="flex-1 min-w-0 flex flex-col gap-0.5">
                                            ...
                                            <span class="text-[9px] text-zinc-400 dark:text-zinc-555 block truncate"><?php echo esc_html($lead['shoot_type'] ?? 'Photography Shoot'); ?> &bull; Budget: <?php echo esc_html(cora_format_rupees($lead['budget'] ?? 0)); ?></span>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                    ...
                    <!-- BENTO CARD 4: Recent Galleries (Spans 1 Column) -->
                    <div class="border border-zinc-200/50 dark:border-zinc-800/55 rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-2xs min-h-[258px]">
                        <div class="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-zinc-800/40 mb-4">
                            <h3 class="text-xs font-bold text-zinc-400 dark:text-zinc-555 uppercase tracking-widest">Recent Galleries</h3>
                            <button onclick="coraNavigateTo('gallery')" class="text-xs font-semibold text-zinc-500 hover:text-zinc-900 dark:text-zinc-455 dark:hover:text-white transition-colors flex items-center gap-1 cursor-pointer">
                                Galleries &rarr;
                            </button>
                        </div>
                        ...
                        <div class="space-y-2">
                            <?php
                            $recent_galleries = is_array($cora_galleries) ? array_slice(array_reverse($cora_galleries), 0, 3) : array();
                            if ( empty($recent_galleries) ) :
                            ?>
                                <div class="flex flex-col items-center justify-center py-8 text-center">
                                    <h4 class="text-xs font-semibold text-zinc-700 dark:text-zinc-400">No galleries available.</h4>
                                    <p class="text-[9px] text-zinc-400 dark:text-zinc-500 mt-1">Click above to add gallery.</p>
                                </div>
                            <?php else : ?>
                                <?php foreach ( $recent_galleries as $gallery ) : 
                                    $assets = isset( $gallery['assets'] ) ? $gallery['assets'] : array();
                                    $is_secured = ! empty( $gallery['password'] );
                                ?>
                                    <div class="flex items-center justify-between p-2.5 bg-white/60 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/40 rounded-xl hover:border-zinc-300 dark:hover:border-zinc-700 transition-all cursor-pointer shadow-3xs gap-3" onclick="coraNavigateTo('gallery')">
                                        <div class="flex-1 min-w-0 flex flex-col gap-0.5">
                                            <strong class="font-semibold text-zinc-800 dark:text-zinc-150 text-xs block truncate"><?php echo esc_html($gallery['title'] ?? 'Untitled Gallery'); ?></strong>
                                            <span class="text-[9px] text-zinc-400 dark:text-zinc-555 block truncate"><?php echo count($assets); ?> assets &bull; <?php echo $is_secured ? 'Password Protected' : 'Public Link'; ?></span>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                    ...
                    <!-- BENTO CARD 5: Studio Vault (Spans 1 Column) -->
                    <div class="border border-zinc-200/50 dark:border-zinc-800/55 rounded-2xl bg-white dark:bg-zinc-900 p-6 shadow-2xs min-h-[258px]">
                        <div class="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-zinc-800/40 mb-4">
                            <h3 class="text-xs font-bold text-zinc-400 dark:text-zinc-555 uppercase tracking-widest">Studio Vault</h3>
                        ...
                        <div class="space-y-2">
                            ...
                            ?>
                                <div class="flex flex-col items-center justify-center py-8 text-center">
                                    <span class="text-[10px] text-zinc-400 dark:text-zinc-555">No documents in vault.</span>
                                </div>
                            <?php else : ?>
                                <?php foreach ( $recent_docs as $doc ) : ?>
                                    <div class="flex items-center justify-between p-2.5 bg-white/60 dark:bg-zinc-900/30 border border-zinc-100 dark:border-zinc-800/40 rounded-xl hover:border-zinc-300 dark:hover:border-zinc-700 transition-all cursor-pointer shadow-3xs gap-3" onclick="coraNavigateTo('vault')">
                                        <div class="flex-1 min-w-0 flex flex-col">
                                            ...
                                            <span class="text-[8px] text-zinc-400 dark:text-zinc-555 block truncate"><?php echo esc_html($doc['category'] ?? 'General'); ?></span>
                                        </div>
                                    </div>
                                <?php endforeach; ?>
                            <?php endif; ?>
                            ...
                            <div class="md:col-span-2 space-y-4">
                                <div class="flex items-center justify-between pb-3 border-b border-zinc-100 dark:border-zinc-800/40">
                                    <div class="flex flex-col gap-0.5">
                                        ...
                                        <span class="text-[9px] text-zinc-400 dark:text-zinc-550 font-medium">Activate automated background pipelines.</span>
                                    </div>
                                </div>
                                ...
                                <div class="space-y-3">
                                    ...
                                    <div class="flex items-center justify-between p-3 bg-zinc-50/40 dark:bg-zinc-855/20 border border-zinc-200/40 dark:border-zinc-800/40 rounded-xl">
                                        <div class="flex flex-col gap-0.5 min-w-0">
                                            <span class="font-semibold text-xs text-zinc-800 dark:text-zinc-150 truncate">SEO Crawler</span>
                                            <span class="text-[9px] text-zinc-400 dark:text-zinc-500 truncate">Automatic search indexing & galleries rank</span>
                                        </div>
                                        <div class="flex items-center gap-3 shrink-0">
                                    ...
                                    <div class="flex items-center justify-between p-3 bg-zinc-50/40 dark:bg-zinc-855/20 border border-zinc-200/40 dark:border-zinc-800/40 rounded-xl">
                                        <div class="flex flex-col gap-0.5 min-w-0">
                                            ...
                                            <span class="text-[9px] text-zinc-400 dark:text-zinc-500 truncate">Automated shoot signing pipelines</span>
                                        </div>
                                        <div class="flex items-center gap-3 shrink-0">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div class="space-y-1.5 flex-1">
                            ...
                            <p class="text-sm text-zinc-700 dark:text-zinc-350 leading-relaxed font-medium">
                                You have <strong>3 upcoming shoots</strong> scheduled this week. We recommend generating caption packages and media keywords for the <em>Jaipur Destination Wedding</em> shoot completed yesterday.
                            </p>
                        </div>
                        <button onclick="coraNavigateTo('ai-assistants')" class="hidden md:inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-855/80 text-zinc-700 dark:text-zinc-300 active:scale-[0.98] transition-all cursor-pointer shadow-2xs">
                            ...
                            Generate Caption Packages
                        </button>
                    </div>
                    <div class="md:hidden">
                        <button onclick="coraNavigateTo('ai-assistants')" class="w-full justify-center inline-flex items-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-md border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:bg-zinc-50 dark:hover:bg-zinc-855/80 text-zinc-700 dark:text-zinc-300 active:scale-[0.98] transition-all cursor-pointer shadow-2xs">
                            <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-400">
                                <rect x="2" y="2" width="20" height="20" rx="5" ry="5"></rect>
                                <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
                            </svg>
            ...
            <section id="cora-page-bookings" class="cora-page-section cora-active space-y-6">
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-zinc-200 pb-3">
                    ...
                    <div class="cora-filters-bar flex items-center gap-1.5 overflow-x-auto whitespace-nowrap scrollbar-none max-w-full">
                        <button class="cora-filter-tab active px-3 py-1.5 text-xs font-semibold rounded-md border border-transparent text-zinc-650 hover:text-zinc-900 hover:bg-zinc-100 transition-all cursor-pointer inline-block shrink-0" data-filter="all">All Shoots</button>
                    ...
                    <div class="flex items-center gap-2 w-full md:w-auto">
                        <div class="relative flex-1 md:w-64">
                            ...
                            <input type="text" id="cora-crm-search-input" class="w-full pl-8 pr-2.5 py-1.5 text-xs border border-zinc-200 rounded-md bg-white text-zinc-900 placeholder:text-zinc-400 focus:border-zinc-400 focus:outline-none transition-colors" placeholder="Search shoots or clients...">
                        </div>
                ...
                <div id="cora-bookings-table-container" class="cora-table-wrapper md:border md:border-zinc-200/80 md:rounded-xl md:shadow-sm md:bg-white md:overflow-hidden">
                        ...
                        <table class="cora-table min-w-full divide-y divide-zinc-200" id="cora-bookings-table">
                            ...
                            <tbody class="divide-y divide-zinc-200/80 bg-white">
                                ...
                                $type_badge_classes = array(
                                    'Maternity Portrait' => 'cora-badge-purple',
                                    'Destination Wedding' => 'cora-badge-orange',
                                    'Product Shoot' => 'cora-badge-teal',
                                );
                                
                                if ( empty( $reversed_clients ) ) :
                                ?>
                                    <tr>
                                        <td colspan="7" class="px-4 py-8 text-center text-zinc-400">No bookings found. Click "+ New Shoot" to add one.</td>
                                    </tr>
                                <?php else : ?>
                                    <?php foreach ( $reversed_clients as $client ) : 
                                        $status = $client['status'] ?? 'confirmed';
                                        $type = $client['shoot_type'] ?? 'Maternity Portrait';
                                    ...
                                    <tr data-id="<?php echo esc_attr( $client['id'] ); ?>" data-status="<?php echo esc_attr( $status ); ?>" class="hover:bg-zinc-50/30 transition-colors cursor-pointer" onclick="coraOpenClientLifecycle('<?php echo esc_attr( $client['id'] ); ?>')">
                                        ...
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-zinc-500" data-label="Location"><?php echo esc_html( $client['city'] ?? 'Delhi Studio' ); ?></td>
                                        <td class="px-4 py-3 whitespace-nowrap text-sm text-zinc-500" data-label="Date"><?php echo esc_html( $client['shoot_date'] ?? 'Pending Date' ); ?></td>
                    ...
                    <div class="md:hidden space-y-4 pt-4 pb-2" id="cora-bookings-cards-list">
                        <?php if ( empty( $reversed_clients ) ) : ?>
                            <div class="py-8 text-center text-zinc-400 text-xs">No bookings found. Click "+ New Shoot" to add one.</div>
                        <?php else : ?>
                            <?php foreach ( $reversed_clients as $client ) : 
                                $status = $client['status'] ?? 'confirmed';
                                $type = $client['shoot_type'] ?? 'Maternity Portrait';
                            ...
                            <div class="cora-booking-card-item bg-white border border-zinc-200 rounded-xl p-4 shadow-sm hover:shadow hover:border-zinc-300 transition-all cursor-pointer flex flex-col gap-3" 
                                ...
                                <div class="grid grid-cols-2 gap-3 text-xs">
                                    <div class="flex flex-col gap-0.5 min-w-0">
                                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Shoot Date</span>
                                        <span class="font-medium text-zinc-700 truncate"><?php echo esc_html( $client['shoot_date'] ?? 'Pending Date' ); ?></span>
                                    </div>
                                    <div class="flex flex-col gap-0.5 min-w-0">
                                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Location</span>
                                        <span class="font-medium text-zinc-700 truncate" title="<?php echo esc_attr( $client['city'] ?? 'Delhi Studio' ); ?>"><?php echo esc_html( $client['city'] ?? 'Delhi Studio' ); ?></span>
                                    </div>
                                    <div class="flex flex-col gap-0.5 min-w-0">
                    ...
                    <div class="hidden md:block overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="divide-y divide-zinc-100 text-xs text-zinc-650" id="cora-clients-table-body">
                                ...
                                <?php else : ?>
                                    <?php foreach ( $cora_clients as $client ) : ?>
                                    <tr data-id="<?php echo esc_attr( $client['id'] ); ?>" class="hover:bg-zinc-50/50 transition-colors">
                                        ...
                                        <td class="py-3 px-4" data-label="Celebration">
                                            <div class="font-medium text-zinc-700"><?php echo esc_html( isset($client['scale']) ? $client['scale'] : 'Multi-Day Wedding' ); ?></div>
                                        ...
                                        <td class="py-3 px-4 text-right" data-label="Actions">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <button class="cora-btn-action px-2.5 py-1 bg-zinc-950 hover:bg-zinc-800 text-white rounded-md text-[10px] font-semibold cursor-pointer" 
                                                        onclick="coraPrefillAddShoot(<?php echo esc_attr( json_encode( $client ) ); ?>)">
                                                    Add Shoot
                                                </button>
                            ...
                            <div class="cora-client-card-item bg-white border border-zinc-200 rounded-xl p-4 shadow-sm hover:shadow hover:border-zinc-300 transition-all flex flex-col gap-3">
                                ...
                                <div class="grid grid-cols-2 gap-3 text-xs">
                                    <div class="flex flex-col gap-0.5 min-w-0">
                                        ...
                                        <span class="font-medium text-zinc-700 truncate"><?php echo esc_html( isset($client['scale']) ? $client['scale'] : 'Multi-Day Wedding' ); ?></span>
                                    </div>
                                    <div class="flex flex-col gap-0.5 min-w-0">
                                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Location</span>
            ...
            <?php if ( $sub_page === 'gallery' ) : ?>
            <?php $cora_galleries = get_option( 'cora_studio_galleries', array() ); ?>
            <section id="cora-page-gallery" class="cora-page-section cora-active flex flex-col h-full overflow-hidden bg-white -m-4 p-0 md:-m-8 md:p-0">
                <!-- TOP NAVIGATION BAR -->
                <div class="border-b border-zinc-200 bg-zinc-50 px-4 sm:px-6 md:px-8 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
                    <div class="flex items-center gap-5 justify-center sm:justify-start">
                        <button class="text-xs sm:text-sm font-bold text-zinc-900 border-b-2 border-zinc-900 pb-1 cora-gallery-tab-btn" data-tab="vault-all" onclick="coraSwitchGalleryTab('vault-all', this)">Master Media Vault</button>
                        <button class="text-xs sm:text-sm font-semibold text-zinc-500 hover:text-zinc-900 border-b-2 border-transparent pb-1 cora-gallery-tab-btn" data-tab="client-galleries" onclick="coraSwitchGalleryTab('client-galleries', this)">Shared Galleries</button>
                    </div>
                ...
                <div class="flex-1 flex flex-col h-full overflow-y-auto relative bg-white">
                    ...
                    <div id="cora-vault-grid-view" class="p-6 md:p-8 space-y-6 transition-all duration-300 ease-in-out">
                        <div class="cora-page-header flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            <div>
                                ...
                                <p class="cora-section-desc text-xs text-zinc-500 mt-1">Select assets to organize or create client galleries.</p>
                            </div>
                            <div class="flex items-center gap-2">
                                <button id="cora-btn-create-gallery" class="hidden cora-btn-primary px-3 py-1.5 bg-zinc-950 text-white font-semibold rounded text-[11px] hover:bg-zinc-800 transition-all active:scale-[0.98] cursor-pointer items-center gap-1.5 shadow-sm" onclick="coraCreateClientGalleryFromSelection()">
                    ...
                    <!-- LIST VIEW (Client Galleries) -->
                    <div id="cora-gallery-list-view" class="hidden p-3.5 sm:p-6 md:p-8 space-y-4 sm:space-y-6 transition-all duration-300 ease-in-out">
                        <div class="cora-page-header flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4 border-b border-zinc-100 pb-3 sm:pb-4">
                            <div class="flex items-center gap-2.5 sm:gap-3">
                                ...
                                <div>
                                    <h1 class="cora-page-title text-lg sm:text-2xl font-bold tracking-tight text-zinc-900">Shared Client Galleries</h1>
                                    <p class="cora-section-desc text-[10px] sm:text-xs text-zinc-500 mt-0.5 sm:mt-1">Create password-protected folders and sync client photo selections for albums.</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                    ...
                    <div class="md:border md:border-zinc-200/80 md:rounded-xl md:overflow-hidden md:shadow-sm md:bg-white">
                        ...
                        <div class="hidden md:block overflow-x-auto">
                            <table class="min-w-full divide-y divide-zinc-200 text-left">
                                <thead class="bg-zinc-50">
                                    <tr>
                                        <th class="px-6 py-3.5 text-xs font-bold text-zinc-400 uppercase tracking-wider">Gallery Name</th>
                                ...
                                <tbody class="divide-y divide-zinc-200 bg-white" id="cora-galleries-table-body">
                                    <?php if ( empty( $cora_galleries ) ) : ?>
                                    <tr>
                                        <td colspan="7" class="px-6 py-12 text-center">
                                            <div class="flex flex-col items-center justify-center space-y-3">
                                                ...
                                                <h3 class="text-sm font-semibold text-zinc-900">No Galleries Found</h3>
                                                <p class="text-xs text-zinc-500 max-w-sm">Create your first client gallery folder to start sharing photos and collecting client selections.</p>
                                            </div>
                                        </td>
                                    </tr>
                                    <?php else : ?>
                                        <?php foreach ( $cora_galleries as $gallery ) : 
                                            $share_url = home_url( '/shared-gallery/' . $gallery['hash'] );
                                            $assets = isset( $gallery['assets'] ) ? $gallery['assets'] : array();
                                            $likes = isset( $gallery['likes'] ) ? $gallery['likes'] : array();
                                            $template_label = 'Grid';
                                            if ( isset( $gallery['template'] ) && $gallery['template'] === 'masonry' ) {
                                                $template_label = 'Masonry';
                                            } elseif ( isset( $gallery['template'] ) && $gallery['template'] === 'carousel' ) {
                                                $template_label = 'Carousel';
                                            }
                                            $is_secured = ! empty( $gallery['password'] );
                                        ?>
                                        <tr class="hover:bg-zinc-50/40 transition-colors cursor-pointer" onclick="if(event.target.tagName !== 'BUTTON' && event.target.tagName !== 'INPUT' && !event.target.closest('button')) coraShowGalleryDetails('<?php echo esc_js( $gallery['id'] ); ?>')" data-gallery-id="<?php echo esc_attr( $gallery['id'] ); ?>">
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                ...
                                                <div class="text-[10px] text-zinc-400 font-mono mt-0.5 ml-6"><?php echo esc_html( $gallery['id'] ); ?></div>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-xs font-semibold">
                                                <div class="flex items-center justify-end gap-1.5">
                                                    <button class="p-1.5 border border-zinc-200 rounded hover:bg-zinc-100 text-zinc-700 hover:text-black transition-colors cursor-pointer" onclick="event.stopPropagation(); coraShowGalleryDetails('<?php echo esc_js( $gallery['id'] ); ?>')" title="Manage Assets">
                                                        <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                            <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                                                    ...
                                                    <button class="p-1.5 border border-zinc-200 rounded hover:bg-zinc-100 text-zinc-700 hover:text-black transition-colors cursor-pointer" onclick="event.stopPropagation(); coraOpenShareGalleryModal('<?php echo esc_js( $gallery['id'] ); ?>')" title="Gallery Settings &amp; Share">
                                                        <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                            <circle cx="12" cy="12" r="3"></circle>
                                                    ...
                                                    <button class="p-1.5 border border-zinc-200 rounded hover:bg-zinc-100 text-red-500 hover:text-red-700 hover:bg-red-50/50 transition-colors cursor-pointer" onclick="event.stopPropagation(); coraDeleteGallery('<?php echo esc_js( $gallery['id'] ); ?>')" title="Delete Gallery">
                                                        <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                        ...
                        <!-- Mobile/Tablet Card View (Visible below 768px) -->
                        <div class="md:hidden space-y-3" id="cora-galleries-cards-list">
                            <?php if ( empty( $cora_galleries ) ) : ?>
                                <div class="py-10 text-center flex flex-col items-center space-y-3">
                                    ...
                                    <h3 class="text-sm font-semibold text-zinc-900">No Galleries Found</h3>
                                    <p class="text-[11px] text-zinc-500 max-w-xs">Create your first client gallery to start sharing photos and collecting client selections.</p>
                                </div>
                            <?php else : ?>
                                <?php foreach ( $cora_galleries as $gallery ) :
                                    $share_url = home_url( '/shared-gallery/' . $gallery['hash'] );
                                    ...
                                    $is_secured = ! empty( $gallery['password'] );
                                ?>
                                <div class="cora-gallery-card-item bg-white border border-zinc-200 rounded-xl p-4 shadow-sm hover:shadow hover:border-zinc-300 transition-all cursor-pointer flex flex-col gap-3"
                                     data-gallery-id="<?php echo esc_attr( $gallery['id'] ); ?>"
                                     onclick="if(event.target.tagName !== 'BUTTON' && !event.target.closest('button')) coraShowGalleryDetails('<?php echo esc_js( $gallery['id'] ); ?>')">

                                    <!-- Card Header: Gallery Name & Template badge -->
                                    <div class="flex items-start justify-between gap-2 border-b border-zinc-100 pb-2.5">
                                        <div class="flex items-center gap-2 min-w-0">
                                            ...
                                            <div class="flex flex-col min-w-0">
                                                <span class="font-bold text-sm text-zinc-900 truncate"><?php echo esc_html( $gallery['title'] ); ?></span>
                                                <span class="text-[9px] text-zinc-400 font-mono mt-0.5 truncate"><?php echo esc_html( $gallery['id'] ); ?></span>
                                            </div>
                                        </div>
                                    ...
                                    <div class="flex items-center justify-between border-t border-zinc-100 pt-2.5 mt-0.5">
                                        ...
                                        <div class="flex items-center gap-1.5">
                                            ...
                                            <button class="p-1.5 border border-zinc-200 rounded hover:bg-zinc-100 text-zinc-700 hover:text-black transition-colors cursor-pointer" onclick="event.stopPropagation(); coraOpenShareGalleryModal('<?php echo esc_js( $gallery['id'] ); ?>')" title="Gallery Settings">
                                                <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                                    <circle cx="12" cy="12" r="3"></circle>
                                                    <path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33H15a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 16 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H15a2 2 0 0 1 0-4h.09a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"></path>
                                                </svg>
                ...
                <!-- DETAILS GRID VIEW -->
                <div id="cora-gallery-details-view" class="hidden flex-col h-full p-6 md:p-8 space-y-6 transition-all duration-300 ease-in-out">
                    <!-- Details Header -->
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-zinc-200/50 pb-4">
                        <div class="flex items-center gap-4">
                            <button class="p-1.5 rounded-md border border-zinc-200 text-zinc-500 hover:text-zinc-900 hover:bg-zinc-50 transition-colors cursor-pointer flex items-center justify-center bg-white shadow-sm" onclick="coraShowGalleryListView()" title="Back to Galleries">
                            ...
                            <div class="flex flex-col">
                                <div class="flex items-center gap-2 group">
                                    <h2 id="cora-detail-gallery-title-text" class="text-xl font-bold tracking-tight text-zinc-900 cursor-text" onclick="coraEditActiveGalleryTitle()">Gallery Title</h2>
                                    <input type="text" id="cora-detail-gallery-title-input" class="hidden text-xl font-bold tracking-tight text-zinc-900 border-b border-zinc-300 bg-transparent focus:outline-none focus:border-zinc-900 px-0 py-0 w-64" onblur="coraSaveActiveGalleryTitle()" onkeydown="if(event.key === 'Enter') coraSaveActiveGalleryTitle()">
                                ...
                                <div id="cora-detail-gallery-stats" class="text-xs text-zinc-500 font-medium flex items-center gap-2 mt-0.5">
                    ...
                    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-zinc-50/50 p-3 rounded-lg border border-zinc-200/60">
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            <div class="relative w-full sm:w-64">
                                ...
                                <input type="text" id="cora-detail-gallery-search" placeholder="Search assets..." class="w-full pl-8 pr-3 py-1.5 text-xs bg-white border border-zinc-200 rounded-md focus:outline-none focus:border-zinc-400 text-zinc-800 font-medium placeholder-zinc-400" onkeyup="coraRenderActiveGalleryAssets()">
                            </div>
                        </div>
                        <div class="flex items-center gap-3 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
                            <div class="flex items-center bg-zinc-200/50 p-0.5 rounded-md text-[10px] font-bold text-zinc-500 uppercase tracking-wider shrink-0" id="cora-detail-gallery-filters">
                            ...
                            <select id="cora-detail-gallery-sort" class="text-xs bg-white border border-zinc-200 rounded-md px-2 py-1.5 focus:outline-none focus:border-zinc-400 text-zinc-700 font-semibold cursor-pointer shrink-0" onchange="coraRenderActiveGalleryAssets()">
                    ...
                    <!-- Grid -->
                    <div id="cora-detail-gallery-grid" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 pb-8">
            ...
            <section id="cora-page-settings" class="cora-page-section cora-active space-y-5">
                ...
                <div class="cora-page-header flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="cora-section-desc text-[11px] sm:text-xs text-zinc-500 mt-0.5">Configure your studio profile, integrations, and automation preferences.</p>
                    </div>
                </div>
                ...
                <div class="bg-white border border-zinc-200/80 rounded-xl shadow-sm overflow-hidden">
                    <div class="px-5 pt-5 pb-4 border-b border-zinc-100">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-zinc-900">Your Studio</h3>
                                <p class="text-[11px] text-zinc-500 mt-0.5">Basic info about your photography brand.</p>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="flex flex-col gap-1.5">
                                <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Studio / Brand Name</label>
                                <input type="text" id="cora-settings-brand-name" class="w-full border border-zinc-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:border-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-200 transition-all placeholder:text-zinc-300" value="<?php echo esc_attr( get_option( 'cora_studio_brand_name', 'Cora for Studio' ) ); ?>" placeholder="e.g. Pixel Stories Studio">
                            </div>
                            <div class="flex flex-col gap-1.5">
                        ...
                        <div class="flex flex-col gap-1.5">
                            <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Updates Server URL (JSON)</label>
                            <input type="text" id="cora-settings-updates-url" class="w-full border border-zinc-200 rounded-lg px-3 py-2.5 text-sm bg-white focus:border-zinc-400 focus:outline-none focus:ring-1 focus:ring-zinc-200 transition-all font-mono text-zinc-700 placeholder:font-sans placeholder:text-zinc-300" value="<?php echo esc_attr( get_option( 'cora_updates_server_url', 'https://cora.ai/updates/cora-studio-ai.json' ) ); ?>" placeholder="e.g. https://yourdomain.com/updates/cora-studio-ai.json">
                            <span class="text-[10px] text-zinc-400">Leave default to check updates from cora.ai or input your own server endpoint.</span>
                        </div>
                        <div class="flex flex-col gap-1.5">
                    ...
                    <div class="px-5 py-3 bg-zinc-50/50 border-t border-zinc-100 flex justify-end">
                        <button class="text-xs font-semibold px-4 py-2 bg-zinc-900 text-white rounded-lg hover:bg-zinc-800 transition-all active:scale-[0.97] cursor-pointer inline-flex items-center gap-1.5" onclick="coraSaveStudioSettings(this)">
                ...
                <div class="bg-white border border-zinc-200/80 rounded-xl shadow-sm overflow-hidden">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div class="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11px] text-zinc-500 mt-0.5">Let clients find your studio on Google and manage reviews.</p>
                                </div>
                            </div>
                    ...
                    <div class="p-5 space-y-5">
                        ...
                        <div class="space-y-3">
                            ...
                            <p class="text-[11px] text-zinc-500 leading-relaxed sm:ml-7">Clients can search your studio name and instantly connect your Google listing. This uses a Google Maps API key.</p>
                ...
                <p class="cora-section-desc text-sm text-zinc-500 -mt-2">Track active tools, in-progress integration                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pt-2">
                    ...
                    <div id="cora-card-client-galleries" class="cora-feature-card bg-[#fafaf9] border border-zinc-300 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-955 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group relative overflow-hidden animate-in fade-in zoom-in-95 duration-200" data-feature="Beautiful Client Galleries">
                        ...
                        <div class="space-y-3">
                            ...
                            <h3 class="text-sm font-bold text-zinc-955 leading-snug group-hover:text-black">Beautiful Client Galleries</h3>
                        ...
                        <div class="pt-3 border-t border-zinc-200/60 mt-2 flex justify-between items-center text-[10px] font-bold text-zinc-500 group-hover:text-zinc-900">
                            <span>Open Studio Galleries</span>
                            <span>→</span>
                        </div>
                    </div>

                    <!-- 2. Easy Photo Selection (ACTIVE) -->
                    <div id="cora-card-photo-selection" class="cora-feature-card bg-[#fafaf9] border border-zinc-300 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-950 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group relative overflow-hidden animate-in fade-in zoom-in-95 duration-200" data-feature="Easy Photo Selection">
                        ...
                        <div class="space-y-3">
                            ...
                            <h3 class="text-sm font-bold text-zinc-955 leading-snug group-hover:text-black">Easy Photo Selection</h3>
                        ...
                        <div class="pt-3 border-t border-zinc-200/60 mt-2 flex justify-between items-center text-[10px] font-bold text-zinc-500 group-hover:text-zinc-900">
                            <span>Active in Client Galleries</span>
                            <span>→</span>
                        </div>
                    </div>
                    ...
                    <div class="cora-feature-card cora-feature-soon bg-white border border-zinc-200/80 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-350 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group animate-in fade-in zoom-in-95 duration-200" data-feature="Branded Print Storefront">
                        <div class="space-y-3">
                            ...
                            <p class="text-xs text-zinc-550 leading-normal">Sell premium layflat print albums, canvas prints, and custom frames directly to gallery visitors with automated print lab fulfillment.</p>
                        </div>
                    </div>
                    ...
                    <div id="cora-card-instant-quotations" class="cora-feature-card bg-[#fafaf9] border border-zinc-300 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-950 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group relative overflow-hidden animate-in fade-in zoom-in-95 duration-200" data-feature="Instant Quotations">
                        ...
                        <div class="space-y-3">
                            ...
                            <p class="text-xs text-zinc-650 leading-normal">Generate professional PDFs with your wedding packages and send them to clients in 1 click.</p>
                        </div>
                        <div class="pt-3 border-t border-zinc-200/60 mt-2 flex justify-between items-center text-[10px] font-bold text-zinc-500 group-hover:text-zinc-900">
                    ...
                    <div id="cora-card-zero-paperwork" class="cora-feature-card bg-[#fafaf9] border border-zinc-300 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-950 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group relative overflow-hidden animate-in fade-in zoom-in-95 duration-200" data-feature="Automated Invoicing & Receipts">
                        ...
                        <div class="space-y-3">
                            ...
                            <p class="text-xs text-zinc-650 leading-normal">Automatically generate wedding contracts, advance booking receipts, and tax-compliant invoices.</p>
                        </div>
                        <div class="pt-3 border-t border-zinc-200/60 mt-2 flex justify-between items-center text-[10px] font-bold text-zinc-500 group-hover:text-zinc-900">
                            <span>Open Studio Vault</span>
                            <span>→</span>
                    ...
                    <div class="cora-feature-card cora-feature-soon bg-white border border-zinc-200/80 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-350 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group animate-in fade-in zoom-in-95 duration-200" data-feature="Smart Review Acquisition">
                        <div class="space-y-3">
                            ...
                            <p class="text-xs text-zinc-550 leading-normal">Auto-send custom review requests via WhatsApp/SMS after photo gallery delivery to collect 5-star Google Business ratings.</p>
                        </div>
                    </div>
                    ...
                    <div class="cora-feature-card cora-feature-soon bg-white border border-zinc-200/80 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-350 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group animate-in fade-in zoom-in-95 duration-200" data-feature="Referral Rewards Engine">
                        <div class="space-y-3">
                            ...
                            <p class="text-xs text-zinc-550 leading-normal">Create automated referral links for past couples. Give them print storefront discounts when their friends book a photoshoot.</p>
                        </div>
                    </div>
                    ...
                    <div id="cora-card-equipment-tracking" class="cora-feature-card bg-[#fafaf9] border border-zinc-300 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-950 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group relative overflow-hidden animate-in fade-in zoom-in-95 duration-200" data-feature="Equipment Registry & Gear Log">
                        ...
                        <div class="space-y-3">
                            ...
                            <h3 class="text-sm font-bold text-zinc-955 leading-snug group-hover:text-black">Equipment Registry & Gear Log</h3>
                    ...
                    <div class="cora-feature-card cora-feature-soon bg-white border border-zinc-200/80 rounded-xl p-5 shadow-sm hover:shadow-md hover:border-zinc-350 transition-all duration-200 cursor-pointer flex flex-col justify-between min-h-[160px] group animate-in fade-in zoom-in-95 duration-200" data-feature="Multi-Day Event Timeline">
                        <div class="space-y-3">
                            ...
                            <p class="text-xs text-zinc-550 leading-normal">Map out multi-day Indian wedding schedules (Haldi, Sangeet, Reception) with venue locations, crew shifts, and real-time client view access.</p>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section id="cora-page-gbp" class="cora-page-section cora-active space-y-6">
                ...
                <div class="cora-page-header flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <h1 class="cora-page-title text-2xl font-bold tracking-tight text-zinc-900">Google Business Profile Manager</h1>
                            ...
                            <div class="flex gap-2">
                                <div class="flex-1 relative">
                                    ...
                                    <input
                                        ...
                                        placeholder="Business name, e.g. Nitin Arora Photography"
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-5">
                        ...
                        <div class="cora-card bg-white border border-zinc-200/85 rounded-xl p-6 shadow-sm">
                            ...
                            <p class="text-xs text-zinc-500 leading-relaxed">Sign in with Google above to unlock post publishing — share updates, offers, and shoot previews directly on your Google listing.</p>
                        </div>
                    </div>
                    <!-- Right: Business card -->
                    <div class="space-y-5">
                        ...
                        <div class="cora-card bg-white border border-zinc-200/85 rounded-xl p-5 shadow-sm space-y-3">
                            ...
                            <div class="space-y-2 text-xs text-zinc-500">
                                ...
                                <p class="flex items-start gap-2"><span class="text-zinc-300 shrink-0">→</span>Upload shoot photos — Google uses image quality as a signal</p>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="cora-card bg-white border border-zinc-200/85 rounded-xl p-5 shadow-sm space-y-4">
                            ...
                            <div id="cora-gbp-reviews-empty" class="hidden py-6 flex flex-col items-center gap-3 text-center">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-zinc-400 mt-1">Share your Google Review link with clients after each shoot.</p>
                                </div>
                            </div>
                        </div>
                        <!-- Post Scheduler -->
                        <div class="cora-card bg-white border border-zinc-200/85 rounded-xl p-5 shadow-sm space-y-4">
                            ...
                            <div class="space-y-3">
                                <div>
                                    ...
                                    <textarea id="cora-gbp-post-content" class="w-full border border-zinc-200 rounded-md p-2.5 text-sm bg-white focus:border-zinc-400 focus:outline-none h-24 resize-none" placeholder="Share a recent shoot highlight, new availability, or a seasonal offer..."></textarea>
                                </div>
                                <div class="grid grid-cols-2 gap-3">
                                    <div>
                    ...
                    <div class="space-y-5">
                        ...
                        <div class="cora-card bg-white border border-zinc-200/85 rounded-xl p-5 shadow-sm space-y-3">
                            ...
                            <div class="space-y-2 text-xs text-zinc-500">
                                ...
                                <p class="flex items-start gap-2"><span class="text-zinc-300 shrink-0">→</span>Upload shoot photos for ranking signals</p>
            ...
            <section id="cora-page-team-roles" class="cora-page-section cora-active space-y-6">
                ...
                <div class="cora-sub-tabs border-b border-zinc-200 flex gap-4 text-xs font-bold text-zinc-550 select-none pb-0.5">
                    ...
                    <button class="cora-sub-tab pb-2 border-b-2 border-transparent hover:text-zinc-900 cursor-pointer flex items-center gap-1.5" id="cora-sub-tab-team-attendance" data-sub-target="team-attendance">
                        Attendance Logs
                ...
                <!-- SUB-SECTION 4: ATTENDANCE LOGS -->
                <div id="cora-sub-page-team-attendance" class="cora-sub-section hidden space-y-4">
                    <div class="flex items-center justify-between mb-2">
                        <div>
                            <h3 class="text-sm font-bold text-zinc-900">Attendance Registry</h3>
                            <p class="text-[11px] text-zinc-500 mt-0.5">Track daily punch-in logs with geolocation audits. Admins can manually edit times if required.</p>
                        </div>
                        <div class="flex items-center gap-2">
                            <button id="cora-btn-set-office" onclick="coraToggleOfficeLocationDrawer(true)" class="text-xs font-medium text-blue-600 bg-blue-50 border border-blue-200 rounded px-2.5 py-1 hover:bg-blue-100 flex items-center gap-1.5 transition-all shadow-sm active:scale-95 cursor-pointer">
                                <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                                <?php
                                $office_loc = get_option( 'cora_office_location', array() );
                                if ( ! empty( $office_loc['name'] ) ) {
                                    $office_btn_text = 'Office: ' . substr( $office_loc['name'], 0, 15 ) . ( strlen( $office_loc['name'] ) > 15 ? '...' : '' );
                                } elseif ( ! empty( $office_loc['lat'] ) ) {
                                    $office_btn_text = 'Update Office Location';
                                } else {
                                    $office_btn_text = 'Set Office Location';
                                }
                                ?>
                                <span id="cora-office-btn-text"><?php echo esc_html( $office_btn_text ); ?></span>
                            </button>
                            <?php if ( current_user_can( 'administrator' ) ) : ?>
                            <button onclick="coraAddAttendanceRecord()" class="text-xs font-medium text-zinc-700 bg-white border border-zinc-200 rounded px-2.5 py-1 hover:bg-zinc-50 flex items-center gap-1.5 transition-all shadow-sm active:scale-95 cursor-pointer">
                                <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>
                                Add Record
                            </button>
                            <?php endif; ?>
                            <button onclick="coraLoadAttendance()" class="text-xs font-medium text-zinc-700 bg-white border border-zinc-200 rounded px-2.5 py-1 hover:bg-zinc-50 flex items-center gap-1.5 transition-all shadow-sm active:scale-95 cursor-pointer">
                                <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round"><polyline points="23 4 23 10 17 10"></polyline><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"></path></svg>
                                Refresh Log
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
                        <div class="bg-white border border-zinc-200 rounded-lg p-3 flex flex-col justify-center">
                            <span class="text-[10px] uppercase font-bold text-zinc-500 mb-1">Total Team</span>
                            <span id="cora-overview-total" class="text-lg font-bold text-zinc-900">--</span>
                        </div>
                        <div class="bg-white border border-zinc-200 rounded-lg p-3 flex flex-col justify-center">
                            <span class="text-[10px] uppercase font-bold text-zinc-500 mb-1">Present Today</span>
                            <span id="cora-overview-present" class="text-lg font-bold text-green-600">--</span>
                        </div>
                        <div class="bg-white border border-zinc-200 rounded-lg p-3 flex flex-col justify-center">
                            <span class="text-[10px] uppercase font-bold text-zinc-500 mb-1">Missing / Absent</span>
                            <span id="cora-overview-absent" class="text-lg font-bold text-red-600">--</span>
                        </div>
                        <div class="bg-white border border-zinc-200 rounded-lg p-3 flex flex-col justify-center">
                            <span class="text-[10px] uppercase font-bold text-zinc-500 mb-1">Flagged Locations</span>
                            <span id="cora-overview-flagged" class="text-lg font-bold text-amber-600">--</span>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-zinc-200 rounded-lg overflow-hidden shadow-sm">
                        <table class="w-full text-left text-xs">
                            <thead class="bg-zinc-50/50 border-b border-zinc-200 text-zinc-500">
                                <tr>
                                    <th class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Staff Member</th>
                                    <th class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Date</th>
                                    <th class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Punch In</th>
                                    <th class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Punch Out</th>
                                    <th class="px-4 py-3 font-bold uppercase tracking-wider text-[10px]">Location</th>
                                    <th class="px-4 py-3 font-bold uppercase tracking-wider text-[10px] text-right">Actions</th>
                                </tr>
                            </thead>
                            <tbody id="cora-attendance-tbody" class="divide-y divide-zinc-150 text-zinc-700">
                                <tr><td colspan="6" class="px-4 py-10 text-center text-zinc-400">Loading registry...</td></tr>
                            </tbody>
                        </table>
                    </div>
                </div>
                            ...
                            <table class="min-w-full divide-y divide-zinc-200 text-xs text-left" id="cora-equipment-table">
                                <thead>
                                    <tr class="bg-zinc-50/50">
                                        ...
                                        <th class="px-4 py-3 font-bold text-zinc-500 uppercase tracking-wider text-[10px] w-28 whitespace-nowrap">Serial Number</th>
                                        ...
                                        <th class="px-4 py-3 font-bold text-zinc-500 uppercase tracking-wider text-[10px] min-w-[160px]">Active Event / Shoot</th>
                                ...
                                <tbody class="divide-y divide-zinc-150 bg-white" id="cora-equipment-table-body">
                                    ...
                                    <tr class="hover:bg-zinc-50/30 cora-eq-row" data-id="<?php echo esc_attr($item['id']); ?>" data-name="<?php echo esc_attr($item['name']); ?>">
                                        ...
                                        <td class="px-4 py-3.5 whitespace-nowrap"><span class="bg-zinc-100 text-zinc-500 font-mono text-[9px] px-1.5 py-0.5 rounded border border-zinc-200/40"><?php echo esc_html($item['serial']); ?></span></td>
                                        <td class="px-4 py-3.5 whitespace-nowrap">
                ...
                <div id="cora-sub-page-eq-add" class="cora-sub-section hidden space-y-4">
                    <div class="cora-card bg-white border border-zinc-200/85 rounded-xl p-5 shadow-sm max-w-xl space-y-4">
                        ...
                        <div class="cora-form-group flex flex-col gap-1.5">
                            ...
                            <div class="flex items-center gap-3">
                                <div id="cora-gear-photo-preview" class="w-14 h-14 border border-zinc-200 rounded-lg flex items-center justify-center bg-zinc-50 overflow-hidden shrink-0">
                                    <span class="text-[9px] text-zinc-400 text-center px-1 font-semibold" id="cora-gear-photo-placeholder">No Photo</span>
                                </div>
                                <div class="flex flex-col gap-1">
                                    <input type="file" id="cora-gear-photo-file" accept="image/*" class="hidden">
                                    <button type="button" class="px-3 py-1.5 border border-zinc-250 rounded-md text-xs font-bold text-zinc-700 bg-white hover:bg-zinc-50 active:scale-95 transition-all cursor-pointer shadow-sm" onclick="jQuery('#cora-gear-photo-file').click()">
                                        Choose Photo
                                    </button>
                                </div>
                            </div>
                ...
                <div id="cora-vault-list-view" class="space-y-5">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between border-b border-zinc-100/60 pb-4 sm:pb-0 sm:border-b-0">
                        <div class="cora-page-header flex items-start sm:items-center gap-3">
                            ...
                            <div>
                                <h1 class="cora-page-title text-xl sm:text-2xl font-bold tracking-tight text-zinc-900">Studio Vault</h1>
                ...
                <div id="cora-vault-editor-view" class="hidden space-y-6">
                    ...
                    <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between border-b border-zinc-200 pb-4">
                        ...
                        <div class="flex items-center gap-3 w-full md:w-auto">
                            <button class="flex items-center gap-1.5 px-3 py-1.5 border border-zinc-200 rounded-md text-xs font-semibold text-zinc-700 bg-white hover:bg-zinc-50 transition-all cursor-pointer shrink-0" onclick="coraCloseEditor()">
                                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                    <line x1="19" y1="12" x2="5" y2="12"></line>
                                    <polyline points="12 19 5 12 12 5"></polyline>
                                </svg>
                        ...
                        <div class="flex items-center justify-between md:justify-end gap-3 w-full md:w-auto">
                            ...
                            <div class="flex items-center gap-1.5 sm:gap-2">
                                ...
                                <button id="cora-editor-toggle-sidebar-btn" onclick="coraEditorToggleSidebar()" class="px-2.5 py-1.5 sm:px-3 sm:py-1.5 border border-zinc-200 rounded-md text-xs font-semibold text-zinc-700 bg-white hover:bg-zinc-50 transition-all cursor-pointer flex items-center gap-1.5 shrink-0" title="Toggle Settings Sidebar">
                                    <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round">
                                        <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                                        <line x1="9" y1="3" x2="9" y2="21"></line>
                                    </svg>
                    ...
                    <div id="cora-editor-workspace-columns" class="flex flex-col lg:flex-row gap-6 items-start">
                        ...
                        <aside class="w-full lg:w-80 bg-white border border-zinc-200 rounded-xl p-5 shadow-sm space-y-4 shrink-0">
                            ...
                            <div class="cora-form-group flex flex-col gap-1.5">
                                ...
                                <select id="cora-doc-client-select" class="w-full border border-zinc-200 rounded-md p-2 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors">
                                    ...
                                    <optgroup label="Leads/Prospects">
                                        <?php foreach ( $cora_leads as $lead ) : ?>
                            ...
                            <div class="cora-form-group flex flex-col gap-1.5">
                                ...
                                <select id="cora-doc-template-select" class="w-full border border-zinc-200 rounded-md p-2 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors cursor-pointer" onchange="coraEditorLoadTemplate(this.value)">
                                    <option value="">— Select Template —</option>
                                    <option value="wedding_proposal">Photography & Video Proposal</option>
                                    <option value="intimate_proposal">Intimate Event Proposal</option>
                                    <option value="standard_invoice">Tax Invoice Template</option>
                                    <option value="commercial_invoice">Commercial Shoot Invoice</option>
                                </select>
                            </div>
            ...
            <section id="cora-page-leads" class="cora-page-section cora-active space-y-6">
                ...
                <div class="flex overflow-x-auto gap-4 items-start mt-6 select-none pb-8" style="scrollbar-width: none;">
                    ...
                    foreach ($columns as $status_key => $column_info) :
                        $col_leads = array_filter($cora_leads, function($lead) use ($status_key) {
                    ...
                    <div class="cora-kanban-column flex flex-col p-3 rounded-[18px] bg-white border border-zinc-200/70 shadow-sm shrink-0 w-[320px] h-[calc(100vh-180px)] relative" 
                        ...
                        <div class="cora-cards-container flex-1 space-y-2.5 overflow-y-auto pr-1 pb-14" style="scrollbar-width: none;">
                            <?php foreach ($col_leads as $lead) : 
                                ...
                                else if ($status_key === 'Converted') $next_step = 'Plan shoot logistics';
                            ?>
            ...
            <section id="cora-page-plugins" class="cora-page-section cora-active space-y-6">
                <div class="flex items-center justify-between">
                    <div class="cora-page-header flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-zinc-500 mt-0.5 leading-relaxed">Extend your photography studio workspace with Model Context Protocol (MCP) servers, third-party apps, and developer portals.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div class="border border-zinc-200 rounded-xl p-4 bg-white hover:border-zinc-350 transition-all duration-150 flex flex-col justify-between group">
                            <div class="space-y-2.5">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11.5px] text-zinc-500 leading-relaxed mt-1">Allow LLMs to read and write directly into your Google Calendar shoot schedules, Gmail client logs, and Google Drive spreadsheets.</p>
                                </div>
                            </div>
                        ...
                        <div class="border border-zinc-200 rounded-xl p-4 bg-white hover:border-zinc-350 transition-all duration-150 flex flex-col justify-between group">
                            <div class="space-y-2.5">
                                ...
                                <div>
                                    ...
                                    <p class="text-[11.5px] text-zinc-500 leading-relaxed mt-1">Automatically dispatch DLT-approved shoot briefings, signature proposals, and payment links directly to couples via WhatsApp API.</p>
                                </div>
                            </div>
                            <div class="mt-4 pt-3 border-t border-zinc-100 flex items-center justify-between text-[11px] font-semibold text-zinc-400">
                                <span>Status: Unavailable</span>
                        ...
                        <!-- Card 3: Gemini Studio Brain MCP -->
                        <div class="border border-zinc-200 rounded-xl p-4 bg-white hover:border-zinc-350 transition-all duration-150 flex flex-col justify-between group">
                            <div class="space-y-2.5">
                                <div class="flex items-center justify-between">
                                    <div class="w-8 h-8 rounded-lg bg-zinc-100 flex items-center justify-center border border-zinc-200 text-zinc-600">
                                ...
                                <div>
                                    <h4 class="text-sm font-bold text-zinc-900">Gemini Studio Brain MCP</h4>
                                    <p class="text-[11.5px] text-zinc-500 leading-relaxed mt-1">Hook the Gemini Pro model into your workspace files. Analyze wedding inquiry briefs, cull images smart-tags, and write client contracts.</p>
                                </div>
                            </div>
                            <div class="mt-4 pt-3 border-t border-zinc-100 flex items-center justify-between text-[11px] font-semibold text-zinc-400">
                                <span>Status: Unavailable</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="border border-zinc-200 rounded-xl p-4 bg-white hover:border-zinc-350 transition-all duration-150 flex flex-col justify-between group text-left">
                            <div class="space-y-2">
                                ...
                                <p class="text-[10.5px] text-zinc-500 leading-normal">Auto-archive shoot preview galleries, contract PDFs, and high-res client deliverables directly to Google Drive folders.</p>
                            </div>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div class="border border-zinc-200 rounded-xl p-4 bg-white hover:border-zinc-350 transition-all duration-150 flex items-center gap-4 text-left">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-zinc-500 leading-relaxed mt-0.5">Configure URL webhooks to get instantly notified on shoot lifecycle state changes.</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="cora-page-header flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-zinc-500 mt-0.5">Draft, optimize, and track SEO performance for your studio's articles.</p>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-left border-collapse">
                        ...
                        <tbody class="divide-y divide-zinc-100 text-sm text-zinc-700" id="cora-articles-table-body">
                            <?php if (empty($cora_posts)): ?>
                                <tr>
                                    <td colspan="5" class="py-8 text-center text-zinc-500 text-sm">No articles found. Click "New AI Article" to start drafting!</td>
                                </tr>
                            <?php else: ?>
                                ...
                                <tr class="hover:bg-zinc-50/50 transition-colors cursor-pointer cora-article-row" onclick="coraEditArticle(<?php echo $post->ID; ?>)">
                                    ...
                                    <td class="py-3 px-4">
                                        <?php if($is_published): ?>
                                            <span class="px-2 py-1 bg-green-100 text-green-700 rounded-md text-[10px] font-bold tracking-wider uppercase">Published</span>
                                        <?php else: ?>
                                            <span class="px-2 py-1 bg-yellow-100 text-yellow-700 rounded-md text-[10px] font-bold tracking-wider uppercase">Draft</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="py-3 px-4">
                                        <div class="flex items-center gap-2">
                                    ...
                                    <td class="py-3 px-4">
                                        <?php if($is_published): ?>
                                            <div class="text-xs font-semibold text-zinc-800">Tracking...</div>
                                        <?php else: ?>
            ...
            <!-- SECTION: MY PROFILE -->
            <?php if ( $sub_page === 'my-profile' ) : ?>
            <?php
            $profile_user = $current_wp_user;
            $profile_phone = get_user_meta( $profile_user->ID, 'cora_phone', true );
            $profile_bio = get_user_meta( $profile_user->ID, 'description', true );
            $profile_avatar = get_user_meta( $profile_user->ID, 'cora_avatar_url', true );
            $profile_registered = date( 'M j, Y', strtotime( $profile_user->user_registered ) );

            // Role-specific stats
            $profile_assigned_bookings = 0;
            $profile_completed_bookings = 0;
            foreach ( $cora_clients as $client ) {
                $crew = isset( $client['crew'] ) ? (array) $client['crew'] : array();
                if ( in_array( $profile_user->display_name, $crew ) || in_array( $profile_user->user_login, $crew ) ) {
                    $profile_assigned_bookings++;
                    if ( isset( $client['status'] ) && $client['status'] === 'completed' ) {
                        $profile_completed_bookings++;
                    }
                }
            }
            // Admin gets total stats
            if ( $current_user_role === 'administrator' ) {
                $profile_assigned_bookings = count( $cora_clients );
                $profile_completed_bookings = 0;
                foreach ( $cora_clients as $c ) {
            ...
            ?>
            <section id="cora-page-my-profile" class="cora-page-section cora-active space-y-5">
                <!-- Page Header -->
                <div class="cora-page-header flex items-center gap-3">
                    <span class="cora-page-emoji text-zinc-900 flex shrink-0">
                        <svg viewBox="0 0 24 24" width="28" height="28" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round">
                    ...
                    <div>
                        <h1 class="cora-page-title text-xl sm:text-2xl font-bold tracking-tight text-zinc-900">My Profile</h1>
                        <p class="cora-section-desc text-[11px] sm:text-xs text-zinc-500 mt-0.5">View and manage your personal account details.</p>
                    </div>
                </div>
                    ...
                    <div class="p-6 flex flex-col sm:flex-row items-center sm:items-start gap-5">
                        ...
                        <div class="relative group shrink-0">
                            <?php if ( $profile_avatar ) : ?>
                                <img id="cora-profile-avatar-img" src="<?php echo esc_url( $profile_avatar ); ?>" class="w-20 h-20 rounded-full object-cover border-2 border-zinc-200/60 shadow-sm" alt="Profile Avatar" />
                            <?php else : ?>
                                <div id="cora-profile-avatar-img" class="w-20 h-20 rounded-full bg-zinc-200 text-zinc-700 flex items-center justify-center font-bold text-xl uppercase border-2 border-zinc-200/60 shadow-sm">
                                    <?php echo esc_html( substr( $current_user_display_name, 0, 2 ) ); ?>
                                </div>
                            <?php endif; ?>
                            <label for="cora-profile-avatar-input" class="absolute inset-0 flex items-center justify-center bg-zinc-950/40 rounded-full opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer">
                                <svg viewBox="0 0 24 24" width="18" height="18" stroke="white" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>
                            </label>
                            <input type="file" id="cora-profile-avatar-input" class="hidden" accept="image/*" />
                        </div>
                        <!-- Name & Role -->
                        <div class="flex flex-col items-center sm:items-start gap-1 min-w-0 flex-1">
                            <h2 class="text-lg font-bold text-zinc-900 truncate"><?php echo esc_html( $current_user_display_name ); ?></h2>
                            <div class="flex items-center gap-2 flex-wrap justify-center sm:justify-start">
                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider rounded-full border
                                    <?php if ( $current_user_role === 'administrator' ) : ?>
                                        bg-zinc-900 text-white border-zinc-900
                                    <?php elseif ( $current_user_role === 'cora_manager' ) : ?>
                                        bg-zinc-200 text-zinc-800 border-zinc-300
                                    <?php else : ?>
                                        bg-zinc-100 text-zinc-600 border-zinc-200
                                    <?php endif; ?>
                                ">
                                    <svg viewBox="0 0 24 24" width="10" height="10" stroke="currentColor" stroke-width="2.5" fill="none"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"></path></svg>
                                    <?php echo esc_html( $current_user_role_label ); ?>
                                </span>
                                <span class="text-[10px] text-zinc-400 font-medium">UID: <?php echo esc_html( $profile_user->ID ); ?></span>
                            </div>
                            <p class="text-[11px] text-zinc-400 mt-0.5">
                                <svg viewBox="0 0 24 24" width="10" height="10" stroke="currentColor" stroke-width="2" fill="none" class="inline -mt-0.5 mr-0.5"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>
                                Joined <?php echo esc_html( $profile_registered ); ?>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <div class="bg-white border border-zinc-200/80 rounded-xl p-4 flex flex-col">
                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider mb-1">
                            <?php echo $current_user_role === 'administrator' ? 'Total Bookings' : 'Assigned Shoots'; ?>
                        </span>
                        <span class="text-xl font-bold text-zinc-900"><?php echo intval( $profile_assigned_bookings ); ?></span>
                    </div>
                    <div class="bg-white border border-zinc-200/80 rounded-xl p-4 flex flex-col">
                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider mb-1">Completed</span>
                        <span class="text-xl font-bold text-zinc-900"><?php echo intval( $profile_completed_bookings ); ?></span>
                    </div>
                    <div class="bg-white border border-zinc-200/80 rounded-xl p-4 flex flex-col">
                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider mb-1">Platform Role</span>
                        <span class="text-sm font-bold text-zinc-900 truncate"><?php echo esc_html( $current_user_role_label ); ?></span>
                    </div>
                    <div class="bg-white border border-zinc-200/80 rounded-xl p-4 flex flex-col">
                        <span class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider mb-1">
                            <?php echo $current_user_role === 'administrator' ? 'Total Revenue' : 'Status'; ?>
                        </span>
                        <?php if ( $current_user_role === 'administrator' ) : ?>
                            <span class="text-sm font-bold text-zinc-900"><?php echo cora_format_rupees( $dynamic_revenue_total ); ?></span>
                        <?php else : ?>
                ...
                <div class="bg-white border border-zinc-200/80 rounded-xl shadow-sm overflow-hidden">
                    <div class="px-5 pt-5 pb-4 border-b border-zinc-100">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-zinc-900">Personal Information</h3>
                                <p class="text-[11px] text-zinc-500 mt-0.5">Update your name, email, phone, and bio.</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-5 space-y-4">
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="flex flex-col gap-1.5">
                                <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Display Name</label>
                                <input type="text" id="cora-profile-display-name" value="<?php echo esc_attr( $current_user_display_name ); ?>" class="w-full border border-zinc-200 rounded-lg p-2.5 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors font-medium text-zinc-900" />
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Email Address</label>
                                <input type="email" id="cora-profile-email" value="<?php echo esc_attr( $profile_user->user_email ); ?>" class="w-full border border-zinc-200 rounded-lg p-2.5 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors font-medium text-zinc-900" />
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div class="flex flex-col gap-1.5">
                                <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Phone Number</label>
                                <input type="text" id="cora-profile-phone" value="<?php echo esc_attr( $profile_phone ); ?>" placeholder="e.g. +91 98765 43210" class="w-full border border-zinc-200 rounded-lg p-2.5 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors font-medium text-zinc-900" />
                            </div>
                            <div class="flex flex-col gap-1.5">
                                <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Username</label>
                                <input type="text" value="<?php echo esc_attr( $profile_user->user_login ); ?>" disabled class="w-full border border-zinc-200 rounded-lg p-2.5 text-sm bg-zinc-50 text-zinc-500 font-medium cursor-not-allowed" />
                            </div>
                        </div>
                        <div class="flex flex-col gap-1.5">
                            <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Bio / About Me</label>
                            <textarea id="cora-profile-bio" rows="3" placeholder="Write a short bio..." class="w-full border border-zinc-200 rounded-lg p-2.5 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors font-medium text-zinc-900 resize-none"><?php echo esc_textarea( $profile_bio ); ?></textarea>
                        </div>

                        <!-- Save Button -->
                        <div class="flex items-center justify-between pt-2">
                            <button id="cora-profile-save-btn" class="px-5 py-2.5 bg-zinc-900 text-white font-semibold rounded-lg hover:bg-zinc-800 transition-all active:scale-[0.97] cursor-pointer text-sm inline-flex items-center gap-2" onclick="coraSaveMyProfile()">
                                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2.5" fill="none"><polyline points="20 6 9 17 4 12"></polyline></svg>
                                Save Changes
                            </button>
                            <span id="cora-profile-save-status" class="text-[11px] text-zinc-400 font-medium"></span>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-zinc-200/80 rounded-xl shadow-sm overflow-hidden">
                    <div class="px-5 pt-5 pb-4 border-b border-zinc-100">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-zinc-900">Account & Security</h3>
                                <p class="text-[11px] text-zinc-500 mt-0.5">Read-only account details. Contact your admin to change your role.</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-5">
                        <div class="space-y-3">
                            <div class="flex items-center justify-between py-2 border-b border-zinc-100/80">
                                <span class="text-xs text-zinc-500 font-medium">User ID</span>
                                <span class="text-xs font-bold text-zinc-900 font-mono"><?php echo esc_html( $profile_user->ID ); ?></span>
                            </div>
                            <div class="flex items-center justify-between py-2 border-b border-zinc-100/80">
                                <span class="text-xs text-zinc-500 font-medium">Role</span>
                                <span class="text-xs font-bold text-zinc-900"><?php echo esc_html( $current_user_role_label ); ?></span>
                            </div>
                            <div class="flex items-center justify-between py-2 border-b border-zinc-100/80">
                                <span class="text-xs text-zinc-500 font-medium">Registered On</span>
                                <span class="text-xs font-bold text-zinc-900"><?php echo esc_html( $profile_registered ); ?></span>
                            </div>
                            <div class="flex items-center justify-between py-2 border-b border-zinc-100/80">
                                <span class="text-xs text-zinc-500 font-medium">Login Username</span>
                                <span class="text-xs font-bold text-zinc-900 font-mono"><?php echo esc_html( $profile_user->user_login ); ?></span>
                            </div>
                            <div class="flex items-center justify-between py-2">
                                <span class="text-xs text-zinc-500 font-medium">Password</span>
                                <a href="<?php echo esc_url( wp_lostpassword_url( home_url( '/workspace/my-profile' ) ) ); ?>" class="text-xs font-semibold text-zinc-700 hover:text-zinc-950 underline underline-offset-2 transition-colors">Reset Password</a>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white border border-zinc-200/80 rounded-xl shadow-sm overflow-hidden">
                    <div class="px-5 pt-5 pb-4 border-b border-zinc-100">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="text-sm font-bold text-zinc-900">Role Capabilities</h3>
                                <p class="text-[11px] text-zinc-500 mt-0.5">Sections you have access to based on your assigned role.</p>
                            </div>
                        </div>
                    </div>
                    <div class="p-5">
                        <?php
                        $all_features_map = array(
                            'dashboard' => array( 'label' => 'Dashboard', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><rect x="3" y="3" width="7" height="7"></rect><rect x="14" y="3" width="7" height="7"></rect><rect x="14" y="14" width="7" height="7"></rect><rect x="3" y="14" width="7" height="7"></rect></svg>' ),
                            'bookings' => array( 'label' => 'Shoot Bookings', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>' ),
                            'team-roles' => array( 'label' => 'Team & Roles', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>' ),
                            'equipment' => array( 'label' => 'Equipment', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"></path><circle cx="12" cy="13" r="4"></circle></svg>' ),
                            'financials' => array( 'label' => 'Financials', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>' ),
                            'vault' => array( 'label' => 'Studio Vault', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"></path></svg>' ),
                            'gallery' => array( 'label' => 'Gallery', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>' ),
                            'settings' => array( 'label' => 'Settings', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg>' ),
                            'leads' => array( 'label' => 'Leads (CRM)', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="8.5" cy="7" r="4"></circle><line x1="20" y1="8" x2="20" y2="14"></line><line x1="23" y1="11" x2="17" y2="11"></line></svg>' ),
                            'blogs' => array( 'label' => 'AI Content', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline></svg>' ),
                            'gbp' => array( 'label' => 'Google Profile', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><circle cx="12" cy="12" r="10"></circle><line x1="2" y1="12" x2="22" y2="12"></line><path d="M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z"></path></svg>' ),
                            'plugins' => array( 'label' => 'Apps & MCP', 'icon' => '<svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none"><rect x="4" y="4" width="16" height="16" rx="2" ry="2"></rect><rect x="9" y="9" width="6" height="6"></rect><line x1="9" y1="1" x2="9" y2="4"></line><line x1="15" y1="1" x2="15" y2="4"></line><line x1="9" y1="20" x2="9" y2="23"></line><line x1="15" y1="20" x2="15" y2="23"></line><line x1="20" y1="9" x2="23" y2="9"></line><line x1="20" y1="14" x2="23" y2="14"></line><line x1="1" y1="9" x2="4" y2="9"></line><line x1="1" y1="14" x2="4" y2="14"></line></svg>' ),
                        );

                        $user_allowed = array();
                        if ( $current_user_role === 'administrator' ) {
                            $user_allowed = array_keys( $all_features_map );
                        } else {
                            $perms = get_option( 'cora_role_permissions', array() );
                            $user_allowed = isset( $perms[ $current_user_role ] ) ? $perms[ $current_user_role ] : array( 'dashboard' );
                        }
                        ?>
                        <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                            <?php foreach ( $all_features_map as $feature_key => $feature_info ) :
                                $has_access = in_array( $feature_key, $user_allowed );
                            ?>
                            <div class="flex items-center gap-2 px-3 py-2.5 rounded-lg border <?php echo $has_access ? 'border-zinc-200 bg-white' : 'border-zinc-100 bg-zinc-50 opacity-50'; ?>">
                                <span class="shrink-0 <?php echo $has_access ? 'text-zinc-700' : 'text-zinc-400'; ?>"><?php echo $feature_info['icon']; ?></span>
                                <span class="text-xs font-medium <?php echo $has_access ? 'text-zinc-800' : 'text-zinc-400'; ?> truncate"><?php echo esc_html( $feature_info['label'] ); ?></span>
                                <?php if ( $has_access ) : ?>
                                    <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="3" fill="none" class="text-emerald-500 ml-auto shrink-0"><polyline points="20 6 9 17 4 12"></polyline></svg>
                                <?php else : ?>
                                    <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none" class="text-zinc-300 ml-auto shrink-0"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
                                <?php endif; ?>
                            </div>
                            <?php endforeach; ?>
                        </div>
    ...
    <aside id="cora-add-shoot-drawer" class="collapsed fixed top-0 right-0 z-50 h-full w-full max-w-[480px] bg-white border-l border-zinc-200 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out">
    ...
    <aside id="cora-lead-drawer" class="collapsed fixed top-0 right-0 z-50 h-full w-full max-w-[480px] bg-white border-l border-zinc-200 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out">
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div id="cora-lead-tab-general" class="cora-lead-tab-content space-y-4">
                ...
                <div class="border border-zinc-200/80 rounded-lg p-4 space-y-3.5 bg-zinc-50/20">
                    <div class="flex items-center gap-1.5 border-b border-zinc-150 pb-2 mb-1">
                        ...
                        <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Shoot Scope</span>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-3.5">
                        <div class="cora-form-group flex flex-col gap-1">
                            ...
                            <select id="cora-lead-scale" class="w-full border border-zinc-200 rounded-md px-2 py-2 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors cursor-pointer">
                                ...
                                <option value="documentary">Pre-wedding Documentary</option>
                            </select>
                        </div>
                        <div class="cora-form-group flex flex-col gap-1">
            ...
            <div id="cora-lead-tab-assets" class="cora-lead-tab-content space-y-4 hidden">
                <!-- Demo presentation gallery link & tracking -->
                <div class="border border-zinc-200/80 rounded-lg p-4 space-y-3.5 bg-zinc-50/20">
                    <div class="flex items-center gap-1.5 border-b border-zinc-150 pb-2 mb-1">
                    ...
                    <div class="cora-form-group flex flex-col gap-1">
                        ...
                        <select id="cora-lead-demo-gallery" class="w-full border border-zinc-200 rounded-md px-2 py-2 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors cursor-pointer">
                            <option value="">-- No Demo Gallery Linked --</option>
                            <!-- Loop options dynamically in JS -->
                        </select>
                    </div>

                    <!-- Tracking Info Box -->
                    <div id="cora-lead-gallery-tracking-box" class="border border-zinc-200 rounded-md p-3 bg-white space-y-2.5 hidden">
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-[10px] text-zinc-450 uppercase font-bold tracking-wider">Sharing State</span>
                            <span id="cora-lead-gallery-shared-badge" class="px-2 py-0.5 text-[10px] font-bold rounded bg-zinc-100 text-zinc-650">Not Shared</span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span class="text-[10px] text-zinc-450 uppercase font-bold tracking-wider">Client Viewed</span>
                            <span id="cora-lead-gallery-viewed-badge" class="px-2 py-0.5 text-[10px] font-bold rounded bg-zinc-100 text-zinc-650">Unopened</span>
                        </div>
            ...
            <div id="cora-lead-tab-equipment" class="cora-lead-tab-content space-y-4 hidden">
                <div class="border border-zinc-200/80 rounded-lg p-4 space-y-3.5 bg-zinc-50/20">
                    <div class="flex items-center gap-1.5 border-b border-zinc-150 pb-2 mb-1">
                        ...
                        <span class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Required Shoot Gear</span>
                    </div>
                    
                    <p class="text-[11px] text-zinc-500 leading-relaxed mb-1">Select and assign equipment from the studio catalog required for this prospect's shoot scope.</p>

                    <div id="cora-lead-equipment-list" class="grid grid-cols-1 gap-2.5 max-h-[300px] overflow-y-auto pr-1">
                        <!-- Loop through coraData.equipment in JS to output checkboxes -->
                        <div class="text-[11px] text-zinc-450 py-1 text-center">No studio equipment catalog loaded.</div>
                    </div>
                </div>
            </div>
    ...
    <aside id="cora-client-drawer" class="collapsed fixed top-0 right-0 z-50 h-full w-full max-w-[500px] bg-white border-l border-zinc-200 shadow-none flex flex-col transition-transform duration-300 ease-in-out">
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-6">
            ...
            <!-- Timeline Stage 4: Assets & Galleries -->
            <div class="relative pl-6 border-l-2 border-transparent">
                ...
                <div id="cora-lifecycle-assets-container" class="space-y-2">
                    <div class="text-[11px] text-zinc-400 italic py-2">No galleries delivered yet.</div>
                </div>
            </div>
    ...
    <aside id="cora-gallery-drawer" class="collapsed fixed top-0 right-0 z-50 h-full w-[460px] max-w-[90vw] bg-white border-l border-zinc-200 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out">
        <div class="cora-ai-sidebar-header flex justify-between items-center px-4 py-3 border-b border-zinc-200/80 bg-zinc-50 shrink-0">
            <span id="cora-gallery-drawer-title" class="text-xs font-bold text-zinc-800 flex items-center uppercase tracking-wider gap-1.5">
                <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2.2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="text-zinc-500">
                    <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                    <circle cx="8.5" cy="8.5" r="1.5"></circle>
                    <polyline points="21 15 16 10 5 21"></polyline>
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-5">
            <input type="hidden" id="cora-gallery-id" value="">
            
            <div class="cora-form-group flex flex-col gap-1.5">
                <label class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Gallery Title</label>
                <input type="text" id="cora-gallery-title" placeholder="e.g. Arjun & Priya Pre-Wedding" class="w-full border border-zinc-200 rounded-md p-2 text-xs bg-white focus:border-zinc-400 focus:outline-none transition-colors text-zinc-800 font-medium">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="cora-form-group flex flex-col gap-1.5">
                    <label class="text-[9px] font-bold text-zinc-400 uppercase tracking-wider">Layout Template</label>
                    <select id="cora-gallery-template" class="w-full border border-zinc-200 rounded-md p-2 text-xs bg-white focus:border-zinc-400 focus:outline-none transition-colors text-zinc-800 font-semibold cursor-pointer">
                ...
                <div class="cora-form-group flex flex-col gap-1.5">
                    ...
                    <input type="text" id="cora-gallery-password" placeholder="Leave empty for public" class="w-full border border-zinc-200 rounded-md p-2 text-xs bg-white focus:border-zinc-400 focus:outline-none transition-colors text-zinc-800 font-medium">
                </div>
            </div>

            <!-- Client Selections Panel (Visible only when editing) -->
            <div id="cora-gallery-selections-section" class="hidden border border-zinc-200/80 rounded-lg p-3 bg-zinc-50/50 space-y-2">
                ...
                <div id="cora-gallery-selections-list" class="max-h-24 overflow-y-auto text-[11px] text-zinc-600 divide-y divide-zinc-100 font-medium">
            ...
            <div class="space-y-3">
                ...
                <div id="cora-gallery-assets-container" class="space-y-3 min-h-[150px]">
        ...
        <div class="cora-drawer-footer p-4 flex items-center gap-2.5 shrink-0 border-t border-zinc-100 bg-zinc-50">
            <button id="cora-gallery-submit-btn" class="flex-1 py-2.5 bg-zinc-950 text-white font-semibold rounded-md hover:bg-zinc-800 transition-all active:scale-[0.98] cursor-pointer text-sm" onclick="coraSaveGalleryData()">
                Save Gallery Folder
            </button>
    ...
    <aside id="cora-transaction-drawer" class="collapsed fixed top-0 right-0 z-50 h-full w-full max-w-[420px] bg-white border-l border-zinc-200 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out">
        ...
        <div class="flex-1 overflow-y-auto p-5 space-y-4">
            ...
            <div class="cora-form-group flex flex-col gap-1.5">
                ...
                <input type="text" id="cora-tx-desc" class="w-full border border-zinc-200 rounded-md p-2 text-sm bg-white focus:border-zinc-400 focus:outline-none transition-colors" placeholder="e.g. 50% Advance for Pre-wedding Shoot">
            </div>
    ...
    <nav class="cora-bottom-nav fixed bottom-0 left-0 w-full h-14 bg-white/95 backdrop-blur-sm border-t border-zinc-200 flex justify-around items-center z-45 px-2 lg:hidden shadow-[0_-2px_10px_rgba(0,0,0,0.03)]">
        ...
        <div class="cora-bottom-nav-item <?php echo $sub_page === 'gallery' ? 'cora-active' : ''; ?> flex flex-col items-center justify-center text-[10px] font-medium text-zinc-400 cursor-pointer py-1 flex-1 transition-colors duration-150" data-target="gallery">
            <svg viewBox="0 0 24 24" width="18" height="18" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round" class="mb-0.5">
                <rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect>
                <circle cx="8.5" cy="8.5" r="1.5"></circle>
                <polyline points="21 15 16 10 5 21"></polyline>
    ...
    <!-- Modals -->
    <!-- Share Gallery Modal -->
    <div id="cora-modal-share-gallery" class="cora-modal-overlay">
    ...
    <div id="cora-modal-link-drive" class="cora-modal-overlay">
        <div class="cora-modal-card">
            ...
            <div class="cora-modal-body space-y-4">
                <p class="text-sm text-zinc-600">Paste a direct link to a Google Drive file to add it to this gallery without downloading.</p>
                <div>
        ...
        <div class="cora-modal-card">
            ...
            <div class="cora-modal-body space-y-4">
                <div class="bg-zinc-50 border border-zinc-200/80 rounded-md p-3">
                    <p class="text-xs text-zinc-700 font-medium">Link a public Google Drive folder to automatically sync its contents to this gallery. Since direct connection requires credentials, this demo simulates syncing by adding 3 professional wedding portraits and 5 cinematic video reels.</p>
                </div>
                <div>
    ...
    <!-- Advanced Full-Page AI Content Editor -->
    <style>
    #cora-full-page-editor { display: none; }
    #cora-full-page-editor:not(.hidden) { display: flex !important; }
    </style>
    <div id="cora-full-page-editor" class="hidden fixed inset-0 z-[100] bg-white flex-col h-full overflow-hidden">
        ...
        <header class="flex items-center justify-between px-6 py-4 border-b border-zinc-200 bg-white shrink-0">
            <div class="flex items-center gap-4">
                ...
                <span class="text-xs font-bold text-zinc-400 uppercase tracking-wider">AI Content Editor</span>
            </div>
            <div class="flex items-center gap-3">
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <aside class="w-[320px] bg-zinc-50 border-l border-zinc-200 flex flex-col overflow-y-auto shrink-0">
                ...
                <div class="p-6 space-y-4">
                    <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">SEO Optimization</span>
                    <div class="space-y-1">
                        <label class="text-[10px] font-medium text-zinc-600 block">Focus Keyword</label>
                        <input type="text" id="cora-seo-keyword" placeholder="e.g. Wedding Photography" class="w-full text-xs border border-zinc-200 rounded p-2 focus:outline-none focus:border-zinc-400 bg-white">
                    </div>
                    <div class="space-y-1">
            ...
            <div class="cora-modal-body space-y-5 p-4 sm:p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" id="cora-upload-tags" placeholder="e.g. wedding, rings, outdoors" class="w-full bg-white border border-zinc-200 rounded-lg px-3 py-2 sm:py-2.5 text-xs text-zinc-900 focus:outline-none focus:border-zinc-900 focus:ring-1 focus:ring-zinc-200 transition-all placeholder:text-zinc-300">
    ...
    <!-- Set Office Location Drawer -->
    <aside id="cora-office-location-drawer" class="translate-x-full fixed top-0 right-0 z-[150] h-full w-[450px] max-w-[90vw] bg-white border-l border-zinc-200 shadow-2xl flex flex-col transition-transform duration-300 ease-in-out">
        <header class="flex items-center justify-between px-5 py-3 border-b border-zinc-200 bg-white shrink-0">
            <h3 class="text-sm font-bold text-zinc-900 flex items-center gap-2">
                <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2.2" fill="none"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path><circle cx="12" cy="10" r="3"></circle></svg>
                Office Location Setting
            </h3>
            <button class="text-zinc-400 hover:text-zinc-900 transition-colors cursor-pointer" onclick="coraToggleOfficeLocationDrawer(false)">
                <svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
            </button>
        </header>

        <div class="flex-1 overflow-y-auto bg-zinc-50/30 p-5 space-y-4">
            <p class="text-xs text-zinc-500 mb-2">Set the precise location of the office. Any attendance punch exceeding a 1000m radius of this point will be flagged for review.</p>
            
            <div class="flex flex-col gap-1.5 relative z-[2000]">
                <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Search Address</label>
                <div class="relative flex gap-2">
                    <input type="text" id="cora-office-address-search" oninput="coraDebounceSearch(this.value)" placeholder="Type location to search..." class="flex-1 bg-white border border-zinc-200 rounded-md px-3 py-2 text-xs focus:outline-none focus:border-zinc-900">
                    <button class="px-3 py-2 bg-zinc-900 text-white rounded-md text-xs font-semibold cursor-pointer hover:bg-zinc-800 transition-colors" onclick="coraSearchOfficeLocation()">Search</button>
                </div>
                <div id="cora-office-search-results" class="hidden absolute top-[55px] left-0 w-full bg-white border border-zinc-200 shadow-lg rounded-md z-[2000] max-h-[150px] overflow-y-auto"></div>
            </div>
            
            <div class="border border-zinc-200 rounded-lg overflow-hidden bg-white shadow-sm mt-4">
                <div id="cora-office-map" style="height: 300px; width: 100%;"></div>
            </div>

            <div class="grid grid-cols-2 gap-3 mt-4">
                <div>
                    <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Latitude</label>
                    <input type="text" id="cora-office-lat" readonly class="w-full bg-zinc-100 border border-zinc-200 rounded-md px-3 py-2 text-xs text-zinc-600">
                </div>
                <div>
                    <label class="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Longitude</label>
                    <input type="text" id="cora-office-lng" readonly class="w-full bg-zinc-100 border border-zinc-200 rounded-md px-3 py-2 text-xs text-zinc-600">
                </div>
            </div>
        </div>

        <div class="p-5 border-t border-zinc-200 bg-white shrink-0 flex gap-3">
            <button class="flex-1 py-2 bg-white border border-zinc-200 text-zinc-700 font-semibold rounded-md hover:bg-zinc-50 transition-colors text-xs cursor-pointer" onclick="coraToggleOfficeLocationDrawer(false)">Cancel</button>
            <button class="flex-1 py-2 bg-zinc-950 text-white font-semibold rounded-md hover:bg-zinc-800 transition-colors text-xs cursor-pointer" onclick="coraSaveOfficeLocationDrawer()">Save Location</button>
        </div>
    </aside>
...
<!-- Dynamic timezone-aware greeting listener -->
<script>
...
<!-- Cora Notification System (Sidebar + Header Bell) -->
<script>
(function() {
    ...
    function renderCoraNotifications() {
        ...
        if (unreadCount > 0) {
            ...
            if (sidebarBadge) { sidebarBadge.textContent = unreadCount; sidebarBadge.classList.remove('hidden'); }
        } else {
        ...
        [listContainer, sidebarListContainer].forEach(container => {
            if (!container) return;
            container.innerHTML = html;
            container.querySelectorAll('[data-id]').forEach(el => {
                el.addEventListener('click', function(e) {
                    handleCoraNotifClick(e, this.getAttribute('data-id'), this.getAttribute('data-url'));
                });
            });
        });
    }

    // Toggle header notification dropdown
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (bellBtn) bellBtn.addEventListener('click', toggleNotificationDropdown);
...
<!-- Cora Advanced Command Search Modal (Command Palette for Studio) -->
<div id="cora-command-palette" class="fixed inset-0 z-[999999] hidden items-start justify-center p-4 pt-[12vh] bg-zinc-950/40 backdrop-blur-sm transition-all duration-200">
    <div class="cora-command-container w-full max-w-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[380px] transition-transform transform scale-95 duration-200">
        ...
        <div class="flex items-center gap-3 px-4 border-b border-zinc-100 dark:border-zinc-800/40 py-3.5 shrink-0">
            ...
            <input type="text" id="cora-command-input" placeholder="Search pages, settings, shoots, or projects..." class="flex-1 text-sm bg-transparent border-0 outline-none focus:ring-0 text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 py-0.5" autocomplete="off">
        ...
        <div class="flex items-center gap-1.5 px-4 py-2 border-b border-zinc-100 dark:border-zinc-800/40 bg-zinc-50/50 dark:bg-zinc-900/40 overflow-x-auto shrink-0 select-none no-scrollbar">
            ...
            <button type="button" class="cora-search-pill text-xs font-medium px-3 py-1 rounded-full border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 text-zinc-650 dark:text-zinc-405 hover:bg-zinc-55 dark:hover:bg-zinc-800 transition-all cursor-pointer" data-filter="shoots">Shoots</button>
        ...
        <div class="border-t border-zinc-100 dark:border-zinc-800/40 px-4 py-2.5 bg-zinc-50/50 dark:bg-zinc-900/40 flex items-center justify-between shrink-0">
            ...
            <button type="button" class="px-3 py-1.5 bg-zinc-950 hover:bg-zinc-800 dark:bg-zinc-100 dark:hover:bg-white text-white dark:text-zinc-950 font-semibold text-xs rounded-lg transition-colors shadow-sm flex items-center gap-1.5 cursor-pointer" onclick="coraCloseCommandPalette(); window.coraToggleSidebar(true);">
...
<style>
#cora-command-palette { display: none; }
#cora-command-palette:not(.active) { display: none !important; }
#cora-command-palette.active { display: flex !important; }
#cora-command-palette.active .cora-command-container { transform: scale(1) !important; }
.cora-command-item.selected { background-color: #f4f4f5 !important; }
.cora-dark-theme #cora-command-palette { background-color: rgba(9, 9, 11, 0.6) !important; }
.cora-dark-theme .cora-command-container { background-color: #09090b !important; border-color: #27272a !important; }
.cora-dark-theme #cora-command-input { color: #f4f4f5 !important; }
</style>

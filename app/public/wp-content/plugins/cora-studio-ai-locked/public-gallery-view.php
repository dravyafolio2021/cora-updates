...
$gallery_id = $found_gallery['id'];
$cookie_name = 'cora_gallery_auth_' . md5( $gallery_id );
$authenticated = false;
$password_error = false;

if ( empty( $found_gallery['password'] ) ) {
    $authenticated = true;
} else {
    if ( isset( $_POST['gallery_password'] ) ) {
        if ( $_POST['gallery_password'] === $found_gallery['password'] ) {
            setcookie( $cookie_name, md5( $found_gallery['password'] ), time() + 86400 * 7, '/' );
            $_COOKIE[$cookie_name] = md5( $found_gallery['password'] ); // Force mock load
            $authenticated = true;
        } else {
            $password_error = true;
        }
    } elseif ( isset( $_COOKIE[$cookie_name] ) && $_COOKIE[$cookie_name] === md5( $found_gallery['password'] ) ) {
        $authenticated = true;
    }
}

$assets = isset( $found_gallery['assets'] ) ? $found_gallery['assets'] : array();
$share_images = !isset( $found_gallery['share_images'] ) || $found_gallery['share_images'] !== false;
$share_videos = !isset( $found_gallery['share_videos'] ) || $found_gallery['share_videos'] !== false;
...
$likes = isset( $found_gallery['likes'] ) ? $found_gallery['likes'] : array();
$template = isset( $found_gallery['template'] ) ? $found_gallery['template'] : 'grid';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title><?php echo esc_html( $found_gallery['title'] ); ?> • Client Gallery</title>
    <!-- Premium Fonts -->
...
<?php if ( ! $authenticated ) : ?>
    ...
    <div class="lock-container">
        <div class="lock-card">
            ...
            <p class="lock-desc">This client gallery folder requires a password to view. Please enter the access code provided by your photographer.</p>
            <form method="POST" action="">
                <input type="password" name="gallery_password" placeholder="Access Code" class="lock-input" required autofocus>
...
<?php else : ?>
    ...
    <header>
        <div class="header-content">
            <div class="header-title-sec">
                <h1 class="gallery-title"><?php echo esc_html( $found_gallery['title'] ); ?></h1>
                <span class="gallery-subtitle"><?php echo count( $assets ); ?> Assets • Indian Standard Format</span>
            </div>
    ...
    <div class="gallery-container">
        <div id="cora-shared-gallery-view" class="<?php 
        ...
        ?>">
            ...
            <?php else : ?>
                ...
                ?>
                    <div class="gallery-card" data-asset-id="<?php echo esc_attr( $asset['id'] ); ?>" data-type="<?php echo esc_attr( $asset['type'] ); ?>" data-liked="<?php echo $is_liked ? 'true' : 'false'; ?>" onclick="openCoraPublicLightbox('<?php echo esc_js($asset['id']); ?>')">
                        ...
                        <?php if ( $asset['type'] === 'video' ) : ?>
                            <div class="media-wrapper video-type" style="position: relative;">
                                ...
                                <?php elseif ( $is_direct_video ) : ?>
                                    <video src="<?php echo esc_url( $url ); ?>" class="gallery-img" style="object-fit: cover; pointer-events: none;" muted playsinline preload="metadata"></video>
                                ...
                                <?php else : ?>
                                    <iframe src="<?php echo esc_url( $url ); ?>" class="gallery-iframe" style="pointer-events: none;" allow="autoplay; encrypted-media" tabindex="-1"></iframe>
                                    <div class="video-play-overlay">
                                        <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                                            <polygon points="5 3 19 12 5 21 5 3"></polygon>
                                        </svg>
                        ...
                        <?php else : ?>
                            <div class="media-wrapper image-type">
                                <img src="<?php echo esc_url( $url ); ?>" alt="<?php echo esc_attr( $asset['name'] ); ?>" class="gallery-img" loading="lazy">
                            </div>
                        <?php endif; ?>
    ...
    <script>
        const galleryHash = '<?php echo esc_js( $found_gallery['hash'] ); ?>';
        ...
        function coraToggleLike(assetId) {
            const card = document.querySelector(`.gallery-card[data-asset-id="${assetId}"]`);
            if (!card) return;
            ...
            data.append('action', 'cora_toggle_gallery_like');
            data.append('gallery_hash', galleryHash);
        ...
        function coraChangeLayout(layoutType, btnElement) {
            ...
            const container = document.getElementById('cora-shared-gallery-view');

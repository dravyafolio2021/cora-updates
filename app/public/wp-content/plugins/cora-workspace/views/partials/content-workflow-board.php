...
// SIMPLIFIED 5-COLUMN STAGES FOR NON-TECHNICAL TEAMS
$stages = [
    'idea'      => 'Idea',
    'drafting'  => 'Drafting',
    'review'    => 'Review',
...
<!-- HORIZONTALLY SCROLLABLE KANBAN BOARD CONTAINER -->
  ...
  <div class="w-80 shrink-0 bg-[#F8F6F0] border border-[#E8E4DC] rounded-2xl flex flex-col transition-all ct-stage-container opacity-100" data-stage="<?php echo $col_key; ?>">
    ...
    <div class="p-3.5 border-b border-[#E8E4DC] flex items-center justify-between bg-[#F4F0E6] rounded-t-2xl">
      ...
      <div class="flex items-center gap-1">
        <span class="ct-stage-count px-2.5 py-0.5 rounded-full bg-zinc-100 border border-zinc-200 text-[11px] font-bold text-zinc-900"><?php echo count($col_cards); ?></span>
        <button class="p-1 text-zinc-400 hover:text-zinc-900 rounded hover:bg-zinc-100 transition-colors" title="Add to <?php echo $col_label; ?>" onclick="openCreateArticleDrawer()">
    ...
    <div class="min-h-[480px] space-y-3 p-3 flex-1 ct-stage-column transition-all opacity-100" 
      ...
      <?php if (empty($col_cards)): ?>
        <div class="h-32 rounded-xl border border-dashed border-zinc-300 flex flex-col items-center justify-center text-zinc-400 text-xs gap-1 bg-white/60">
          ...
          <span class="font-medium text-[11px]">Drop card here</span>
        </div>
      ...
      ?>
        <!-- WORKFLOW CARD (SOLID WHITE NO HARSH OUTLINE) -->
        <div draggable="true" 
...
window.coraWbDrop = function(e, targetStage) {
  ...
  if(cardEl) {
    ...
    // OPTIMISTIC DOM MOVEMENT: Move card instantly into target column DOM
    const targetColEl = document.querySelector(`.ct-stage-column[data-stage="${targetStage}"]`);
    if(targetColEl) {
      // Remove "Drop card here" placeholder if present
      const placeholder = targetColEl.querySelector('.h-32');

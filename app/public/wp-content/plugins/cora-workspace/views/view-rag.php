...
<div class="space-y-6">
    <!-- Header -->
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-5 border-b border-zinc-200 ">
        <div>
            <h1 class="text-2xl font-bold tracking-tight text-zinc-900 flex items-center gap-2.5">
                <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="1.8" fill="none" class="text-zinc-900 "><path d="M4 19.5A2.5 2.5 0 0 1 6.5 17H20"></path><path d="M6.5 2H20v20H6.5A2.5 2.5 0 0 1 4 19.5v-15A2.5 2.5 0 0 1 6.5 2z"></path></svg>
                RAG Knowledge Base
            </h1>
        ...
        <div class="flex items-center gap-2">
            <button onclick="openRagResourceDrawer(0)" class="inline-flex items-center gap-2 bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors cursor-pointer shadow-xs">
    ...
    <div class="p-6 border border-zinc-200 bg-zinc-50/30 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xs relative overflow-hidden">
        <div class="flex items-start gap-4">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center shrink-0 bg-zinc-950 text-white ">
            ...
            <div>
                <div class="flex items-center gap-2">
                    <h3 class="text-base font-bold text-zinc-900 ">Workspace Knowledge Auto-Sync</h3>
                    <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold <?php echo $sync_interval !== 'disabled' ? 'bg-green-50 text-green-700 border border-green-200/50' : 'bg-zinc-150 text-zinc-500 border border-zinc-200 '; ?>">
                        <?php echo $sync_interval !== 'disabled' ? 'Active' : 'Disabled'; ?>
                    </span>
                </div>
                <p class="text-xs text-zinc-500 mt-1 leading-relaxed max-w-xl">
                    Cora updates this workspace's second brain on a <strong class="font-semibold text-zinc-900 "><?php echo esc_html($sync_interval !== 'disabled' ? $sync_interval : 'manual'); ?></strong> cycle using connected AI models.
                </p>
                <div class="flex flex-wrap gap-x-4 gap-y-1.5 mt-3 text-[11px] text-zinc-450">
                    <span class="flex items-center gap-1">
                        ...
                        Last Sync: <strong class="font-bold text-zinc-755 "><?php echo $last_sync_at ? esc_html(date_i18n(get_option('date_format') . ' ' . get_option('time_format'), strtotime($last_sync_at))) : 'Never'; ?></strong>
                    </span>
                    <span class="flex items-center gap-1">
                        ...
                        Cycle budget: <strong class="font-bold text-zinc-755 "><?php echo $sync_budget_pct; ?>%</strong> of quota
                    </span>
                </div>
            </div>
        </div>
        <div class="flex items-center gap-2.5 shrink-0">
            <button onclick="openSyncSettingsDrawer()" class="p-2.5 border border-zinc-200 text-zinc-600 hover:text-zinc-900 rounded-xl hover:bg-zinc-50 transition-all cursor-pointer" title="Sync Settings">
            ...
            <button onclick="openSyncPreviewDrawer()" class="px-4 py-2 border border-zinc-200 text-zinc-800 hover:text-zinc-950 text-xs font-semibold rounded-xl hover:bg-zinc-50 transition-all cursor-pointer">
                Preview Update
            </button>
            <button onclick="triggerImmediateSync(this)" class="inline-flex items-center gap-2 bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-bold px-4 py-2 rounded-xl transition-all cursor-pointer shadow-xs">
                Sync Now
            </button>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
        ...
        <div class="p-5 border border-zinc-200 bg-white rounded-2xl flex flex-col justify-between min-h-[120px] shadow-xs">
            <div class="flex justify-between items-start">
                <span class="text-xs font-bold text-zinc-400 uppercase tracking-wider">Total Resources</span>
                <span class="p-1.5 bg-zinc-50 rounded-lg text-zinc-700 ">
                    <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line><polyline points="10 9 9 9 8 9"></polyline></svg>
                </span>
            </div>
            <div>
                <h3 class="text-3xl font-black text-zinc-900 font-mono tracking-tight"><?php echo number_format($total_resources); ?></h3>
        ...
        <div class="p-5 border border-zinc-200 bg-white rounded-2xl flex flex-col justify-between min-h-[120px] shadow-xs">
            <div class="flex justify-between items-start">
                ...
                <span class="text-[10px] font-bold font-mono px-2 py-0.5 bg-zinc-100 text-zinc-650 rounded-md">
                    <?php echo $usage_percent; ?>% Used
                </span>
            </div>
            <div class="mt-4">
                <div class="flex justify-between text-xs font-bold text-zinc-900 font-mono mb-1.5">
                ...
                <div class="w-full h-1.5 bg-zinc-100 rounded-full overflow-hidden">
                    <div class="h-full bg-zinc-950 rounded-full transition-all duration-500" style="width: <?php echo $usage_percent; ?>%;"></div>
                </div>
            </div>
        </div>
        ...
        <div class="p-5 border border-zinc-200 bg-white rounded-2xl flex flex-col justify-between min-h-[120px] shadow-xs">
            ...
            <div class="grid grid-cols-4 gap-1.5 text-center mt-3">
                <div class="bg-zinc-50 p-2 rounded-xl border border-zinc-100 ">
                    ...
                    <span class="text-xs font-bold text-zinc-900 font-mono"><?php echo $text_count; ?></span>
                </div>
                <div class="bg-zinc-50 p-2 rounded-xl border border-zinc-100 ">
                    <span class="block text-[9px] text-zinc-400 font-bold uppercase">URLs</span>
                    <span class="text-xs font-bold text-zinc-900 font-mono"><?php echo $url_count; ?></span>
                </div>
                <div class="bg-zinc-50 p-2 rounded-xl border border-zinc-100 ">
                    <span class="block text-[9px] text-zinc-400 font-bold uppercase">Blogs</span>
                    <span class="text-xs font-bold text-zinc-900 font-mono"><?php echo $article_count; ?></span>
                </div>
                <div class="bg-zinc-50 p-2 rounded-xl border border-zinc-100 " title="AI Synced Summaries">
                    <span class="block text-[9px] text-zinc-400 font-bold uppercase">Syncs</span>
                    <span class="text-xs font-bold text-zinc-900 font-mono"><?php echo $sync_count; ?></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <?php if ( ! empty( $sync_history ) ) : ?>
        <div class="border border-zinc-200 bg-white rounded-2xl shadow-xs overflow-hidden">
            <details class="group">
                <summary class="flex justify-between items-center px-5 py-4 cursor-pointer font-bold text-xs text-zinc-700 hover:bg-zinc-50/50 transition-all select-none list-none">
                    <div class="flex items-center gap-2">
                ...
                <div class="px-5 pb-5 pt-1 divide-y divide-zinc-100 ">
                    <?php foreach ( $sync_history as $log ) : 
                        $status_badge = 'bg-zinc-100 text-zinc-800 ';
                        if ( $log['status'] === 'success' ) {
                            $status_badge = 'bg-green-50 text-green-700 ';
                        } elseif ( $log['status'] === 'error' ) {
                            $status_badge = 'bg-red-50 text-red-700 ';
                        }
                        ?>
                        <div class="py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    <span class="font-semibold text-zinc-850 ">
                                ...
                                <p class="text-zinc-500 "><?php echo esc_html($log['message']); ?></p>
                            </div>
    ...
    <div class="border border-zinc-200 bg-white rounded-2xl shadow-xs overflow-hidden">
        <!-- Table Toolbar -->
        <div class="px-5 py-4 border-b border-zinc-150 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-zinc-50/20 ">
            <div class="relative max-w-xs w-full">
                ...
                <input type="text" id="rag-table-search" oninput="filterRagResourcesTable()" class="w-full border border-zinc-200 rounded-lg pl-9 pr-3 py-1.5 text-xs bg-white focus:border-zinc-400 outline-none text-zinc-900 " placeholder="Search resources...">
            </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse" id="rag-resources-table">
                <thead>
                    <tr class="border-b border-zinc-200 bg-zinc-50/50 text-[10px] font-bold uppercase tracking-wider text-zinc-500">
                ...
                <tbody class="divide-y divide-zinc-100 ">
                    ...
                    <?php else : ?>
                        <?php foreach ( $resources as $res ) : 
                            ...
                            ?>
                            <tr class="rag-resource-row hover:bg-zinc-50/40 transition-colors" data-id="<?php echo $res['id']; ?>" data-title="<?php echo esc_attr($res['title']); ?>" data-content="<?php echo esc_attr($raw_content); ?>" data-source-type="<?php echo esc_attr($res['source_type']); ?>" data-source-id="<?php echo esc_attr($res['source_id']); ?>">
                                <td class="px-6 py-4">
                                    <div class="text-xs font-bold text-zinc-900 truncate max-w-sm" title="<?php echo esc_attr($res['title']); ?>">
                                ...
                                <td class="px-6 py-4">
                                    <?php if ( $res['source_type'] === 'text' ) : ?>
                                        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-medium bg-zinc-100 text-zinc-800 border border-zinc-200 ">
                                    ...
                                    <?php elseif ( $res['source_type'] === 'platform_sync' ) : ?>
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-medium bg-green-50 text-green-700 border border-green-200/50">
                                            <span class="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span>
                                            Platform Sync
                                        </span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4 text-xs font-mono text-zinc-850 ">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="inline-flex items-center gap-2">
                                        <?php if ($res['source_type'] === 'platform_sync') : ?>
                                            <button onclick="triggerImmediateSync(this)" class="p-1.5 border border-zinc-200 text-zinc-500 hover:text-zinc-900 rounded-lg hover:bg-zinc-50 transition-colors cursor-pointer" title="Force Sync Platform Summaries">
                                        ...
                                        <?php elseif ($res['source_type'] !== 'article') : ?>
                                            <button onclick="openRagResourceDrawer(<?php echo $res['id']; ?>)" class="p-1.5 border border-zinc-200 text-zinc-500 hover:text-zinc-900 rounded-lg hover:bg-zinc-50 transition-colors cursor-pointer" title="Edit Resource">
                                        ...
                                        <?php else : ?>
                                            <a href="<?php echo get_edit_post_link($res['source_id']); ?>" class="p-1.5 border border-zinc-200 text-zinc-500 hover:text-zinc-900 rounded-lg hover:bg-zinc-50 transition-colors cursor-pointer" target="_blank" title="Edit Blog in WordPress">
                                        ...
                                        <?php endif; ?>
                                        <button onclick="deleteRagResource(<?php echo $res['id']; ?>)" class="p-1.5 border border-zinc-200 text-zinc-400 hover:text-red-500 rounded-lg hover:bg-red-50/30 hover:border-red-200 transition-colors cursor-pointer" title="Delete Resource">
...
<div id="cora-rag-resource-drawer" class="fixed top-0 right-0 h-full w-full sm:w-120 bg-white border-l border-zinc-200 shadow-2xl z-[9999] transform translate-x-full transition-transform duration-300 flex flex-col">
    <!-- Header -->
    <div class="px-6 py-4 border-b border-zinc-100 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-2.5">
            ...
            <div>
                <h3 class="text-base font-bold text-zinc-900 " id="rag-drawer-title">Ingest Resource</h3>
                <p class="text-xs text-zinc-400" id="rag-drawer-subtitle">Index content to RAG knowledge pool</p>
            </div>
        </div>
        <button onclick="closeRagResourceDrawer()" class="p-1.5 text-zinc-400 hover:text-zinc-700 rounded-lg hover:bg-zinc-100 transition-colors cursor-pointer">
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-5">
        ...
        <!-- Title -->
        <div>
            <label class="block text-xs font-bold text-zinc-650 mb-1.5">Resource Name / Title</label>
            <input type="text" id="rag-field-title" class="w-full border border-zinc-200 rounded-lg px-3 py-2 text-xs bg-white focus:border-zinc-400 outline-none text-zinc-900 " placeholder="e.g. Luxury Real Estate Marketing Pitch">
        </div>

        <!-- Source Type -->
        <div>
            <label class="block text-xs font-bold text-zinc-650 mb-1.5">Ingestion Format</label>
            <select id="rag-field-source-type" onchange="toggleRagSourceFields()" class="w-full border border-zinc-200 rounded-lg px-3 py-2 text-xs bg-white focus:border-zinc-400 outline-none text-zinc-900 ">
        ...
        <div id="rag-container-content">
            <label class="block text-xs font-bold text-zinc-650 mb-1.5">Content Text</label>
            <textarea id="rag-field-content" rows="12" class="w-full border border-zinc-200 rounded-lg px-3 py-2 text-xs bg-white focus:border-zinc-400 outline-none text-zinc-900 font-sans" placeholder="Paste or type content resource here..."></textarea>
        </div>

        <!-- URL Area -->
        <div id="rag-container-url" class="hidden">
            <label class="block text-xs font-bold text-zinc-650 mb-1.5">Source URL Address</label>
            <input type="url" id="rag-field-url" class="w-full border border-zinc-200 rounded-lg px-3 py-2 text-xs bg-white focus:border-zinc-400 outline-none text-zinc-900 font-mono" placeholder="https://example.com/resource-article">
            <p class="text-[10px] text-zinc-400 mt-1">Cora will crawl, index metadata and text from this URL.</p>
        </div>
    </div>

    <!-- Footer Controls -->
    <div class="px-6 py-4 border-t border-zinc-100 flex items-center justify-end gap-2.5 shrink-0 bg-zinc-50/20 ">
        <button onclick="closeRagResourceDrawer()" class="px-4 py-2 text-xs font-bold text-zinc-500 hover:text-zinc-800 transition-colors cursor-pointer">
            Cancel
        </button>
        <button onclick="saveRagResource()" id="rag-save-btn" class="inline-flex items-center gap-2 bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-semibold px-4 py-2 rounded-lg transition-colors cursor-pointer shadow-xs">
            Ingest Content
        </button>
    </div>
</div>
...
<div id="cora-rag-settings-drawer" class="fixed top-0 right-0 h-full w-full sm:w-120 bg-white border-l border-zinc-200 shadow-2xl z-[9999] transform translate-x-full transition-transform duration-300 flex flex-col">
    <!-- Header -->
    <div class="px-6 py-4 border-b border-zinc-100 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-2.5">
            <span class="p-2 bg-zinc-100 rounded-lg text-zinc-900 ">
            ...
            <div>
                <h3 class="text-base font-bold text-zinc-900 ">Sync Settings</h3>
                <p class="text-xs text-zinc-400">Configure auto-sync intervals & budget guards</p>
            </div>
        </div>
        <button onclick="closeSyncSettingsDrawer()" class="p-1.5 text-zinc-400 hover:text-zinc-700 rounded-lg hover:bg-zinc-100 transition-colors cursor-pointer">
            <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-6">
        <div>
            <label class="block text-xs font-bold text-zinc-650 mb-1.5">Automated Sync Interval</label>
            <select id="rag-settings-interval" class="w-full border border-zinc-200 rounded-lg px-3 py-2 text-xs bg-white focus:border-zinc-400 outline-none text-zinc-900 ">
        ...
        <div>
            <label class="block text-xs font-bold text-zinc-650 mb-1.5">Token Budget Safety Cap</label>
            <div class="flex items-center gap-4">
                <input type="range" id="rag-settings-budget" min="5" max="50" step="5" value="<?php echo $sync_budget_pct; ?>" class="w-full h-1 bg-zinc-200 rounded-lg appearance-none cursor-pointer accent-zinc-950 " oninput="document.getElementById('rag-settings-budget-label').innerText = this.value + '%'">
                <span id="rag-settings-budget-label" class="font-mono text-xs font-bold text-zinc-850 min-w-[35px] text-right"><?php echo $sync_budget_pct; ?>%</span>
            </div>
    ...
    <!-- Footer -->
    <div class="px-6 py-4 border-t border-zinc-100 flex items-center justify-end gap-2.5 shrink-0 bg-zinc-50/20 ">
        <button onclick="closeSyncSettingsDrawer()" class="px-4 py-2 text-xs font-bold text-zinc-500 hover:text-zinc-800 transition-colors cursor-pointer">
            Cancel
        </button>
        <button onclick="saveRagSyncSettings()" id="rag-settings-save-btn" class="bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors cursor-pointer shadow-xs">
            Save Settings
        </button>
    </div>
</div>
...
<div id="cora-rag-sync-preview-drawer" class="fixed top-0 right-0 h-full w-full sm:w-120 bg-white border-l border-zinc-200 shadow-2xl z-[9999] transform translate-x-full transition-transform duration-300 flex flex-col">
    <!-- Header -->
    <div class="px-6 py-4 border-b border-zinc-100 flex items-center justify-between shrink-0">
        <div class="flex items-center gap-2.5">
            <span class="p-2 bg-zinc-100 rounded-lg text-zinc-900 ">
            ...
            <div>
                <h3 class="text-base font-bold text-zinc-900 ">Sync Preview & Delta</h3>
                <p class="text-xs text-zinc-400">Platform activity changed since last sync</p>
            </div>
        </div>
        <button onclick="closeSyncPreviewDrawer()" class="p-1.5 text-zinc-400 hover:text-zinc-700 rounded-lg hover:bg-zinc-100 transition-colors cursor-pointer">
            <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
        </button>
    </div>
    ...
    <div class="flex-1 overflow-y-auto p-6 space-y-6">
        ...
        <div class="p-4 bg-zinc-50 rounded-2xl border border-zinc-100 space-y-4">
            <div class="flex items-center justify-between text-xs">
                <span class="text-zinc-500">Estimated sync footprint:</span>
                <span class="font-mono font-bold text-zinc-800 "><span id="rag-preview-total-est">0</span> tokens</span>
            </div>
            <div class="flex items-center justify-between text-xs">
                <span class="text-zinc-500">Remaining workspace quota:</span>
                <span class="font-mono font-bold text-zinc-855 "><span id="rag-preview-quota-left">0</span> / <span id="rag-preview-quota-total">0</span></span>
            </div>

            <!-- Double-layered Progress Bar representing Used + Estimated Sync Size -->
            <div class="w-full h-2 bg-zinc-200 rounded-full overflow-hidden relative">
                <!-- Quota Used -->
                <div id="rag-preview-progress-bar-used" class="absolute left-0 top-0 h-full bg-zinc-950 rounded-full transition-all duration-300"></div>
            ...
            <div class="flex justify-end gap-3 text-[9px] text-zinc-400">
                <span class="flex items-center gap-1"><span class="w-2 h-2 bg-zinc-950 rounded-full"></span> Current Usage</span>
    ...
    <!-- Footer -->
    <div class="px-6 py-4 border-t border-zinc-100 flex items-center justify-end gap-2.5 shrink-0 bg-zinc-50/20 ">
        <button onclick="closeSyncPreviewDrawer()" class="px-4 py-2 text-xs font-bold text-zinc-500 hover:text-zinc-800 transition-colors cursor-pointer">
            Wait for Schedule
        </button>
        <button onclick="triggerImmediateSync(this)" class="bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-bold px-4 py-2 rounded-lg transition-colors cursor-pointer shadow-xs">
            Confirm & Sync Now
        </button>
    </div>
</div>
...
<div id="cora-rag-confirm-drawer" class="fixed inset-0 z-[9999] flex items-center justify-center hidden">
    ...
    <div class="relative w-full max-w-sm mx-4 bg-white border border-zinc-200 rounded-2xl shadow-2xl overflow-hidden p-5 animate-in fade-in zoom-in-95 duration-150">
        <div class="flex items-start gap-3 mb-4">
            <div class="w-9 h-9 rounded-xl flex items-center justify-center shrink-0 bg-red-50 text-red-600 ">
            ...
            <div>
                <h4 class="text-sm font-bold text-zinc-900 " id="rag-confirm-title">Remove Resource?</h4>
                <p class="text-xs text-zinc-500 mt-1 leading-relaxed" id="rag-confirm-message">
                    Are you sure you want to permanently remove this resource? This action cannot be undone.
                </p>
            </div>
        </div>
        <div class="flex items-center gap-2.5">
            <button onclick="closeRagConfirm()" class="flex-1 py-2 text-xs font-semibold text-zinc-700 bg-zinc-100 hover:bg-zinc-200 rounded-xl cursor-pointer">
                Cancel
            </button>
    ...
    jQuery.post(coraREData.ajaxUrl, {
        ...
        if (res.success) {
            ...
            data.categories.forEach(cat => {
                const badgeClass = cat.changes > 0 ? 'bg-green-50 text-green-700 ' : 'bg-zinc-50 text-zinc-400';
                html += `
                <div class="p-3.5 border border-zinc-150 rounded-xl flex items-center justify-between">
                    <div>
                        <span class="text-xs font-bold text-zinc-850 ">${cat.name}</span>

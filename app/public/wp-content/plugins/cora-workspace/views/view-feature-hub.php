...
$cora_industry = cora_get_active_industry();

if ( $cora_industry === 'custom' ) {
    ...
    $features_list = array(
        'Workspace' => array(
            ...
            'financials' => array(
                ...
                'desc'  => 'Track agency transactions, payout preferences, invoicing, and GST breakdowns.',
            ...
            'team-roles' => array(
                'title' => 'User & Roles',
                'desc'  => 'Configure workspace roles, security policies, and user permissions matrix.',
            ...
            'media' => array(
                'title' => 'Media Manager',
                'desc'  => 'Manage images, videos, and galleries with advanced metadata and filtering.',
            ...
            'vault' => array(
                'title' => 'File Manager',
                'desc'  => 'Central secure document storage vault with E-Sign contracts registry.',
        ...
        'Operations' => array(
            ...
            'crew_scheduler' => array(
                ...
                'desc'  => 'Manage team shifts, availability grids, and dispatch assignees to shoots or viewings.',
                'icon'  => '<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>'
            ),
            'equipment' => array(
                'title' => 'Camera Equipment',
                'desc'  => 'Track camera bodies, lenses, lighting gear, custody logs, and status audits.',
            ...
            'showings' => array(
                ...
                'desc'  => 'Schedule and manage buyer property showings, agent assignments, and customer feedback.',
                'icon'  => '<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"></rect><line x1="16" y1="2" x2="16" y2="6"></line><line x1="8" y1="2" x2="8" y2="6"></line><line x1="3" y1="10" x2="21" y2="10"></line></svg>'
            ),
            'properties' => array(
                'title' => 'Property Listings',
                'desc'  => 'List buildings, office locations, plot configurations, and geocoded coordinates.',
            ...
            <form id="cora-custom-features-form" onsubmit="event.preventDefault();" style="display: flex; flex-direction: column; gap: 32px;">
                <?php foreach ( $features_list as $category => $items ) : ?>
                    <div style="display: flex; flex-direction: column; gap: 16px;">
                        <h3 style="font-size: 13px; font-weight: 750; color: #71717a; text-transform: uppercase; letter-spacing: 0.05em; margin: 0; padding-bottom: 6px; border-bottom: 1px solid #f4f4f5;"><?php echo esc_html( $category ); ?></h3>
                        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 16px;">
                            <?php foreach ( $items as $slug => $data ) :
                                $is_active = in_array( $slug, $enabled, true );
                            ?>
                                <div style="background: #ffffff; border: 1px solid #e4e4e7; border-radius: 14px; padding: 18px; display: flex; align-items: start; gap: 14px; transition: all 0.2s ease-in-out; position: relative;">
                                    <div style="width: 32px; height: 32px; border-radius: 8px; background: #f4f4f5; display: flex; align-items: center; justify-content: center; color: #18181b; shrink-to-fit: 0; flex-shrink: 0;">
                                        <?php echo $data['icon']; ?>
                                    </div>
                                    <div style="flex-grow: 1; min-width: 0; display: flex; flex-direction: column; gap: 4px;">
                                        <div style="font-size: 13.5px; font-weight: 700; color: #09090b;"><?php echo esc_html( $data['title'] ); ?></div>
                                        <div style="font-size: 11.5px; color: #71717a; line-height: 1.4;"><?php echo esc_html( $data['desc'] ); ?></div>
                                    </div>
                                    <div style="flex-shrink: 0; align-self: center;">
                                        <label class="cora-switch-label" style="position: relative; display: inline-block; width: 36px; height: 20px;">
                                            ...
                                            <span style="position: absolute; cursor: pointer; top: 0; left: 0; right: 0; bottom: 0; background-color: #e4e4e7; transition: .3s; border-radius: 9999px;"></span>
                                        </label>
                                    </div>
                                </div>
                            <?php endforeach; ?>
    ...
    <style>
    /* Premium Monochromatic Switch Animations */
        ...
        window.coraToggleCustomFeature = function(checkbox) {
            ...
            $.post(coraREData.ajaxUrl, {
                ...
                if (response && response.success) {
                    if (typeof window.coraShowToast === 'function') {
                        window.coraShowToast("Modules updated successfully! Reloading...");
                    }
                    setTimeout(function() {
...
<div class="cora-fh-container">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 pt-5" id="cora-modules-grid">
            ...
            <div class="cora-module-item-card bg-zinc-50/50 hover:bg-zinc-100/60 border border-zinc-200/60 rounded-xl p-4 transition-all" data-search="cora real estate suite v1.4.2 mobile bottom navigation irn rupee symbol">
                ...
                <div class="flex items-center justify-between text-[10px] text-zinc-400 font-medium">
                    ...
                    <span class="font-mono text-zinc-400">cora-real-estate.php</span>
                </div>
            </div>
            ...
            <div class="cora-module-item-card bg-zinc-50/50 hover:bg-zinc-100/60 border border-zinc-200/60 rounded-xl p-4 transition-all" data-search="cora workspace core v2.1.0 command palette search overflow desktop header">
                ...
                <div class="flex items-center justify-between text-[10px] text-zinc-400 font-medium">
                    ...
                    <span class="font-mono text-zinc-400">cora-workspace.php</span>
                </div>
            </div>
            ...
            <div class="cora-module-item-card bg-zinc-50/50 hover:bg-zinc-100/60 border border-zinc-200/60 rounded-xl p-4 transition-all" data-search="cora studio ai engine v1.8.0 claude minimalist aesthetic b-roll toast">
                ...
                <div class="flex items-center justify-between text-[10px] text-zinc-400 font-medium">
                    ...
                    <span class="font-mono text-zinc-400">cora-studio-ai.php</span>
                </div>
            </div>
            ...
            <div class="cora-module-item-card bg-zinc-50/50 hover:bg-zinc-100/60 border border-zinc-200/60 rounded-xl p-4 transition-all" data-search="cora crm lead automation v1.3.5 bookings contact drawer shoot pipeline">
                ...
                <div class="flex items-center justify-between text-[10px] text-zinc-400 font-medium">
                    ...
                    <span class="font-mono text-zinc-400">class-cora-crm.php</span>
                </div>
            </div>
            ...
            <div class="cora-module-item-card bg-zinc-50/50 hover:bg-zinc-100/60 border border-zinc-200/60 rounded-xl p-4 transition-all" data-search="cora financials billing v1.2.0 saas calculator pdf invoice inr rupee">
                ...
                <div class="flex items-center justify-between text-[10px] text-zinc-400 font-medium">
                    ...
                    <span class="font-mono text-zinc-400">class-cora-finance.php</span>
                </div>
            </div>
            ...
            <div class="cora-module-item-card bg-zinc-50/50 hover:bg-zinc-100/60 border border-zinc-200/60 rounded-xl p-4 transition-all" data-search="model context protocol mcp v1.1.0 rest api tools subagents">
                ...
                <div class="flex items-center justify-between text-[10px] text-zinc-400 font-medium">
                    ...
                    <span class="font-mono text-zinc-400">class-cora-mcp.php</span>
                </div>
            </div>
            ...
            <div class="cora-module-item-card bg-zinc-50/50 hover:bg-zinc-100/60 border border-zinc-200/60 rounded-xl p-4 transition-all" data-search="git lovable sync hub v1.0.4 github repo branch live url auto save">
                ...
                <div class="flex items-center justify-between text-[10px] text-zinc-400 font-medium">
                    ...
                    <span class="font-mono text-zinc-400">view-settings-suite.php</span>
                </div>
            </div>
    ...
    <script>
    function coraFilterModules(query) {
        const q = (query || '').toLowerCase().trim();
        const cards = document.querySelectorAll('#cora-modules-grid .cora-module-item-card');
        cards.forEach(card => {

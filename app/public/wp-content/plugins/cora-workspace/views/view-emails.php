...
?>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-6 shrink-0">
        ...
        <div class="bg-white border border-zinc-200 rounded-2xl p-4 shadow-3xs flex items-center justify-between">
            <div>
                <span class="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Total Emails Sent</span>
                <div class="text-xl font-extrabold text-zinc-900 mt-1" id="stat-total-sent">--</div>
                <span class="text-[10px] text-zinc-500 font-medium" id="stat-month-sent">-- sent this month</span>
            </div>
            <div class="w-10 h-10 rounded-xl bg-zinc-100 flex items-center justify-center text-zinc-700 shrink-0">
        ...
        <div class="bg-white border border-zinc-200 rounded-2xl p-4 shadow-3xs flex items-center justify-between">
            <div>
                <span class="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Delivery Success</span>
                <div class="text-xl font-extrabold text-zinc-900 mt-1" id="stat-success-rate">99.4%</div>
                <span class="text-[10px] text-emerald-600 font-semibold">Hostinger Relay Active</span>
            </div>
            <div class="w-10 h-10 rounded-xl bg-emerald-50 border border-emerald-200/50 flex items-center justify-center text-emerald-600 shrink-0">
        ...
        <div class="bg-white border border-zinc-200 rounded-2xl p-4 shadow-3xs flex items-center justify-between cursor-pointer hover:border-zinc-300 transition-all" onclick="coraSwitchEmailSubTab('email-tab-templates')">
            <div>
                <span class="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">Active Templates</span>
                <div class="text-xl font-extrabold text-zinc-900 mt-1" id="stat-active-templates">6 Active</div>
                <span class="text-[10px] text-zinc-500 font-medium">System & Custom Presets</span>
            </div>
            <div class="w-10 h-10 rounded-xl bg-zinc-100 flex items-center justify-center text-zinc-700 shrink-0">
        ...
        <div class="bg-white border border-zinc-200 rounded-2xl p-4 shadow-3xs flex items-center justify-between cursor-pointer hover:border-zinc-300 transition-all" onclick="coraSwitchEmailSubTab('email-tab-settings')">
            <div>
                <span class="text-[10px] font-bold tracking-wider text-zinc-400 uppercase">SMTP Provider</span>
                <div class="text-sm font-bold text-zinc-900 mt-1 truncate" id="stat-smtp-provider">Hostinger Business</div>
                <span class="text-[10px] text-zinc-500 font-mono">Port 587 / TLS SSL</span>
            </div>
            <div class="w-10 h-10 rounded-xl bg-zinc-100 flex items-center justify-center text-zinc-700 shrink-0">
    ...
    <div class="flex items-center gap-1 border-b border-zinc-200 mt-6 shrink-0 overflow-x-auto">
        <button id="tab-btn-email-tab-compose" onclick="coraSwitchEmailSubTab('email-tab-compose')"
                class="cora-email-sub-tab px-4 py-2.5 text-xs font-bold text-zinc-900 border-b-2 border-zinc-950 transition-all whitespace-nowrap cursor-pointer">
            Compose & Send
        </button>
        <button id="tab-btn-email-tab-templates" onclick="coraSwitchEmailSubTab('email-tab-templates')"
                class="cora-email-sub-tab px-4 py-2.5 text-xs font-semibold text-zinc-500 hover:text-zinc-900 border-b-2 border-transparent transition-all whitespace-nowrap cursor-pointer">
            Email Templates
        </button>
    ...
    <div class="flex-1 mt-6 min-h-0 relative">
        ...
        <div id="email-tab-compose" class="cora-email-tab-content flex-1 grid grid-cols-1 lg:grid-cols-12 gap-6 h-full min-h-0">
            ...
            <div class="lg:col-span-7 flex flex-col bg-white border border-zinc-200 rounded-2xl p-5 shadow-xs overflow-y-auto">
                <div class="flex items-center justify-between pb-3 border-b border-zinc-100 ">
                    ...
                    <button type="button" onclick="resetEmailComposeForm()" class="text-[11px] font-medium text-zinc-400 hover:text-zinc-600 bg-transparent border-0 cursor-pointer">Clear Form</button>
                </div>
                ...
                <form id="cora-email-compose-form" class="space-y-4 mt-4 flex-1 flex flex-col">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Select CRM Lead / Client</label>
                            <select id="compose-recipient-picker" onchange="onRecipientPickerChange(this)"
                                    class="w-full h-10 px-3 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-900 outline-hidden focus:border-zinc-400 transition-all shadow-3xs cursor-pointer">
                                <option value="">-- Choose from Leads / Clients --</option>
                            </select>
                        </div>
                        <div class="space-y-1">
                            <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Or Manual Recipient Email *</label>
                            <input type="email" id="email-to" placeholder="client@example.com" required
                                   class="w-full h-10 px-3.5 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-900 outline-hidden focus:border-zinc-400 transition-all shadow-3xs" />
                        </div>
                    </div>

                    <!-- Template Preset Selector -->
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Load Saved Template Preset</label>
                        <select id="compose-template-selector" onchange="loadTemplateIntoComposer(this.value)"
                                class="w-full h-10 px-3 rounded-xl border border-zinc-200 bg-zinc-50 text-xs font-semibold text-zinc-800 outline-hidden focus:border-zinc-400 transition-all shadow-3xs cursor-pointer">
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1.5 flex-wrap pt-0.5">
                            <button type="button" onclick="insertVariableTag('{client_name}')" class="px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-mono font-semibold text-zinc-700 border-0 cursor-pointer transition-all">{client_name}</button>
                            <button type="button" onclick="insertVariableTag('{event_name}')" class="px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-mono font-semibold text-zinc-700 border-0 cursor-pointer transition-all">{event_name}</button>
                            <button type="button" onclick="insertVariableTag('{event_date}')" class="px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-mono font-semibold text-zinc-700 border-0 cursor-pointer transition-all">{event_date}</button>
                            <button type="button" onclick="insertVariableTag('{portal_url}')" class="px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-mono font-semibold text-zinc-700 border-0 cursor-pointer transition-all">{portal_url}</button>
                            <button type="button" onclick="insertVariableTag('{invoice_amount}')" class="px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-mono font-semibold text-zinc-700 border-0 cursor-pointer transition-all">{invoice_amount}</button>
                            <button type="button" onclick="insertVariableTag('{studio_name}')" class="px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-mono font-semibold text-zinc-700 border-0 cursor-pointer transition-all">{studio_name}</button>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1 flex-1 flex flex-col min-h-48">
                        ...
                        <textarea id="email-message" placeholder="Write your professional email message here..." required oninput="updateLivePreview()"
                                  class="w-full flex-1 min-h-52 p-4 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-900 outline-hidden focus:border-zinc-400 transition-all resize-none shadow-3xs leading-relaxed"></textarea>
                    </div>

                    <!-- Attachment Link Simulator -->
                    <div class="flex items-center justify-between p-3 rounded-xl bg-zinc-50 border border-zinc-200 shrink-0">
                        ...
                        <button type="button" onclick="attachVaultDocumentSim()" class="text-[11px] font-bold text-zinc-900 hover:underline bg-transparent border-0 cursor-pointer">
                            + Select Vault Asset
                        </button>
                    </div>
                    ...
                    <div class="flex items-center justify-between pt-2 shrink-0">
                        ...
                        <button type="submit" id="btn-send-email"
                                class="h-10 px-6 rounded-xl bg-zinc-950 hover:bg-zinc-900 text-white text-xs font-bold flex items-center gap-2 cursor-pointer transition-all border-none shadow-xs">
            ...
            <div class="lg:col-span-5 flex flex-col bg-white border border-zinc-200 rounded-2xl p-5 shadow-xs overflow-y-auto">
                <div class="flex items-center justify-between pb-3 border-b border-zinc-100 ">
                    <h3 class="text-sm font-bold text-zinc-900 flex items-center gap-2">
                ...
                <div class="mt-4 p-5 rounded-2xl bg-zinc-50 border border-zinc-200/80 flex-1 flex flex-col font-sans">
                    <!-- Simulated Email Header -->
                    <div class="border-b border-zinc-200/80 pb-3 mb-4 space-y-1.5">
                        <div class="flex items-center justify-between text-[11px]">
                            <span class="text-zinc-400 font-bold uppercase">To:</span>
                            <span class="font-mono text-zinc-800 font-semibold" id="preview-to">client@example.com</span>
                        </div>
                        <div class="flex items-center justify-between text-[11px]">
                            <span class="text-zinc-400 font-bold uppercase">From:</span>
                            <span class="font-mono text-zinc-500 " id="preview-from"><?php echo esc_html(get_option('admin_email')); ?></span>
                        </div>
                        <div class="flex items-start justify-between text-[11px] pt-1">
                            ...
                            <span class="font-bold text-zinc-900 text-right truncate pl-2" id="preview-subject">(No Subject Specified)</span>
                        </div>
                    </div>

                    <!-- Email Body Container Card -->
                    <div class="flex-1 bg-white rounded-xl p-5 border border-zinc-200/60 shadow-3xs flex flex-col justify-between">
                        <div id="preview-body" class="text-xs text-zinc-800 leading-relaxed space-y-3 whitespace-pre-wrap font-medium">
                            <span class="text-zinc-400 italic text-[11px]">Message body preview will render here in real-time as you type...</span>
                        </div>

                        <div class="mt-6 pt-4 border-t border-zinc-100 text-center text-[10px] text-zinc-400 font-sans">
        ...
        <div id="email-tab-templates" class="cora-email-tab-content hidden flex-1 flex-col space-y-5">
            <!-- Templates Toolbar -->
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-zinc-200 rounded-2xl p-4 shadow-3xs">
                <div class="flex items-center gap-3 flex-1">
                    <input type="text" id="template-search-input" placeholder="Search templates..." oninput="renderEmailTemplates()"
                           class="h-9 px-3.5 rounded-xl border border-zinc-200 bg-white text-xs text-zinc-900 outline-hidden w-full sm:w-64" />
                    <select id="template-category-filter" onchange="renderEmailTemplates()"
                            class="h-9 px-3 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-800 outline-hidden cursor-pointer">
                ...
                <button onclick="openEmailTemplateDrawer()" 
                        class="h-9 px-4 rounded-xl bg-zinc-950 hover:bg-zinc-900 text-white text-xs font-bold flex items-center gap-2 border-0 cursor-pointer shadow-xs shrink-0">
        ...
        <div id="email-tab-sequences" class="cora-email-tab-content hidden flex-1 flex-col space-y-6">
            <div class="bg-white border border-zinc-200 rounded-2xl p-5 shadow-xs">
                <div class="flex items-center justify-between pb-4 border-b border-zinc-100 ">
                    <div>
                        <h3 class="text-sm font-bold text-zinc-900 ">Automated Client Drip Sequences</h3>
                        <p class="text-xs text-zinc-500 mt-0.5">Automate timely follow-ups, proofing notifications, and review acquisition drips.</p>
                    </div>
                    <span class="px-3 py-1 rounded-full bg-emerald-50 text-[10px] font-bold text-emerald-700 border border-emerald-200 ">3 Workflows Active</span>
                </div>

                <div class="space-y-6 mt-5">
                    <!-- Sequence 1: Lead Inquiry Follow-up -->
                    <div class="p-4 rounded-xl border border-zinc-200/80 bg-zinc-50/50 space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-4 gap-2 pt-2">
                            <div class="p-2.5 bg-white rounded-lg border border-zinc-200/60 text-[11px]">
                    ...
                    <div class="p-4 rounded-xl border border-zinc-200/80 bg-zinc-50/50 space-y-3">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-2">
                            ...
                            <div class="p-2.5 bg-white rounded-lg border border-zinc-200/60 text-[11px]">
                                ...
                                <span class="font-semibold text-zinc-900 truncate block mt-0.5">Google Business Review Request</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white border border-zinc-200 rounded-2xl p-4 shadow-3xs">
                <div class="flex items-center gap-3 flex-1">
                    <input type="text" id="outbox-search-input" placeholder="Search by recipient or subject..." oninput="filterOutboxLogs()"
                           class="h-9 px-3.5 rounded-xl border border-zinc-200 bg-white text-xs text-zinc-900 outline-hidden w-full sm:w-72" />
                ...
                <button onclick="exportOutboxLogsCSV()" class="h-9 px-3.5 rounded-xl border border-zinc-200 bg-white hover:bg-zinc-50 text-zinc-700 text-xs font-semibold flex items-center gap-1.5 cursor-pointer">
        ...
        <div id="email-tab-settings" class="cora-email-tab-content hidden flex-1 flex-col space-y-6">
            <div class="max-w-2xl bg-white border border-zinc-200 rounded-2xl p-6 shadow-xs">
                <div class="border-b border-zinc-100 pb-4 mb-5 flex items-center justify-between">
                    <div>
                        <h3 class="text-sm font-bold text-zinc-900 flex items-center gap-2">
                        ...
                        <p class="text-xs text-zinc-500 mt-1">Configure your official Hostinger SMTP mail relay details for guaranteed inbox deliverability.</p>
                    </div>
                    <button type="button" id="smtp-lock-badge" onclick="coraToggleSMTPLock()" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-zinc-100 border border-zinc-200 text-zinc-700 text-xs font-semibold shrink-0 cursor-pointer">
                        <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2" fill="none" class="text-emerald-600 "><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg>
                        System Locked
                    </button>
                </div>
                ...
                <form id="smtp-settings-form" class="space-y-4" onsubmit="event.preventDefault();">
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <input type="text" id="smtp-host" value="<?php echo esc_attr(get_option('cora_smtp_host', 'smtp.hostinger.com')); ?>" readonly
                                   class="w-full h-10 px-3.5 rounded-xl border border-zinc-200 bg-zinc-50 text-xs font-semibold text-zinc-500 cursor-not-allowed outline-hidden" />
                        </div>
                        <div class="space-y-1">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div class="space-y-1">
                            ...
                            <select id="smtp-secure" disabled
                                    class="w-full h-10 px-3 rounded-xl border border-zinc-200 bg-zinc-50 text-xs font-semibold text-zinc-500 cursor-not-allowed outline-hidden">
                    ...
                    <div class="flex items-center justify-between pt-4 border-t border-zinc-100 ">
                        <button type="button" onclick="openSmtpTestDrawer()" class="h-10 px-4 rounded-xl border border-zinc-200 text-xs font-semibold text-zinc-700 hover:bg-zinc-50 cursor-pointer flex items-center gap-2">
                        ...
                        <button type="button" id="smtp-save-btn" onclick="coraSaveSMTPSettings()" class="hidden h-10 px-4 rounded-xl bg-zinc-900 text-white text-xs font-semibold hover:bg-zinc-800 cursor-pointer flex items-center gap-2">
                            Save Settings
                        </button>
                    </div>
                </form>
...
<div id="cora-email-template-drawer" class="fixed inset-y-0 right-0 w-full sm:max-w-lg bg-white border-l border-zinc-200 shadow-2xl z-[150] flex flex-col translate-x-full transition-transform duration-300 ease-in-out">
    <div class="flex items-center justify-between p-5 border-b border-zinc-200 shrink-0">
        <h3 class="text-xs font-bold text-zinc-900 uppercase tracking-wider" id="template-drawer-title">Create Email Template</h3>
        <button onclick="closeEmailTemplateDrawer()" class="text-zinc-400 hover:text-zinc-700 border-0 bg-transparent cursor-pointer text-sm font-bold">✕ Close</button>
    </div>
    ...
    <form id="template-editor-form" onsubmit="saveEmailTemplate(event)" class="flex-1 flex flex-col overflow-y-auto p-6 space-y-4">
        ...
        <div class="space-y-1">
            ...
            <input type="text" id="tpl-editor-name" placeholder="e.g. Shoot Confirmation" required
                   class="w-full h-10 px-3.5 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-900 outline-hidden" />
        </div>

        <div class="space-y-1">
            <label class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">Category</label>
            <select id="tpl-editor-category" class="w-full h-10 px-3 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-900 outline-hidden">
                <option value="Bookings">Bookings</option>
                <option value="Financials">Financials</option>
                <option value="Media & Vault">Media & Vault</option>
                <option value="Leads">Leads</option>
        ...
        <div class="space-y-1 flex-1 flex flex-col min-h-48">
            ...
            <textarea id="tpl-editor-body" placeholder="Write template body with {client_name}, {event_name}, {portal_url}..." required
                      class="w-full flex-1 p-4 rounded-xl border border-zinc-200 bg-white text-xs font-semibold text-zinc-900 outline-hidden leading-relaxed resize-none"></textarea>
        </div>

        <div class="pt-3 border-t border-zinc-200 flex items-center justify-end gap-2 shrink-0">
            <button type="button" onclick="closeEmailTemplateDrawer()" class="h-9 px-4 rounded-xl border border-zinc-200 text-xs font-semibold text-zinc-600 bg-transparent cursor-pointer">Cancel</button>
            <button type="submit" class="h-9 px-5 rounded-xl bg-zinc-950 hover:bg-zinc-900 text-white text-xs font-bold border-0 cursor-pointer shadow-xs">Save Template</button>
        </div>
    </form>
</div>

<!-- 2. SENT EMAIL DETAIL DRAWER -->
<div id="cora-email-detail-drawer" class="fixed inset-y-0 right-0 w-full sm:max-w-md bg-white border-l border-zinc-200 shadow-2xl z-[150] flex flex-col translate-x-full transition-transform duration-300 ease-in-out">
    <div class="flex items-center justify-between p-5 border-b border-zinc-200 shrink-0">
        <h3 class="text-xs font-bold text-zinc-400 uppercase tracking-wider">Sent Communication Details</h3>
        <button onclick="closeEmailDetailDrawer()" class="text-zinc-400 hover:text-zinc-700 border-0 bg-transparent cursor-pointer text-sm font-bold">✕ Close</button>
    </div>
    <div class="flex-1 overflow-y-auto p-5 space-y-4">
        <div class="space-y-1">
            <div class="text-[10px] text-zinc-400 font-bold uppercase">To Recipient</div>
            <div id="drawer-email-to" class="text-xs font-semibold text-zinc-900 "></div>
        </div>
        <div class="space-y-1">
        ...
        <div class="space-y-1">
            ...
            <div id="drawer-email-message" class="text-xs text-zinc-800 bg-zinc-50 p-4 rounded-xl whitespace-pre-wrap leading-relaxed border border-zinc-200 font-medium"></div>
        </div>
        <div class="pt-4 border-t border-zinc-100 ">
            <button type="button" id="btn-resend-drawer-email" onclick="resendDrawerEmail()"
                    class="w-full h-10 rounded-xl bg-zinc-950 hover:bg-zinc-900 text-white text-xs font-bold flex items-center justify-center gap-2 cursor-pointer border-0 shadow-xs">
...
<div id="cora-smtp-test-drawer" class="fixed inset-y-0 right-0 w-full sm:max-w-md bg-white border-l border-zinc-200 shadow-2xl z-[150] flex flex-col translate-x-full transition-transform duration-300 ease-in-out">
    <div class="flex items-center justify-between p-5 border-b border-zinc-200 shrink-0">
        <h3 class="text-xs font-bold text-zinc-400 uppercase tracking-wider">Hostinger SMTP Relay Diagnostic</h3>
        <button onclick="closeSmtpTestDrawer()" class="text-zinc-400 hover:text-zinc-700 border-0 bg-transparent cursor-pointer text-sm font-bold">✕ Close</button>
    </div>
    
    <div class="flex-1 overflow-y-auto p-5 space-y-4">
        <p class="text-xs text-zinc-500 ">Send an instant test email to verify host connection, port 587 handshakes, and SSL/TLS authentication status.</p>
...
jQuery(document).ready(function($) {
    ...
    window.renderEmailTemplates = function() {
        ...
        filtered.forEach(tpl => {
            const card = $(`
                <div class="bg-white border border-zinc-200 rounded-2xl p-5 shadow-3xs flex flex-col justify-between space-y-4 hover:border-zinc-300 transition-all">
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            <span class="px-2.5 py-0.5 rounded-full bg-zinc-100 text-[10px] font-bold text-zinc-700 ">${escapeHtml(tpl.category)}</span>
                            ${tpl.is_system ? '<span class="text-[9px] font-mono text-zinc-400">System Preset</span>' : '<span class="text-[9px] font-mono text-emerald-600 ">Custom Template</span>'}
                        </div>
                        <h4 class="text-xs font-bold text-zinc-900 truncate">${escapeHtml(tpl.name)}</h4>
                        <p class="text-[11px] text-zinc-500 font-semibold truncate">Subject: ${escapeHtml(tpl.subject)}</p>
                    ...
                    <div class="pt-3 border-t border-zinc-100 flex items-center justify-between">
                        ...
                        <div class="flex items-center gap-1.5">
                            <button onclick="editEmailTemplate('${tpl.id}')" class="px-2 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-semibold text-zinc-700 border-0 cursor-pointer">Edit</button>
                            ${!tpl.is_system ? `<button onclick="deleteEmailTemplate('${tpl.id}')" class="px-2 py-1 rounded-lg bg-red-50 hover:bg-red-100 text-[10px] font-semibold text-red-600 border-0 cursor-pointer">Delete</button>` : ''}
                        </div>
                    </div>
                </div>
            `);
        ...
        logs.forEach((log, idx) => {
            ...
            const row = $(`
                <tr class="hover:bg-zinc-50/70 transition-all cursor-pointer">
                    <td class="py-3 px-4 font-bold text-zinc-900 ">${escapeHtml(log.to)}</td>
                    <td class="py-3 px-4 font-medium text-zinc-700 max-w-xs truncate">${escapeHtml(log.subject)}</td>
                    ...
                    <td class="py-3 px-4 text-right">
                        <button onclick='openEmailDetailDrawer(${JSON.stringify(log)})' class="px-2.5 py-1 rounded-lg bg-zinc-100 hover:bg-zinc-200 text-[10px] font-semibold text-zinc-800 border-0 cursor-pointer">
                            View Details
                        </button>
                    </td>
                </tr>
...
window.coraToggleSMTPLock = function() {
    ...
    if (_smtpUnlocked) {
        ...
        $badge.html(`
            <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2" fill="none" class="text-amber-500 "><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 5-5 5 5 0 0 1 5 5"></path></svg>
            Editing

...
<body>
    ...
    $is_driver_invite = ( ( $invite['role'] ?? '' ) === 'cora_field_vendor' );
    $name = '';
    if ( $invite ) {
    ...
    ?>
    <div id="setup-card">
        <?php if ( $is_driver_invite ) : ?>
            <div style="display:inline-flex; align-items:center; gap:6px; padding:4px 10px; background:#f4f4f5; border:1px solid #e4e4e7; border-radius:99px; font-size:11px; font-weight:700; color:#18181b; margin-bottom:14px;">
                <span>🚐</span> Field Sales Driver
            </div>
            <h2>Activate Van Terminal</h2>
            <p class="subtitle">Quickly activate your terminal to access your route consignments and POS sales.</p>
        <?php else : ?>
        ...
        <div style="margin-bottom: 20px;">
            <a href="<?php echo esc_url( home_url( '/workspace/auth/google?token=' . urlencode( $token ) ) ); ?>" style="display: flex; align-items: center; justify-content: center; gap: 10px; width: 100%; padding: 11px 14px; background: #ffffff; border: 1px solid var(--border-color); border-radius: 12px; font-size: 13px; font-weight: 700; color: var(--text-primary); text-decoration: none; box-sizing: border-box; transition: background 0.15s, border-color 0.15s; box-shadow: 0 1px 2px rgba(0,0,0,0.04);">
                ...
                <span>1-Tap Sign up with Google</span>
            </a>
            <div style="display: flex; align-items: center; gap: 12px; margin: 18px 0 6px;">
                <div style="flex: 1; height: 1px; background: var(--border-color);"></div>
                <span style="font-size: 10px; font-weight: 700; color: var(--text-tertiary); text-transform: uppercase; letter-spacing: 0.05em;">Or set password</span>
                <div style="flex: 1; height: 1px; background: var(--border-color);"></div>
            </div>
        </div>
        ...
        <form id="setup-form" onsubmit="handleSetupSubmit(event)">
            ...
            <button type="submit" class="submit-btn" id="submit-btn">
                <?php echo $is_driver_invite ? 'Activate Terminal' : 'Create Account'; ?>

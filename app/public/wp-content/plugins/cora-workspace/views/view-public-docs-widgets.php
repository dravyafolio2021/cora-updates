...
<aside class="w-72 shrink-0 sticky top-8 space-y-6 hidden lg:block" id="cora-docs-widgets-column">
    ...
    <div class="space-y-3">
        <h4 class="text-[10px] font-bold text-zinc-400 dark:text-zinc-550 uppercase tracking-wider">
            On This Page
        </h4>
        <div class="relative pl-4 border-l border-zinc-200 dark:border-zinc-800">
            <!-- Active indicator vertical guide dot container -->
            <ul id="toc-list" class="space-y-2.5 py-0.5">
                <!-- Dynamically populated via JS -->
                <li class="text-xs text-zinc-400 dark:text-zinc-500 italic">
    ...
    <div class="border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-950 rounded-xl p-4 shadow-sm flex flex-col space-y-3.5">
        <div class="flex items-center justify-between pb-2 border-b border-zinc-100 dark:border-zinc-900">
            <div class="flex items-center gap-2">
                <span class="p-1.5 bg-zinc-100 dark:bg-zinc-900 rounded-md text-zinc-850 dark:text-zinc-200">
                ...
                <span class="text-xs font-bold text-zinc-850 dark:text-zinc-200">Ask Cora AI</span>
            </div>
            <!-- Online Indicator -->
            <span class="flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-medium bg-green-50 dark:bg-green-950/20 text-green-700 dark:text-green-455 border border-green-200/30">
        ...
        <div id="cora-chat-messages" class="max-h-48 overflow-y-auto space-y-3 pr-1 text-xs flex flex-col scrollbar-thin scrollbar-thumb-zinc-200">
            <div class="p-2.5 rounded-lg text-xs bg-zinc-100 text-zinc-800 dark:bg-zinc-900 dark:text-zinc-200 self-start max-w-[88%] leading-relaxed">
        ...
        <div class="relative flex items-center shrink-0">
            <input type="text" id="cora-chat-input" onkeydown="handleCoraChatKeyDown(event)" class="w-full border border-zinc-200 dark:border-zinc-800 rounded-lg pl-3 pr-8 py-2 text-xs bg-zinc-50 dark:bg-zinc-900 focus:border-zinc-400 dark:focus:border-zinc-700 outline-none text-zinc-900 dark:text-zinc-100 transition-all font-sans" placeholder="Ask anything about Cora...">
            <button onclick="submitCoraChatQuery()" id="cora-chat-send-btn" class="absolute right-2.5 text-zinc-400 hover:text-zinc-900 dark:hover:text-zinc-200 transition-colors p-1 cursor-pointer" title="Send Question">
    ...
    <div class="space-y-3">
        ...
        <div class="space-y-2">
            <?php foreach ( $related_docs as $doc ) : ?>
                <a href="<?php echo esc_url( home_url( '/docs/' . $doc['slug'] ) ); ?>" class="flex items-center gap-2.5 p-2.5 border border-zinc-200/50 dark:border-zinc-800/80 hover:border-zinc-400 dark:hover:border-zinc-700 bg-white dark:bg-zinc-950 hover:bg-zinc-50 dark:hover:bg-zinc-900 rounded-lg transition-all group shadow-xs">
                    <span class="text-zinc-400 group-hover:text-zinc-850 dark:group-hover:text-zinc-200 transition-colors shrink-0">
                    ...
                    <span class="text-xs text-zinc-600 dark:text-zinc-300 font-medium truncate group-hover:text-zinc-900 dark:group-hover:text-white transition-colors">
    ...
    <div class="border-t border-zinc-200/60 dark:border-zinc-850 pt-5 space-y-3">
        <div class="flex items-center justify-between">
            <span class="text-[10px] font-bold text-zinc-400 dark:text-zinc-550 uppercase tracking-wider">
                Was this helpful?
            </span>
            <div class="flex items-center gap-1.5">
                <button onclick="handleCoraVote(true, this)" class="p-1.5 border border-zinc-200 dark:border-zinc-800 text-zinc-550 hover:text-zinc-900 dark:hover:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-900 rounded-lg transition-all cursor-pointer shadow-xs" title="Yes, helpful">
                ...
                <button onclick="handleCoraVote(false, this)" class="p-1.5 border border-zinc-200 dark:border-zinc-800 text-zinc-550 hover:text-zinc-900 dark:hover:text-zinc-200 hover:bg-zinc-50 dark:hover:bg-zinc-900 rounded-lg transition-all cursor-pointer shadow-xs" title="No, not helpful">
...
(function() {
    ...
    window.coraDocsRefreshTOC = function() {
        ...
        if (!proseContent) {
            tocList.innerHTML = '<li class="text-xs text-zinc-400 dark:text-zinc-500 italic">No structure detected.</li>';
            return;
        }

        const headings = proseContent.querySelectorAll('h2, h3');
        if (headings.length === 0) {
            tocList.innerHTML = '<li class="text-xs text-zinc-400 dark:text-zinc-500 italic">No subheadings on this page.</li>';
            return;
        }
        ...
        headings.forEach((heading, idx) => {
            ...
            dot.className = `toc-dot absolute ${isH3 ? '-left-[4px]' : '-left-[17px]'} w-1.5 h-1.5 rounded-full bg-zinc-900 dark:bg-white scale-0 transition-all duration-200 ease-out`;

            const link = document.createElement('a');
            link.href = '#' + heading.id;
            link.className = `toc-link text-xs text-zinc-400 dark:text-zinc-505 hover:text-zinc-950 dark:hover:text-zinc-100 transition-colors duration-150 block py-0.5 truncate max-w-[240px] ${isH3 ? 'text-[11px]' : ''}`;
        ...
        function handleScrollSpy() {
            ...
            tocItems.forEach((item, index) => {
                if (index === activeIdx) {
                    item.linkEl.classList.remove('text-zinc-400', 'dark:text-zinc-505');
                    item.linkEl.classList.add('text-zinc-950', 'dark:text-zinc-50', 'font-semibold');
                    item.dotEl.classList.remove('scale-0');
                    item.dotEl.classList.add('scale-100');
                } else {
                    item.linkEl.classList.remove('text-zinc-950', 'dark:text-zinc-50', 'font-semibold');
                    item.linkEl.classList.add('text-zinc-400', 'dark:text-zinc-505');
    ...
    window.submitCoraChatQuery = function() {
        ...
        const thinkingBubble = appendCoraChatBubble('ai', `
            <div class="flex items-center gap-1.5 py-0.5" id="cora-chat-thinking">
                <span class="w-1.5 h-1.5 rounded-full bg-zinc-500 dark:bg-zinc-400 animate-bounce"></span>
                <span class="w-1.5 h-1.5 rounded-full bg-zinc-500 dark:bg-zinc-400 animate-bounce" style="animation-delay:0.2s"></span>
                <span class="w-1.5 h-1.5 rounded-full bg-zinc-500 dark:bg-zinc-400 animate-bounce" style="animation-delay:0.4s"></span>
            </div>
        `);
        ...
        .then(res => {
            ...
            if (res.success && res.data && res.data.reply) {
                ...
                appendCoraChatBubble('ai', `<span class="text-red-500 dark:text-red-400 font-semibold">${errMsg}</span>`);
            }
        })
        .catch(err => {
            thinkingBubble.remove();
            input.disabled = false;
            sendBtn.disabled = false;
            appendCoraChatBubble('ai', '<span class="text-red-500 dark:text-red-400 font-semibold">Network failure. Please try again.</span>');
        });
    };
    ...
    function appendCoraChatBubble(sender, content) {
        ...
        bubble.className = `p-2.5 rounded-lg text-xs max-w-[88%] leading-relaxed ${
            sender === 'user'
                ? 'bg-zinc-950 text-white dark:bg-white dark:text-zinc-950 self-end ml-auto shadow-sm'
                : 'bg-zinc-100 text-zinc-800 dark:bg-zinc-900 dark:text-zinc-200 self-start shadow-2xs'
        }`;
    ...
    function parseSimpleMarkdown(text) {
        return text
            ...
            .replace(/\*(.*?)\*/g, '<em>$1</em>')
            .replace(/`([^`]+)`/g, '<code class="bg-zinc-200/50 dark:bg-zinc-850 px-1 py-0.5 rounded font-mono text-[10px] text-zinc-900 dark:text-zinc-100">$1</code>')
            .replace(/\n- (.*?)\n/g, '<br>• $1')
            .replace(/\n- (.*?)$/g, '<br>• $1')
            .replace(/\n\* (.*?)\n/g, '<br>• $1')
            .replace(/\n\* (.*?)$/g, '<br>• $1')
    ...
    function streamCoraChatReply(fullText) {
        const messages = document.getElementById('cora-chat-messages');
        const bubble = document.createElement('div');
        bubble.className = 'p-2.5 rounded-lg text-xs max-w-[88%] leading-relaxed bg-zinc-100 text-zinc-800 dark:bg-zinc-900 dark:text-zinc-200 self-start shadow-2xs';
        messages.appendChild(bubble);
    ...
    window.handleCoraVote = function(wasUpvote, buttonEl) {
        ...
        buttons.forEach(btn => {
            btn.classList.remove('bg-zinc-900', 'text-white', 'dark:bg-white', 'dark:text-zinc-950');
            btn.classList.add('text-zinc-550', 'bg-transparent');
        });

        buttonEl.classList.remove('text-zinc-550', 'bg-transparent');
        buttonEl.classList.add('bg-zinc-900', 'text-white', 'dark:bg-white', 'dark:text-zinc-950');
    ...
    window.coraShowToast = window.coraShowToast || function(msg, type = 'success') {
        ...
        toast.className = 'flex items-center gap-2.5 px-4 py-3 bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-xs font-bold rounded-xl shadow-lg pointer-events-auto transition-all duration-300 transform translate-y-3 opacity-0 border border-zinc-800 dark:border-zinc-200';

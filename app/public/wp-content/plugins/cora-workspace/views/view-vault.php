...
?>
    ...
    <div id="cora-vault-view-dashboard" class="space-y-5 mt-0">
        ...
        <!-- ═════════════════════════════════════════════════════════════════════
             CORA AI VAULT & FILE MANAGER CO-PILOT AGENT HUB
             Empowers natural language document creation, GST math, e-sign & sharing
             ═════════════════════════════════════════════════════════════════════ -->
        <div id="cora-vault-ai-hub" class="bg-white border border-zinc-200/90 rounded-2xl p-4 sm:p-5 shadow-2xs transition-all flex flex-col gap-3.5">
            <!-- Header -->
                ...
                <div class="flex items-center gap-2.5">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h3 class="text-xs sm:text-sm font-extrabold text-zinc-950 tracking-tight m-0">AI Vault &amp; Document Agent</h3>
                            <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[9px] font-bold bg-zinc-100 text-zinc-800 border border-zinc-200 select-none">
                                <span class="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                                Active
                            </span>
                        </div>
                        <p class="text-[11px] text-zinc-500 m-0 hidden sm:block">Chat with your workspace agent to draft invoices with GST, assign contracts, dispatch e-sign, or audit receivables in real time.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex items-center gap-2 relative">
                <div class="relative flex-1">
                    <input type="text" id="cora-vault-ai-prompt" placeholder="Ask AI: e.g. Draft an invoice for Apex Realty of ₹1,50,000 for Commercial Shoot with 18% GST..." class="w-full pl-9 pr-3 py-2.5 bg-zinc-50/80 border border-zinc-200 rounded-xl text-xs outline-none focus:border-zinc-950 focus:bg-white transition-all font-medium text-zinc-900 shadow-3xs" onkeydown="if(event.key==='Enter') coraExecuteVaultAIPrompt();">
                    <svg viewBox="0 0 24 24" width="14" height="14" stroke="currentColor" stroke-width="2" fill="none" class="absolute left-3 top-3 text-zinc-400">
                        <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon>
                    </svg>
                </div>
                <button type="button" id="cora-vault-ai-submit-btn" onclick="coraExecuteVaultAIPrompt()" class="px-4 py-2.5 bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-bold rounded-xl transition-all shadow-3xs flex items-center gap-1.5 cursor-pointer shrink-0 border-none active:scale-[0.98]">
            ...
            <!-- Quick Action Chips -->
            <div class="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-none select-none">
                <button type="button" onclick="coraSetVaultAIPrompt('Draft an invoice for Apex Realty of ₹1,50,000 for Commercial Shoot with 18% GST')" class="px-2.5 py-1 rounded-lg bg-zinc-100/80 hover:bg-zinc-200/80 text-zinc-800 text-[10.5px] font-semibold transition-colors flex items-center gap-1 shrink-0 border border-zinc-200/60 cursor-pointer">
                    <span class="text-zinc-500">⚡</span> Draft Invoice (₹1.5L + GST)
                </button>
                <button type="button" onclick="coraSetVaultAIPrompt('Request E-Sign from Arjun & Priya for Wedding Coverage proposal')" class="px-2.5 py-1 rounded-lg bg-zinc-100/80 hover:bg-zinc-200/80 text-zinc-800 text-[10.5px] font-semibold transition-colors flex items-center gap-1 shrink-0 border border-zinc-200/60 cursor-pointer">
                    <span class="text-zinc-500">⚡</span> Request E-Sign (Arjun &amp; Priya)
                </button>
                <button type="button" onclick="coraSetVaultAIPrompt('Audit unpaid invoices older than 15 days and prepare follow-ups')" class="px-2.5 py-1 rounded-lg bg-zinc-100/80 hover:bg-zinc-200/80 text-zinc-800 text-[10.5px] font-semibold transition-colors flex items-center gap-1 shrink-0 border border-zinc-200/60 cursor-pointer">
                    <span class="text-zinc-500">⚡</span> Audit Unpaid Receivables
                </button>
                <button type="button" onclick="coraSetVaultAIPrompt('Assign Delhi Fashion Week 2026 Agreement to team member Kavya Patel')" class="px-2.5 py-1 rounded-lg bg-zinc-100/80 hover:bg-zinc-200/80 text-zinc-800 text-[10.5px] font-semibold transition-colors flex items-center gap-1 shrink-0 border border-zinc-200/60 cursor-pointer">
                    <span class="text-zinc-500">⚡</span> Assign Contract to Kavya
                </button>
            </div>

            <!-- Dynamic AI Action Proposal Result Box -->
            <div id="cora-vault-ai-proposal-box" class="hidden p-4 rounded-xl bg-zinc-50 border border-zinc-200 flex flex-col gap-3 animate-in fade-in duration-200">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex items-center gap-2">
                        <span class="w-2 h-2 rounded-full bg-emerald-500"></span>
                        <h4 id="cora-vault-prop-title" class="text-xs font-bold text-zinc-950 m-0">AI Action Proposal</h4>
                    </div>
                    <button type="button" onclick="$('#cora-vault-ai-proposal-box').addClass('hidden');" class="text-zinc-400 hover:text-zinc-700 p-1 border-none bg-transparent cursor-pointer">
                        <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.5" fill="none"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
                    </button>
                </div>
                <p id="cora-vault-prop-explanation" class="text-xs text-zinc-600 leading-relaxed m-0"></p>
                <div id="cora-vault-prop-breakdown" class="p-3 rounded-lg bg-white border border-zinc-200/90 text-xs flex flex-wrap items-center justify-between gap-2"></div>
                <div class="flex items-center gap-2 pt-1">
                    <button type="button" id="cora-vault-prop-exec-btn" onclick="coraConfirmVaultAIExecution()" class="px-3.5 py-2 bg-zinc-950 hover:bg-zinc-800 text-white text-xs font-bold rounded-lg transition-all shadow-3xs flex items-center gap-1.5 cursor-pointer border-none">
                        ...
                        <span>Execute &amp; Save to Vault</span>
                    </button>
                    <button type="button" onclick="$('#cora-vault-ai-proposal-box').addClass('hidden');" class="px-3 py-2 bg-white hover:bg-zinc-100 border border-zinc-200 text-zinc-700 text-xs font-semibold rounded-lg transition-colors cursor-pointer">
                        Dismiss
                    </button>
                </div>
            </div>
...
<script>
let currentVaultAIProposal = null;

function coraSetVaultAIPrompt(text) {
    const input = document.getElementById('cora-vault-ai-prompt');
    if (input) {
        input.value = text;
        input.focus();
    }
...
function coraExecuteVaultAIPrompt() {
    const input = document.getElementById('cora-vault-ai-prompt');
    if (!input || !input.value.trim()) return;
    const promptText = input.value.trim();

    const btn = document.getElementById('cora-vault-ai-submit-btn');
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = '<svg class="animate-spin" viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.5" fill="none"><circle cx="12" cy="12" r="10"></circle><path d="M4 12a8 8 0 018-8"></path></svg> <span>Thinking...</span>';
    }

    if (typeof window.coraShowToast === 'function') {
        window.coraShowToast('Cora AI is parsing document intent & calculating GST...', 'info');
    }
    ...
    jQuery.ajax({
        ...
        data: {
            ...
            security: (typeof coraREData !== 'undefined' ? coraREData.ajaxNonce : ''),
            sub_action: 'parse_and_propose',
            prompt: promptText
        },
        success: function(res) {
            if (btn) {
                btn.disabled = false;
                btn.innerHTML = '<span>Take Action</span> <svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.5" fill="none"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg>';
            }

            if (res.success && res.data && res.data.proposal) {
                currentVaultAIProposal = res.data.proposal;
                renderVaultAIProposal(res.data.proposal);
            } else {
                if (typeof window.coraShowToast === 'function') {
                    window.coraShowToast(res.data?.message || 'Could not parse document action.', 'error');
                }
            }
        },
        error: function() {
            ...
            if (typeof window.coraShowToast === 'function') {
                window.coraShowToast('Network error while consulting AI agent.', 'error');
            }
        }
    });
}

function renderVaultAIProposal(prop) {
    const box = document.getElementById('cora-vault-ai-proposal-box');
    const titleEl = document.getElementById('cora-vault-prop-title');
    const expEl = document.getElementById('cora-vault-prop-explanation');
    const bdEl = document.getElementById('cora-vault-prop-breakdown');
    if (!box || !titleEl || !expEl || !bdEl) return;

    titleEl.innerText = prop.title;
    expEl.innerText = prop.explanation || '';

    bdEl.innerHTML = `
        <div class="flex items-center gap-2">
            <span class="font-bold text-zinc-950">${prop.type}:</span>
            <span class="text-zinc-600">${prop.client_name}</span>
        </div>
        <div class="flex items-center gap-3 font-mono font-bold">
            <span class="text-zinc-500">Base: ₹${Number(prop.amount).toLocaleString('en-IN')}</span>
            <span class="text-zinc-500">+18% GST: ₹${Number(prop.tax_amount).toLocaleString('en-IN')}</span>
            <span class="text-zinc-950 text-sm">Total: ₹${Number(prop.grand_total).toLocaleString('en-IN')}</span>
        </div>
    `;
...
function coraConfirmVaultAIExecution() {
    if (!currentVaultAIProposal) return;
    const prop = currentVaultAIProposal;

    const execBtn = document.getElementById('cora-vault-prop-exec-btn');
    if (execBtn) {
        execBtn.disabled = true;
        execBtn.innerHTML = '<svg class="animate-spin" viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.5" fill="none"><circle cx="12" cy="12" r="10"></circle><path d="M4 12a8 8 0 018-8"></path></svg> <span>Saving to Vault...</span>';
    }
    ...
    jQuery.ajax({
        ...
        data: {
            action: 'cora_vault_ai_action',
            security: (typeof coraREData !== 'undefined' ? coraREData.ajaxNonce : ''),
            sub_action: 'save_document',
            doc_data: prop
        },
        success: function(res) {
            if (execBtn) {
                execBtn.disabled = false;
                execBtn.innerHTML = '<svg viewBox="0 0 24 24" width="13" height="13" stroke="currentColor" stroke-width="2.5" fill="none"><polyline points="20 6 9 17 4 12"></polyline></svg> <span>Execute &amp; Save to Vault</span>';
            }

            if (res.success) {
                $('#cora-vault-ai-proposal-box').addClass('hidden');
                $('#cora-vault-ai-prompt').val('');
                if (typeof window.coraShowToast === 'function') {
                    window.coraShowToast(res.data.message || 'Document created & saved to Vault!', 'success');
                }
                // Smoothly reload vault view to display new record
                setTimeout(function() {
                ...
                if (typeof window.coraShowToast === 'function') {
                    window.coraShowToast(res.data?.message || 'Failed to execute document action.', 'error');
                }
            }
        },
        error: function() {
            ...
            if (typeof window.coraShowToast === 'function') {
                window.coraShowToast('Network error while saving document.', 'error');
            }
        }
    });
}

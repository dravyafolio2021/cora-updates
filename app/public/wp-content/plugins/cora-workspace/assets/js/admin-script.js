...
jQuery(document).ready(function($) {
    ...
    window.CORA_PAGE_PRESETS = {
        dashboard: {
            ...
            actions: [
                ...
                },
                {
                    ...
                    prompt: 'Generate a new GST invoice with 18% tax breakdown.',
        ...
        forms: {
            ...
            actions: [
                {
                    id: 'create_lead_form',
                    label: 'Create High-Converting Lead Form',
                    prompt: 'Create a high-converting client lead capture form with name, email, phone, and budget fields.',
                ...
                {
                    id: 'multistep_logic',
                    label: 'Add 3-Step Multi-Stage Logic',
                    prompt: 'Convert the current intake form into a 3-step progressive onboarding questionnaire.',
                    icon: '<svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><polygon points="12 2 2 7 12 12 22 7 12 2"></polygon><polyline points="2 17 12 22 22 17"></polyline><polyline points="2 12 12 17 22 12"></polyline></svg>'
                },
                {
                    id: 'form_gst_calc',
                    label: 'Add GST Tax Calculation & E-Sign',
                    prompt: 'Add GST tax calculation breakdown and digital signature authorization to this form.',
                    icon: '<svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"></path><polyline points="14 2 14 8 20 8"></polyline><line x1="16" y1="13" x2="8" y2="13"></line><line x1="16" y1="17" x2="8" y2="17"></line></svg>'
                },
                {
                    id: 'audit_conversion',
                    label: 'Audit Form Conversion Bottlenecks',
                    prompt: 'Audit this form for mobile UX drop-off bottlenecks and field completion rate.',
                    icon: '<svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"></polyline></svg>'
                }
            ]
        },
        canvas: {
            ...
            actions: [
                ...
                {
                    id: 'pricing_grid',
                    label: 'Build 3-Tier Monochromatic Pricing Matrix',
                    prompt: 'Insert a 3-tier pricing matrix comparing Starter, Professional, and Enterprise packages.',
                    icon: '<svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><rect x="2" y="3" width="20" height="14" rx="2" ry="2"></rect><line x1="8" y1="21" x2="16" y2="21"></line><line x1="12" y1="17" x2="12" y2="21"></line></svg>'
                },
                {
                    id: 'mobile_audit',
                    label: 'Audit Canvas Mobile Responsiveness',
                    prompt: 'Audit current Canvas layout for mobile viewport overflow and tap target sizing.',
                    icon: '<svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><rect x="5" y="2" width="14" height="20" rx="2" ry="2"></rect><line x1="12" y1="18" x2="12.01" y2="18"></line></svg>'
                }
            ]
        },
        blogs: {
            ...
            actions: [
                ...
                {
                    id: 'seo_geo_audit',
                    label: 'Inspect SEO & GEO Ranking Gaps',
                    prompt: 'Inspect current draft for SEO gaps, heading hierarchy, keyword density, and schema markup.',
                ...
                {
                    id: 'faq_schema',
                    label: 'Extract Google FAQ Schema',
                    prompt: 'Extract 4 high-CTR Google People-Also-Ask FAQ schemas with answers for this post.',
                    icon: '<svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>'
                }
            ]
        },
        media: {
        ...
        settings: {
            ...
            actions: [
                ...
                },
                {
                    ...
                    prompt: 'Run an audit of our workspace security, active API keys, and cache state.',
    ...
    window.coraGetActivePageContext = function() {
        ...
        if (!view) {
            ...
            view = pathParts[pathParts.length - 1] || 'dashboard';
        }
        ...
        if (view.includes('canvas') || view.includes('builder') || view.includes('page')) return 'canvas';
        ...
        if (view.includes('user') || view.includes('role') || view.includes('team') || view.includes('member')) return 'users';
        if (view.includes('media') || view.includes('asset') || view.includes('gallery')) return 'media';
        if (view.includes('setting') || view.includes('config') || view.includes('admin')) return 'settings';
        return 'dashboard';
    };

    // Dynamically render page-aware contextual presets & update master AI panel headers
    window.coraRenderPageContextPresets = function() {
        ...
        (ctxData.actions || []).forEach(function(act) {
            if (act.requiredModule && !activeModules[act.requiredModule]) return;
    ...
    window.coraUpdateAIContext = window.coraRenderPageContextPresets;
    ...
    // AI Model Settings Drawer Controls
    window.coraOpenAISettingsDrawer = function(e) {
        if (e) e.stopPropagation();
        const backdrop = $('#cora-ai-settings-backdrop');
        const drawer = $('#cora-ai-settings-drawer');
        if (backdrop.length && drawer.length) {
            ...
            const savedModel = localStorage.getItem('cora_ai_active_model') || 'gemini';
            const savedTemp = localStorage.getItem('cora_ai_temperature') || '0.7';
            $(`input[name="cora_global_ai_model"][value="${savedModel}"]`).prop('checked', true);
            $('#cora-ai-temp-slider').val(savedTemp);
            $('#cora-ai-temp-display').text(savedTemp);
        } else if (typeof window.coraToggleAISettingsDrawer === 'function') {
            window.coraToggleAISettingsDrawer(true);
        } else {
            if (typeof window.coraShowToast === 'function') {
    ...
    window.coraSaveAISettings = function(e) {
        if (e) e.stopPropagation();
        const selectedModel = $('input[name="cora_global_ai_model"]:checked').val() || 'gemini';
        const temp = $('#cora-ai-temp-slider').val() || '0.7';
    ...
    // Master Unified AI Co-Founder Panel Controller
    ...
    // Mode Switcher: Chat (Text) vs Live Voice
    window.coraSetAIMode = function(mode) {
        const chatCont = $('#cora-ai-chat-mode-container');
    ...
    // Language Selector with Instant State & Recognition Sync
    window.coraSetAILanguage = function(lang) {
        localStorage.setItem('cora_voice_lang', lang);
        $('#cora-ai-lang-select, .cora-voice-lang-select').val(lang);
        if (window.coraVoiceEngine && typeof window.coraVoiceEngine.setLanguage === 'function') {
            window.coraVoiceEngine.setLanguage(lang);
        }
        if (typeof window.coraRestartVoiceRecognitionWithLang === 'function') {
            window.coraRestartVoiceRecognitionWithLang(lang);
        }
        const langNames = {
        ...
        if (window.coraShowToast) {
            window.coraShowToast('Language set to ' + (langNames[lang] || lang), 'info');
        }
    };

    // TTS Audio Speaker Toggle
    window.coraToggleAISpeaker = function() {
        if (typeof window.coraToggleVoiceSpeechSynthesis === 'function') {
            window.coraToggleVoiceSpeechSynthesis();
        }
    };

    // One-Tap Speech Dictation into Chat Input
    window.coraToggleSpeechDictation = function() {
        const SpeechRec = window.SpeechRecognition || window.webkitSpeechRecognition;
        if (!SpeechRec) {
            if (window.coraShowToast) window.coraShowToast('Speech recognition not supported in this browser.', 'warning');
            return;
        }
        ...
        try {
            ...
            const micBtn = $('#cora-ai-mic-btn');
            const input = $('#cora-sidebar-chat-input');
            micBtn.addClass('bg-red-50 text-red-600 border-red-300 animate-pulse');
            window._coraDictationActive = true;
            window._coraDictationRec = rec;

            if (window.coraShowToast) window.coraShowToast('Listening... speak your query', 'info');

            let initialVal = input.val();
            if (initialVal && !initialVal.endsWith(' ')) initialVal += ' ';
    ...
    // Submit Chat from Form Input
    window.coraSubmitPanelChat = function(event) {
    ...
    window.coraSendSidebarChat = function() {
        window.coraSubmitPanelChat();
    };

    // Backward-compatible wrappers for copilot and voice actions
    window.coraOpenCopilot = function() { window.coraOpenAIPanel('chat'); };
    window.coraCloseCopilot = function() { window.coraToggleSidebar(false); };
    window.coraOpenUniversalVoice = function() { window.coraOpenAIPanel('voice'); };

    // Toggle Unified AI Panel (Mobile Bottom Sheet + Desktop Side Panel)
    ...
    // Toggle button opens the unified panel
    ...
    // Send chat messages — reads from the native island input or panel input
    window.coraSendSidebarChatMessage = function() {
        const input = $('#cora-sidebar-chat-input').val().trim() ? $('#cora-sidebar-chat-input') : $('#cora-island-ai-input');
        const text = input.val().trim();
        if (!text) return;
                    ...
                    if (response.data.action_results && Array.isArray(response.data.action_results) && response.data.action_results.length > 0) {
                        response.data.action_results.forEach(function(act) {
                            ...
                            if (act.action === 'create_form') {
                                ...
                                cardHtml = `
                                    <div class="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-3 self-start max-w-[95%] w-full">
                                        <div class="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 pb-2.5">
                                            <div class="flex items-center gap-2">
                                                <div class="w-6 h-6 rounded-lg bg-zinc-950 dark:bg-white text-white dark:text-zinc-950 flex items-center justify-center font-bold text-[10px]">
                                                    <svg viewBox="0 0 24 24" width="12" height="12" stroke="currentColor" stroke-width="2" fill="none"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><line x1="9" y1="9" x2="15" y2="9"></line><line x1="9" y1="13" x2="15" y2="13"></line><line x1="9" y1="17" x2="15" y2="17"></line></svg>
                                                </div>
                                                <span class="text-xs font-bold text-zinc-900 dark:text-zinc-100">${d.title || 'Form Created'}</span>
                                            </div>
                                        ...
                                        <div class="space-y-1.5">
                                            <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Included Fields (${d.fields_count || 0}):</span>
                                            <div class="flex flex-wrap gap-1">${fieldsList}</div>
                                        </div>

                                        <div class="space-y-1">
                                            <span class="text-[10px] font-bold text-zinc-400 uppercase tracking-wider block">Public Shareable Link:</span>
                                            <div class="flex items-center gap-1.5 bg-zinc-50 dark:bg-zinc-950 p-1.5 rounded-xl border border-zinc-200 dark:border-zinc-800">
                                                <input type="text" readonly value="${d.public_url}" class="text-[11px] font-mono text-zinc-600 dark:text-zinc-300 bg-transparent flex-1 outline-none px-1 select-all" id="act-url-${d.form_id}">
                                                <button type="button" onclick="navigator.clipboard.writeText('${d.public_url}'); window.coraShowToast && window.coraShowToast('Public form link copied to clipboard!');" class="px-2.5 py-1 bg-zinc-950 text-white text-[10px] font-bold rounded-lg cursor-pointer hover:bg-zinc-800 transition-colors shrink-0">
                                                    Copy Link
                                                </button>
                                            </div>
                                        </div>
                                ...
                                cardHtml = `
                                    <div class="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-2.5 self-start max-w-[95%] w-full">
                                        ...
                                        <div class="text-xs text-zinc-600 dark:text-zinc-400 space-y-1">
                                            ...
                                            ${d.deal_value ? `<div>Deal Value: <strong class="font-mono text-zinc-800 dark:text-zinc-200">₹${Number(d.deal_value).toLocaleString()}</strong></div>` : ''}
                                        </div>
                                ...
                                `;
                            } else if (act.action === 'create_booking') {
                                cardHtml = `
                                    <div class="p-4 rounded-2xl bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 shadow-sm space-y-2.5 self-start max-w-[95%] w-full">
                                        <div class="flex items-center justify-between border-b border-zinc-100 dark:border-zinc-800 pb-2">
                                            <span class="text-xs font-bold text-zinc-900 dark:text-zinc-100">${d.title}</span>
                                            <span class="text-[10px] font-mono text-zinc-500">${d.date} ${d.time}</span>
                                        </div>
                                        <div class="text-xs text-zinc-600 dark:text-zinc-400">
                                            <div>Location: ${d.location} | Crew: ${d.crew}</div>
                                        </div>
                                        <a href="${d.calendar_url}" class="block py-1.5 text-center bg-zinc-950 text-white text-xs font-bold rounded-xl hover:bg-zinc-800 transition-colors">
                                            Open in Calendar
                                        </a>
                                    </div>
                                `;
    ...
    function initListingCoordinatorComponentsIfNeeded() {
        if (!coraQuillListingCoordinator && $('#cora-quill-editor').length > 0) {
            ...
            if (coraPendingEditorContent !== null) {
                coraQuillListingCoordinator.clipboard.dangerouslyPasteHTML(coraPendingEditorContent);
    ...
    window.coraOpenContentDrawer = function() {
        ...
        if (coraQuillListingCoordinator) {
            coraQuillListingCoordinator.setText('');
        } else {
    ...
    window.coraEditArticle = function(id) {
        ...
        initListingCoordinatorComponentsIfNeeded();
        if (coraQuillListingCoordinator) {
            coraQuillListingCoordinator.clipboard.dangerouslyPasteHTML('<p class="text-zinc-400 animate-pulse">Loading content from server...</p>');
        } else {
        ...
        $.post(ajaxurl, {
            ...
            if (response.success) {
                ...
                if (coraQuillListingCoordinator) {
                    coraQuillListingCoordinator.clipboard.dangerouslyPasteHTML(data.content || '');
                } else {
    ...
    window.coraOneClickDraft = function(keyword, title, description) {
        ...
        if (coraQuillListingCoordinator) {
            ...
            `;
            coraQuillListingCoordinator.clipboard.dangerouslyPasteHTML(html);
        }
    ...
    window.coraAutoOptimizeGEO = function() {
        ...
        coraQuillListingCoordinator.clipboard.dangerouslyPasteHTML(updatedHTML);
    ...
    window.coraUpdateWordCount = function() {
        ...
        if (window.coraQuillListingCoordinator && window.coraQuillListingCoordinator.root) {
            ...
            text = $('#cora-quill-editor').text() || '';
            headings = $('#cora-quill-editor').find('h1, h2, h3, h4').length;
            h1s = $('#cora-quill-editor').find('h1').length;
            images = $('#cora-quill-editor').find('img').length;
            links = $('#cora-quill-editor').find('a').length;
        ...
        $('#left-stat-words').text(words);
        $('#left-stat-headings').text(headings);
        $('#left-stat-images').text(images);
        $('#left-stat-links').text(links);
        $('#seo-stat-internal-links').text(internalLinks);
        $('#seo-stat-external-links').text(externalLinks);
        ...
        $('#left-score-value').text(`${score}/100`);
        $('#left-score-bar').css('width', `${score}%`);
        
        // Dynamic checklist sync
        const hasTitle = $('#cora-article-title').val() || '';
        const hasH1 = (h1s > 0 || hasTitle.trim().length > 0);
        const hasMeta = ($('#cora-article-excerpt-bh').val() || $('#cora-article-excerpt').val() || '').trim().length > 0;
        
        // H1 Tag Checklist
        if (hasH1) {
            $('#chk-chk-h1').text('✓').removeClass('text-zinc-300 text-red-500').addClass('text-emerald-600 font-bold');
            $('#chk-indicator-h1').text('Active').removeClass('text-red-500 bg-red-50').addClass('text-zinc-400');
        } else {
            $('#chk-chk-h1').text('✗').removeClass('text-zinc-300 text-emerald-600').addClass('text-red-500 font-bold');
            $('#chk-indicator-h1').text('Missing').removeClass('text-zinc-400').addClass('text-red-500');
        }
        
        // Meta Description Checklist
        if (hasMeta) {
            $('#chk-chk-meta').text('✓').removeClass('text-zinc-300 text-red-500').addClass('text-emerald-600 font-bold');
            $('#chk-indicator-meta').text('Active').removeClass('text-red-500').addClass('text-zinc-400');
        } else {
            $('#chk-chk-meta').text('✗').removeClass('text-zinc-300 text-emerald-600').addClass('text-red-500 font-bold');
            $('#chk-indicator-meta').text('Missing').removeClass('text-zinc-400').addClass('text-red-500');
        }
        
        // Image Alt Text Checklist
        if (images > 0) {
            if (altTextOk) {
                $('#chk-chk-alt').text('✓').removeClass('text-zinc-300 text-red-500').addClass('text-emerald-600 font-bold');
                $('#chk-indicator-alt').text('Active').removeClass('text-red-500').addClass('text-zinc-400');
            } else {
                $('#chk-chk-alt').text('✗').removeClass('text-zinc-300 text-emerald-600').addClass('text-red-500 font-bold');
                $('#chk-indicator-alt').text('No Alt').removeClass('text-zinc-400').addClass('text-red-500');
            }
        } else {
            $('#chk-chk-alt').text('✗').removeClass('text-zinc-300 text-emerald-600').addClass('text-red-500 font-bold');
            $('#chk-indicator-alt').text('No Images').removeClass('text-zinc-400').addClass('text-red-500');
        }
        ...
        if (criticalIssues === 0 && altTextOk && internalLinks > 0) {
            $('#checklist-issues-badge').text('Optimal').removeClass('bg-red-50 text-red-700 border-red-150 bg-amber-50 text-amber-700 border-amber-150').addClass('bg-emerald-50 text-emerald-700 border-emerald-150');
        } else if (criticalIssues > 0) {
            $('#checklist-issues-badge').text('Critical').removeClass('bg-emerald-50 text-emerald-700 border-emerald-150 bg-amber-50 text-amber-700 border-amber-150').addClass('bg-red-50 text-red-700 border-red-150');
        } else {
            $('#checklist-issues-badge').text('Needs Tweaks').removeClass('bg-emerald-50 text-emerald-700 border-emerald-150 bg-red-50 text-red-700 border-red-150').addClass('bg-amber-50 text-amber-700 border-amber-150');
        }
        
        if (score >= 80) {
            $('#left-score-grade').text('A').removeClass('bg-red-50 text-red-700 border-red-150 bg-amber-50 text-amber-700 border-amber-150').addClass('bg-emerald-50 text-emerald-700 border-emerald-150');
            $('#cora-seo-status-text').text('Optimized').removeClass('text-red-500 text-amber-500').addClass('text-emerald-600');
            if (ring) ring.setAttribute('class', 'text-emerald-500 transition-all duration-300');
            $('#left-score-message').text('Perfect optimization! Ready to publish.');
        } else if (score >= 50) {
            $('#left-score-grade').text('B').removeClass('bg-red-50 text-red-700 border-red-150 bg-emerald-50 text-emerald-700 border-emerald-150').addClass('bg-amber-50 text-amber-700 border-amber-150');
            $('#cora-seo-status-text').text('Good').removeClass('text-red-500 text-emerald-600').addClass('text-amber-500');
            if (ring) ring.setAttribute('class', 'text-amber-500 transition-all duration-300');
            $('#left-score-message').text('Good job! A few more tweaks and you\'re set.');
        } else {
            $('#left-score-grade').text('C').removeClass('bg-emerald-50 text-emerald-700 border-emerald-150 bg-amber-50 text-amber-700 border-amber-150').addClass('bg-red-50 text-red-700 border-red-150');
            $('#cora-seo-status-text').text('Poor').removeClass('text-emerald-600 text-amber-500').addClass('text-red-500');
            if (ring) ring.setAttribute('class', 'text-red-500 transition-all duration-300');
            $('#left-score-message').text('Needs attention. Fix the checklist issues.');
        }
    ...
    window.coraAIFixGrammar = function() {
        ...
        setTimeout(function() {
            ...
            coraQuillListingCoordinator.clipboard.dangerouslyPasteHTML(replacedHTML);
        ...
        if (pageId > 0) {
            ...
            $.post(coraREData.ajaxUrl, {
                ...
                if (res.success && res.data) {
                    ...
                    if (coraPageQuill) {
                        coraPageQuill.clipboard.dangerouslyPasteHTML(res.data.content || '');
                    }
                } else {
    ...
    window.coraSaveSystemSettingsSuite = function() {
        ...
        $.post(coraREData.ajaxUrl, data, function(res) {
            if (res && res.success) {
                ...
                const previousInd = coraREData.activeIndustry || 'real_estate';
                if (currentInd !== previousInd) {
                    window.coraShowToast(res.data.message || "Global system settings updated successfully. Reloading workspace...");
                    setTimeout(function() {
                        window.location.reload();
                    }, 1000);
                } else {
    ...
    $(document).on('input change', '#cora-brand-logo-url-suite', function() {
        ...
        var clearBtn = $('#cora-suite-logo-clear');
        if (url) {
    ...
    $(document).on('input change', '#cora-brand-favicon-url-suite', function() {
        ...
        var clearBtn = $('#cora-suite-favicon-clear');
    ...
    window.coraSubmitIslandAI = function() {
        ...
        window.coraOpenAIPanel('chat', prompt);
    };
    ...
    window.coraSetLeadTemperature = function(score) {
        ...
        if (score === 'converted' || score === 'won') {
            ...
            scoreBadge.attr('class', 'px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-rose-50 text-rose-700 dark:bg-rose-950/60 dark:text-rose-300 border border-rose-200 dark:border-rose-800 shrink-0').text('🔥 Hot');
        } else if (score === 'cold') {
            scoreBadge.attr('class', 'px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-sky-50 text-sky-700 dark:bg-sky-950/60 dark:text-sky-300 border border-sky-200 dark:border-sky-800 shrink-0').text('❄️ Cold');
        } else {
            scoreBadge.attr('class', 'px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-300 border border-amber-200 dark:border-amber-800 shrink-0').text('☀️ Warm');
        }
    ...
    (function initDrawerResizingEngine() {
        ...
        $(document).on('mousedown touchstart', '#cora-drawer-resize-handle', function(e) {
            ...
            $(window).on('mousemove touchmove', onMove).on('mouseup touchend', onEnd);
        });
    })();
    ...
    window.coraOpenLeadDetailDrawer = function(leadId) {
        ...
        const name = (memoryLead ? (memoryLead.names || memoryLead.name) : (card.length ? (card.attr('data-name') || card.data('name')) : '')) || 'Valid Enterprise Client';
        ...
        const price = (memoryLead ? memoryLead.price : (card.length ? (card.attr('data-price') || card.data('price')) : '')) || '₹4,50,000';
        ...
        const website = (memoryLead ? memoryLead.website : (card.length ? (card.attr('data-website') || card.data('website')) : '')) || 'https://client.com';
        ...
        const company = (memoryLead ? memoryLead.company : (card.length ? card.attr('data-company') : '')) || 'Valid Enterprise Pvt Ltd';
        const source = (memoryLead ? memoryLead.source : (card.length ? card.attr('data-source') : '')) || 'Website Intake Form';
        ...
        const qualPct = (score === 'hot') ? '94% Match' : ((score === 'cold') ? '64% Match' : '92% Match');
        ...
        $('#cora-drawer-stat-format').text(status);
        ...
        $('#cora-drawer-ai-recommended-move').text(milestone || `Send curated commercial lookbook and lock creative pitch call within 2 hours.`);

        // 8. Input Fields in Grouped Form Panes
        $('#cora-drawer-input-names').val(name);
        $('#cora-drawer-input-company').val(company);
        ...
        $('#cora-drawer-input-source').val(source);
    ...
    // Global Platform Redirection and Clipboard Sync Helpers
    window.coraGetReferQuery = function() {
        ...
        if (subPage === 'blogs' || document.getElementById('cora-view-content-suite')) {
            var activeContentTab = $('.cora-ct-panel:not(.hidden)').attr('id') || '';

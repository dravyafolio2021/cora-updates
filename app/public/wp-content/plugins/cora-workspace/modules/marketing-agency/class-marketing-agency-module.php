...
class Cora_Marketing_Agency_Module implements Cora_Module_Interface {
    public function get_module_id() : string {
        return 'marketing_agency';
    }

    public function get_display_name() : string {
        return 'Marketing & Digital Agency';
    }
    ...
    public function get_navigation_groups(string $active_role) : array {
        ...
        // 1. Workspace group (Platform Foundation - permanently active)
        ...
        if ( $is_enabled( 'affiliates' ) || $is_enabled( 'referrals' ) ) {
            $workspace_items['affiliates'] = array(
                'title' => 'Affiliates & Referrals',
                'badge' => '40%',
                'icon'  => '<svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="1.8" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><line x1="19" y1="8" x2="19" y2="14"></line><line x1="22" y1="11" x2="16" y2="11"></line></svg>'
            );
        }
        ...
        // 3. Sales Channel & Acquisition group
        $sales_items = array();
        ...
        if ( ! empty( $sales_items ) ) {
            $groups[] = array( 'label' => 'Sales Channel', 'items' => $sales_items );
        }

        // 4. AI Growth Engine group
        $ai_items = array();
        if ( $is_enabled( 'gbp' ) ) {
            $ai_items['gbp'] = array(
        ...
        if ( ! empty( $ai_items ) ) {
            $groups[] = array( 'label' => 'AI Marketing', 'items' => $ai_items );
        }
    ...
    public function get_industry_roles() : array {
        return array(
            ...
            'cora_manager'             => 'Account Director',
            'cora_media_buyer'         => 'Performance Media Buyer',
            'cora_copywriter'          => 'Content Strategist & Copywriter',
            'cora_designer'            => 'Creative Designer & Video Editor',
            'cora_seo_specialist'      => 'SEO & Technical Specialist',
            'cora_client_stakeholder'  => 'Client Stakeholder'
        );
    }

    public function get_crm_stages() : array {
        return array(
            'Inbound Inquiry' => array(
                'label' => 'Inbound Inquiry',
                'badge' => 'bg-blue-100 border border-blue-200/60',
                'desc'  => 'New incoming leads & RFPs'
            ),
            'Discovery & Audit' => array(
                'label' => 'Discovery & Audit',
                'badge' => 'bg-indigo-100 border border-indigo-200/60',
                'desc'  => 'Technical audit & strategy session'
            ),
            'Proposal & Pitch' => array(
                'label' => 'Proposal & Pitch',
                'badge' => 'bg-purple-100 border border-purple-200/60',
                'desc'  => 'Pitch deck & scope of work presented'
            ),
            'Retainer Review' => array(
                'label' => 'Retainer Review',
                'badge' => 'bg-amber-100 border border-amber-200/60',
                'desc'  => 'Contract review & scope negotiation'
            ),
            'Active Retainer' => array(
                'label' => 'Active Retainer',
                'badge' => 'bg-emerald-100 border border-emerald-200/60',
                'desc'  => 'Client onboarded on recurring retainer'
            ),
            'Completed / Paused' => array(
                'label' => 'Completed / Paused',
                'badge' => 'bg-zinc-100 border border-zinc-200/60',
                'desc'  => 'Past or paused engagements'
            )
        );
    }

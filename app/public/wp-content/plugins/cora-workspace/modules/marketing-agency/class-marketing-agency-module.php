...
class Cora_Marketing_Agency_Module implements Cora_Module_Interface {
    ...
    public function get_navigation_groups(string $active_role) : array {
        ...
        // 2. Agency Operations group
        $ops_items = array();
        if ( $is_enabled( 'leads' ) ) {
            $ops_items['leads'] = array(
                'title' => 'Client Deals (CRM)',
        ...
        if ( ! empty( $ops_items ) ) {
            $groups[] = array( 'label' => 'Agency Operations', 'items' => $ops_items );
        }
        ...
        if ( $is_enabled( 'emails' ) ) {
            $sales_items['emails'] = array(
                'title' => 'Campaign Emails & Outreach',
        ...
        if ( ! empty( $sales_items ) ) {
            $groups[] = array( 'label' => 'Acquisition & Funnels', 'items' => $sales_items );
        }
        ...
        if ( $is_enabled( 'gbp' ) ) {
            $ai_items['gbp'] = array(
                'title' => 'Google Profile & Local SEO',
        ...
        if ( ! empty( $ai_items ) ) {
            $groups[] = array( 'label' => 'AI Growth Engine', 'items' => $ai_items );
        }

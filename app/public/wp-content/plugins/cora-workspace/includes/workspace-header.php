...
function cora_render_workspace_header( $args = array() ) {
    ...
    (function() {
        ...
        window.coraAskExternalPlatform = function(platform, e) {
            ...
            var now = Date.now();
            if (now - _coraPlatformLastOpen < 1200) {
                return false;
            }
            _coraPlatformLastOpen = now;
            ...
            if (url) {
                if (window.coraShowToast) {
                    window.coraShowToast('Opening ' + platformName + '...' + (query ? ' Query copied to clipboard!' : ''), 'info');
                }
    ...
    <div class="cora-workspace-header select-none w-full max-w-full min-w-0 overflow-visible <?php echo esc_attr( $args['container_class'] ); ?>">
        ...
        <div class="<?php echo esc_attr( $desktop_header_classes ); ?>">
            ...
            <div class="flex items-center gap-3 shrink-0">
                ...
                <div class="cora-platform-stack flex items-center select-none mr-1" style="display: flex; align-items: center;">
                    ...
                    <!-- Gemini Button -->
                    <button type="button" onclick="coraAskExternalPlatform('gemini', event); event.stopPropagation(); return false;" class="group relative w-8 h-8 rounded-full border-0 bg-blue-5/70 hover:bg-blue-100/50 flex items-center justify-center text-blue-600 transition-all duration-200 hover:-translate-y-0.5 hover:scale-110 hover:z-50 shadow-2xs cursor-pointer focus:outline-none" style="z-index: 3 !important; margin-left: -10px !important;">
        ...
        <div class="flex md:hidden items-center justify-between gap-3 mb-1 px-0 py-2 border-b border-zinc-200/60 bg-white select-none">
            ...
            <div class="flex items-center gap-2 shrink-0">
                ...
                <div class="cora-platform-stack flex items-center select-none mr-0.5" style="display: flex; align-items: center;">
                    <!-- ChatGPT Button -->
                    <button type="button" onclick="coraAskExternalPlatform('openai', event); event.stopPropagation(); return false;" class="group relative rounded-full border border-emerald-100 bg-emerald-50/70 flex items-center justify-center text-emerald-600 transition-all duration-200 hover:-translate-y-0.5 shadow-2xs cursor-pointer focus:outline-none" style="width: 24px; height: 24px; min-width: 24px; min-height: 24px; z-index: 5 !important; margin-left: 0px !important; padding: 0 !important;">
                        <svg viewBox="0 0 24 24" width="12" height="12" fill="currentColor"><path d="M9.205 8.658v-2.26c0-.19.072-.333.238-.428l4.543-2.616c.619-.357 1.356-.523 2.117-.523 2.854 0 4.662 2.212 4.662 4.566 0 .167 0 .357-.024.547l-4.71-2.759a.797.797 0 00-.856 0l-5.97 3.473zm10.609 8.8V12.06c0-.333-.143-.57-.429-.737l-5.97-3.473 1.95-1.118a.433.433 0 01.476 0l4.543 2.617c1.309.76 2.189 2.378 2.189 3.948 0 1.808-1.07 3.473-2.76 4.163zM7.802 12.703l-1.95-1.142c-.167-.095-.239-.238-.239-.428V5.899c0-2.545 1.95-4.472 4.591-4.472 1 0 1.927.333 2.712.928L8.23 5.067c-.285.166-.428.404-.428.737v6.898zM12 15.128l-2.795-1.57v-3.33L12 8.658l2.795 1.57v3.33L12 15.128zm1.796 7.23c-1 0-1.927-.332-2.712-.927l4.686-2.712c.285-.166.428-.404.428-.737v-6.898l1.974 1.142c.167.095.238.238.238.428v5.233c0 2.545-1.974 4.472-4.614 4.472zm-5.637-5.303l-4.544-2.617c-1.308-.761-2.188-2.378-2.188-3.948A4.482 4.482 0 014.21 6.327v5.423c0 .333.143.571.428.738l5.947 3.449-1.95 1.118a.432 4.432 0 01-.476 0zm-.262 3.9c-2.688 0-4.662-2.021-4.662-4.519 0-.19.024-.38.047-.57l4.686 2.71c.286.167.571.167.856 0l5.97-3.448v2.26c0 .19-.07.333-.237.428l-4.543 2.616c-.619.357-1.356.523-2.117.523z"/></svg>
                    </button>
                    <!-- Claude Button -->
                    <button type="button" onclick="coraAskExternalPlatform('claude', event); event.stopPropagation(); return false;" class="group relative rounded-full border border-amber-100 bg-amber-50/70 flex items-center justify-center text-amber-600 transition-all duration-200 hover:-translate-y-0.5 shadow-2xs cursor-pointer focus:outline-none" style="width: 24px; height: 24px; min-width: 24px; min-height: 24px; z-index: 4 !important; margin-left: -7px !important; padding: 0 !important;">
    ...
    <?php if ( ! empty( $visible_tabs ) ) : ?>
        <div class="cora-sub-tabs-container cora-sticky-sub-tabs flex items-center gap-1 border-b border-zinc-200/80 dark:border-zinc-800 -mx-3 sm:-mx-4 md:-mx-6 lg:-mx-8 pl-0 pr-3 sm:px-4 md:px-6 lg:px-8 mt-4 pt-1 pb-0 mb-6 select-none overflow-x-auto scrollbar-hide bg-[#FBFaf7] dark:bg-[#0c0c0e]" id="<?php echo esc_attr( $args['tabs_dom_id'] ?? 'cora-sub-navigation-tabs' ); ?>" style="position: -webkit-sticky; position: sticky; top: var(--cora-topbar-height, 48px); z-index: 35; flex-wrap: nowrap; -webkit-overflow-scrolling: touch; scrollbar-width: none; min-height: 44px;">

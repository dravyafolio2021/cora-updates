...
function cora_docs_get_new_pages_v3246() {
    return array(
        ...
        // ── Finance AI Co-founder (v3.4.44) ───────────────────────────────
        array(
            'slug'     => 'finance-ai-cofounder',
            'title'    => 'Finance AI Co-founder',
            'category' => 'finance',
            'content'  => "# Finance AI Co-founder\n\nStarting in v3.4.44, Cora replaces traditional static accounting dashboards with an **AI Financial Co-founder** that watches cash flow, alerts founders to revenue risks, and prepares next actions.\n\n## Core Pillars\n- **Cora's Morning Take**: Proactive plain-English analysis comparing liquid cash buffer with monthly operational burn rate.\n- **Action-Oriented Attention Cards**: Surfaces overdue invoices with 1-click multi-channel follow-ups (Email / WhatsApp Direct).\n- **Cash Flow Trajectory (Chart.js)**: 6-month historical inflow/outflow ledger combined with a 90-day predictive runway forecast.\n- **Ask Cora Copilot**: Context-aware AI assistant answering queries on owner draw limits, upcoming renewals, and tax liabilities.\n\n## AJAX API Reference\n| Action | Parameters | Description |\n|---|---|---|\n| `cora_ajax_finance_ask_cora` | `security`, `query` | AI copilot query processor |\n| `cora_ajax_finance_send_reminder` | `security`, `invoice_id`, `recipient_email`, `tone`, `message` | Dispatches client payment reminder |"
        ),
        array(
            'slug'     => 'finance-gst-invoicing',
            'title'    => 'Indian GST Invoicing & Vault Linking',
            'category' => 'finance',
            'content'  => "# Indian GST Invoicing & Vault Linking\n\nThe 3-step GST Invoice Creator allows solo founders to issue compliant invoices with automatic tax detection and E-Sign contract linking.\n\n## Multi-Step Creator\n1. **Client & Place of Supply**: Auto-populates contact details from CRM (`wp_cora_leads`) and determines Intra-State (CGST 9% + SGST 9%) vs Inter-State (IGST 18%).\n2. **SAC Codes & Line Items**: Dynamic line item calculation with Service Accounting Codes (`998386`, `998314`) and real-time tax math summary.\n3. **Terms & Document Vault**: Links legal E-Sign agreements from the Document Vault directly to invoice payment milestones.\n\n## AJAX API Reference\n| Action | Parameters | Description |\n|---|---|---|\n| `cora_ajax_create_invoice` | `security`, `client_name`, `client_email`, `total_amount`, `tax_pct`, `line_items` | Generates new GST invoice and stores in `cora_invoices` |\n| `cora_ajax_finance_record_income` | `security`, `client_name`, `amount`, `date`, `invoice_id` | Reconciles invoice balance and writes inflow to master ledger |"
        ),
        array(
            'slug'     => 'finance-deal-simulator',
            'title'    => 'Deal Feasibility Simulator',
            'category' => 'finance',
            'content'  => "# Deal Feasibility Simulator\n\nThe Deal Feasibility Simulator evaluates prospective project quotes against direct subcontractor costs, equipment rentals, travel expenses, and mandatory 18% GST reserves before a founder signs a client contract.\n\n## Scoring Model\n- **Net Profit & Margin %**: Calculates exact bottom-line take-home profit.\n- **AI Feasibility Verdict**: Evaluates margins against agency profitability benchmarks (High Margin Go > 45%, Moderate Margin Review 30-45%, Low Margin Warning < 30%).\n- **1-Click Conversion**: Generates a pre-filled client invoice proposal directly from approved simulator numbers."
        ),
        array(
            'slug'     => 'finance-reports',
            'title'    => 'Tax, GST & Accountant Pack',
            'category' => 'finance',
            'content'  => "# Tax, GST & Accountant Pack\n\nThe Financials module automates tax reserve calculations and provides 1-click export packs for external Chartered Accountants (CAs).\n\n## Features\n- **Output vs Input GST**: Compares collected GST against Input Tax Credit (ITC) from eligible expense receipts.\n- **Quarterly Advance Tax Reserve**: Recommended 15% income tax reserve allocation.\n- **Customizable Expense Categories**: User-defined category creation via `+ Custom` with server persistence.\n- **1-Click Accountant Export**: Downloads a bundled ZIP with itemized CSV ledgers formatted for CA audit.\n\n## AJAX API Reference\n| Action | Parameters | Description |\n|---|---|---|\n| `cora_ajax_finance_save_category` | `security`, `category` | Saves user custom category |\n| `cora_ajax_finance_export_pack` | `security`, `period`, `include_invoices`, `include_expenses` | Generates CA ZIP audit pack |"
        ),
        ...
        array(
            ...
            'title'    => 'Content Suite',
            'category' => 'content',
            'content'  => "# Content Suite\n\nThe Content Suite is an AI-powered content creation and publishing hub for agencies managing social media, blog posts, and marketing copy.\n\n## Features\n- **AI Content Generation**: Generate captions, blog posts, and email copy using the configured LLM (Gemini/Claude).\n- **WhatsApp-Style AI Chat**: Conversational short-bubble chat interface (max 2-3 sentences per bubble) designed for quick founder decisions.\n- **RCS Card Formatter**: Automatically parses structured lists and action recommendations into neat checklist cards with vector checkmarks and gray borders.\n- **Real-Time Credits Widget**: Displays session token credit usage in a sticky progress bar synchronized with `localStorage` (`cora_ai_credits_used`).\n- **Width-Locked Suggestions Toggle**: Supports collapsing the suggestions sidebar (saving state to `localStorage` under `cora_suggestions_collapsed`) and applies a locked width grid system (`max-w-[720px]` open, `max-w-[500px]` collapsed).\n- **Clean Drawer Redirects**: Strips URL query parameters (`action` and `post_id`) before reload redirection to ensure the slide-out editor drawer closes on save, submission, or approval.\n- **Content Calendar**: Visual monthly publishing calendar.\n- **Platform Publishing**: Direct publish to Instagram (via Meta API), LinkedIn, and blog posts via WordPress.\n- **Brand Voice**: Per-agency brand voice configuration saved to the RAG layer for consistent AI tone.\n\n## AJAX API Reference\n| Action | Parameters | Description |\n|---|---|---|\n| `cora_ajax_generate_content` | `nonce`, `type`, `prompt`, `brand_voice` | Generates AI content |\n| `cora_ajax_schedule_post` | `nonce`, `content_id`, `platform`, `publish_at` | Schedules a post |\n| `cora_ajax_get_content_calendar` | `nonce`, `month`, `year` | Returns publishing calendar |"
        ),
        array(
